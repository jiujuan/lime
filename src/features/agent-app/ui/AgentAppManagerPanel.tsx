...
import {
  getAgentAppManagerStatus,
  type AgentAppManagerEvidenceSummary,
  type AgentAppManagerLifecycleStatus,
} from "./agentAppManagerStatus";

function managerStatusTone(status: AgentAppManagerLifecycleStatus): string {
...
export function AgentAppManagerPanel({
  flow,
  disabled,
  selectedAppId,
  onSetDisabled,
  ...
  onSetDisabled: (disabled: boolean, state: InstalledAgentAppState) => void;
  onSelectApp: (appId: string) => void;
  onLaunchEntry: (entry: ProjectedEntry, state: InstalledAgentAppState) => void;
  onPreviewUninstall: (
    mode: "keep-data" | "delete-data",
    state: InstalledAgentAppState,
  ) => void;
  evidence: AgentAppManagerEvidenceSummary | null;
  uiRuntimeAvailable: boolean;
  capabilityHostAvailable: boolean;
  repositoryStates: InstalledAgentAppState[];
  ...
  const status = getAgentAppManagerStatus({
    installedState,
    canLaunch: selectedCanLaunch,
    disabled,
  });
  const launchDisabledReason = disabled
    ? t("agentApp.lab.manager.launchDisabled.disabled")
    : t("agentApp.lab.manager.launchDisabled.guard");

  return (
    <div className="space-y-4" data-testid="agent-app-manager">
      <div className={`rounded-2xl border p-4 ${managerStatusTone(status)}`}>
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-sm font-semibold">
              {t(`agentApp.lab.manager.status.${status}`)}
            </p>
          ...
          <span className="rounded-full border border-white/70 bg-white/70 px-2 py-1 text-xs font-medium">
            {t("agentApp.lab.manager.installedApps", {
      ...
      >
        <div className="flex flex-wrap items-center justify-between gap-3">
          <p className="text-sm font-semibold text-slate-900">
            {t("agentApp.lab.manager.repository")}
          </p>
          <p className="text-sm text-slate-600">
            {t("agentApp.lab.manager.repositorySummary", {
              count: repositoryList.length,
        ...
        <div className="mt-3 grid gap-2 md:grid-cols-2">
          {repositoryList.map((state) => {
            ...
            const itemStatus = getAgentAppManagerStatus({
            ...
            return (
              <button
                ...
                data-testid={`agent-app-manager-repository-app-${state.appId}`}
              ...
              >
                <span className="flex items-center justify-between gap-3">
                  ...
                  >
                    {t(`agentApp.lab.manager.status.${itemStatus}`)}
                  </span>
                </span>
      ...
      <div className="grid gap-3 lg:grid-cols-3" data-testid="agent-app-manager-selected-app">
        <div
          className="rounded-2xl border border-slate-200 bg-slate-50 p-4"
          data-testid="agent-app-manager-repository"
        >
          <p className="text-sm font-semibold text-slate-900">
            {t("agentApp.lab.manager.identity")}
          </p>
        ...
        <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
          <p className="text-sm font-semibold text-slate-900">
            {t("agentApp.lab.manager.readiness")}
          </p>
          <p className="mt-2 text-sm text-slate-600">
            {t("agentApp.lab.manager.readinessSummary", {
      ...
      <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <p className="text-sm font-semibold text-slate-900">
            {t("agentApp.lab.manager.lifecycle")}
          </p>
          <div className="mt-3 flex flex-wrap gap-2">
            ...
            >
              <Ban size={14} />
              {t("agentApp.lab.manager.action.disable")}
            </button>
            <button
              type="button"
            ...
            >
              ...
              {t("agentApp.lab.manager.action.enable")}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
        <div className="mb-3 flex items-center gap-2 text-sm font-semibold text-slate-900">
          <Layers3 size={16} />
          {t("agentApp.lab.manager.entries")}
        </div>
          ...
          {installedState?.projection.entries.map((entry) => {
            ...
            return (
              <button
                ...
                data-testid={`agent-app-manager-launch-entry-${entry.key}`}
                disabled={isDisabled}
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <button
          ...
          data-testid="agent-app-manager-uninstall-keep-data"
          onClick={() => installedState && onPreviewUninstall("keep-data", installedState)}
          disabled={!installedState}
        >
          <span className="inline-flex items-center gap-2 text-sm font-semibold text-slate-900">
            <Archive size={16} />
            {t("agentApp.lab.manager.action.keepData")}
          </span>
          <span className="mt-2 block text-sm text-slate-600">
            {t("agentApp.lab.manager.uninstallCounts", {
        ...
        >
          <span className="inline-flex items-center gap-2 text-sm font-semibold text-rose-900">
            <Archive size={16} />
            {t("agentApp.lab.manager.action.deleteData")}
          </span>
      ...
      {evidence ? (
        ...
        >
          <div className="flex flex-wrap items-center gap-2">
            <span className="rounded-full bg-white px-2 py-1 text-xs font-medium text-emerald-700">
              {t(`agentApp.lab.manager.evidence.action.${evidence.action}`)}
            </span>
            {evidence.entryKey ? (
              ...
              {evidence.residualAudit ? (
                ...
                >
                  ...
                  <span
                    ...
                    data-testid="agent-app-manager-residual-retained"
                  >
                  ...
                  >
                    {t("agentApp.lab.manager.evidence.residual.pendingDeletion", {
                  ...
                  <span
                    ...
                    data-testid="agent-app-manager-residual-blocked"
                  >
                    {t("agentApp.lab.manager.evidence.residual.blockedOutOfScope", {
                  ...
                  <span
                    ...
                    data-testid="agent-app-manager-residual-repository"
                  >
                    {t("agentApp.lab.manager.evidence.residual.repositoryIssue", {
        ...
        <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">
          {t("agentApp.lab.manager.evidence.empty")}
        </div>
      )}
    </div>
  );

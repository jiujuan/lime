...
import {
  ...
  type AgentAppCloudReleaseAuditCheckStatus,
  type AgentAppCloudReleaseEvidence,
  type AgentAppCloudReleaseEvidenceStatus,
...
const AUDIT_CHECK_CLASS: Record<AgentAppCloudReleaseAuditCheckStatus, string> = {
...
export function AgentAppReleaseEvidenceSummary({
  evidence,
}: {
  evidence?: AgentAppCloudReleaseEvidence;
}) {
  ...
  const rows: Array<{
    ...
    {
      key: "package",
      labelKey: "agentApp.apps.installReview.releaseEvidence.check.packageHash",
    ...
    {
      key: "manifest",
      labelKey: "agentApp.apps.installReview.releaseEvidence.check.manifestHash",
    ...
    {
      key: "signature",
      labelKey: "agentApp.apps.installReview.releaseEvidence.check.signature",
      status: signatureCheckStatus,
      value: evidence.signatureRef
        ? shortenHash(evidence.signatureRef)
        : t("agentApp.apps.installReview.releaseEvidence.emptyValue"),
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="text-sm font-semibold text-[color:var(--lime-text-strong)]">
            {t("agentApp.apps.installReview.releaseEvidence.title")}
          </p>
          <p className="mt-1 text-xs leading-5 text-[color:var(--lime-text-muted)]">
            {t("agentApp.apps.installReview.releaseEvidence.sourceLine", {
              catalog: t(
                `agentApp.apps.installReview.releaseEvidence.catalogSource.${evidence.catalogSource}`,
              ),
              source: t(
                `agentApp.apps.installReview.releaseEvidence.sourceKind.${evidence.sourceKind}`,
              ),
            })}
          </p>
        </div>
        ...
        >
          {t(
            `agentApp.apps.installReview.releaseEvidence.status.${evidence.status}`,
          )}
        </span>
      </div>
      <dl className="mt-3 divide-y divide-[color:var(--lime-surface-border)] text-sm">
        {rows.map((row) => (
          <div
            ...
            data-testid={`agent-apps-install-review-release-evidence-${row.key}`}
          >
            ...
            >
              {t(
                `agentApp.apps.installReview.releaseEvidence.checkStatus.${row.status}`,
              )}
            </dd>
          </div>
        ))}
      ...
      >
        <div className="flex flex-wrap items-center justify-between gap-2">
          <span className="text-xs font-semibold text-[color:var(--lime-text-strong)]">
            {t("agentApp.apps.installReview.releaseEvidence.audit.title")}
          </span>
          <div className="flex flex-wrap items-center justify-end gap-2">
            <span className="text-xs text-[color:var(--lime-text-muted)]">
              {t("agentApp.apps.installReview.releaseEvidence.audit.counts", {
            ...
            <button
              ...
              title={t(
                "agentApp.apps.installReview.releaseEvidence.audit.copyTitle",
              )}
            >
              <ClipboardCopy size={13} />
              {t("agentApp.apps.installReview.releaseEvidence.audit.copy")}
            </button>
          </div>
        </div>
        {copyState !== "idle" ? (
          ...
          >
            {t(
              `agentApp.apps.installReview.releaseEvidence.audit.copyState.${copyState}`,
            )}
          </p>
        ) : null}
        <div className="mt-2 flex flex-wrap gap-1.5">
          {auditSummary.checks.map((check) => (
            <span
              ...
              data-testid={`agent-apps-install-review-release-audit-${check.key}`}
              title={check.issueCodes.join(", ")}
            >
              {t(
                `agentApp.apps.installReview.releaseEvidence.audit.check.${check.key}`,
              )}
              <span className="ml-1 opacity-80">
                {t(
                  `agentApp.apps.installReview.releaseEvidence.audit.checkStatus.${check.status}`,
                )}
              </span>
            </span>
          ))}

...
describe("MockCapabilityHost", () => {
  it("应通过 SDK facade 运行 entry 并生成带 agent_app provenance 的产物和证据", async () => {
  ...
  it("delete-data 卸载应清理 package、projection、readiness、storage、artifact 和 evidence", async () => {
    ...
    expect(deletedValues).toEqual(
      expect.arrayContaining([
        "<LimeAppData>/agent-apps/projections/content-factory-app.json",
        "<LimeAppData>/agent-apps/readiness/content-factory-app.json",

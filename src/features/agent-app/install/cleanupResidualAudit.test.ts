...
import { buildAgentAppCleanupResidualAudit } from "./cleanupResidualAudit";
...
describe("cleanup residual audit P16-H.4", () => {
  it("delete-data audit 应把 evidence delete targets 标为 pending deletion", () => {
    ...
    const audit = buildAgentAppCleanupResidualAudit({
      state,
      cleanupEvidence,
      generatedAt: now,
    });
      ...
      repositoryIssues: [
        {
          code: "PARSE_FAILED",
          appId: "content-factory-app",
          path: "<LimeAppData>/agent-apps/installed/content-factory-app.json",
          message: "Installed state JSON parse failed.",
        },
        {
          code: "READ_FAILED",
          appId: "other-app",
          path: "<LimeAppData>/agent-apps/installed/other-app.json",

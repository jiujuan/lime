...
describe("Agent App standalone release pipeline", () => {
  ...
  it("缺少 build artifact refs 时必须阻断发布", () => {
    ...
    const plan = buildStandaloneReleasePipelinePlan({
      ...
      buildEvidence: {
        status: "completed",
        artifactRefs: [],
        evidenceRef: ".lime/agent-apps/build.json",
      },
    });
    ...
    expect(plan).toMatchObject({
      ...
      buildEvidenceRef: ".lime/agent-apps/build.json",
  ...
  it("产物级 evidence 齐全时仍不会绕过 non-production descriptor 门禁", () => {
    ...
    expect(plan).toMatchObject({
      status: "blocked",
      readyToPublish: false,
      signingEvidenceRef: ".lime/agent-apps/signing.json",
      notarizationEvidenceRef: ".lime/agent-apps/notarization.json",
      updaterManifestRef: ".lime/agent-apps/latest.json",
      rollbackManifestRef: ".lime/agent-apps/rollback.json",
    });
  });
});

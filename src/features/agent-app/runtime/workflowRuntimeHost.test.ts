...
import {
  WorkflowRuntimeHost,
  type AgentAppWorkflowExecutionContext,
  type AgentAppWorkflowStep,
...
describe("WorkflowRuntimeHost", () => {
    ...
    const result = await runtime.runWorkflow({
      ...
      steps: [
        ...
        },
        {
          ...
          refs: (context: AgentAppWorkflowExecutionContext) => [context.artifactIds[0]],
          assignTo: "evidence",
        },
      ],
    });
      ...
      steps: [
        {
          ...
          idempotencyKey: (context: AgentAppWorkflowExecutionContext) =>
            `${context.runId}:scenario`,
          input: (context: AgentAppWorkflowExecutionContext) => ({
  ...
  it("应拒绝未注册的 raw worker / network 类 step", async () => {
    ...
    } as unknown as AgentAppWorkflowStep;

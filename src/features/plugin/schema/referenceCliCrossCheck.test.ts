...
const STANDARD_ROOT = resolve(
  process.env.LIME_PLUGIN_STANDARD_ROOT ??
    resolve(process.cwd(), "../../limecloud/plugin"),
);
const REFERENCE_CLI = join(STANDARD_ROOT, "bin/plugin-ref.mjs");
...
function buildClientArtifacts(referenceProjection: ReferenceProjection) {
  ...
  const review = buildPluginInstallReview({
    preview,
...
describeIfReferenceAvailable(
  "Plugin P17.2.5 standard reference cross-check",
  () => {
    ...
    it("应把上游 projection schema 字段映射到 Lime projection，缺口必须显式登记", () => {
      ...
      const acceptedDivergences: Record<string, string> = {
        ...
        triggers:
          "Plugin v0.5/v0.6 discovery metadata; Lime projection keeps runtime readiness first and will project discovery metadata in P18.7-C/F.",
        quickstart:
          "Plugin v0.5/v0.6 onboarding metadata; Lime setup guidance is still derived from readiness until P18.7-C/F.",
        skills:
          "Plugin v0.5/v0.6 skill package metadata; runtime skill binding projection is deferred to P18.7-D.",
        publisher:
          "Plugin v0.5/v0.6 marketplace metadata; install review currently keeps package identity only.",
        author:
          "Plugin v0.5/v0.6 marketplace metadata; install review currently keeps package identity only.",
        maintainers:
          "Plugin v0.5/v0.6 marketplace metadata; install review currently keeps package identity only.",
        contributors:
          "Plugin v0.5/v0.6 marketplace metadata; install review currently keeps package identity only.",
        timeline:
          "Plugin v0.5/v0.6 release metadata; package lifecycle projection is deferred until cloud release review.",
        supportWindow:
          "Plugin v0.5/v0.6 support metadata; host policy projection is deferred until marketplace install review.",
        links:
          "Plugin v0.5/v0.6 external links; current Lime projection avoids exposing external metadata before policy review.",
        license:
          "Plugin v0.5/v0.6 license metadata; install review currently keeps package identity only.",
        support:
          "Plugin v0.5/v0.6 support metadata; install review currently keeps package identity only.",
        distribution:
          "Plugin v0.5/v0.6 distribution metadata; cloud catalog projection owns this later.",
        compliance:
          "Plugin v0.5/v0.6 compliance metadata; enterprise policy projection owns this later.",
        errors:
          "reference CLI diagnostics field; Lime projection reports errors through validation/readiness surfaces.",
        i18n: "Plugin v0.5/v0.6 localization metadata; Lime projection does not yet expose localized marketplace copy.",
        signature:
          "Plugin v0.5/v0.6 signature metadata; package verification owns signature state later.",
        health:
          "Plugin v0.5/v0.6 health metadata; runtime health projection is deferred to Host discovery.",
        agentRuntime:
          "Plugin v0.6 task runtime control plane is preserved on normalized manifest; projection into runtime policy is deferred to Plugin runtime execution.",
        requirements:
          "Plugin v0.7 requirement boundary is preserved on normalized manifest; projection into install/readiness UX is deferred to capability handoff review.",
        boundary:
          "Plugin v0.7 responsibility boundary is preserved on normalized manifest; projection into Host/Cloud setup UX is deferred to capability handoff review.",
        integrations:
          "Plugin v0.7 external integration requirements are preserved on normalized manifest; connector readiness projection is deferred to Host/Cloud setup review.",
        operations:
          "Plugin v0.7 operation side-effect policy is preserved on normalized manifest; execution gating remains Host policy responsibility.",
      };

...
import {
  buildPluginAgentUiProjectionEvents,
  buildPluginStandardRuntimeEvents,
} from "./agentUiProjectionBridge";

describe("buildPluginAgentUiProjectionEvents", () => {
  it("把 Plugin runtime projection payload 展开为有序 AgentUI text / tool events", () => {
    const events = buildPluginAgentUiProjectionEvents({
      appId: "content-factory-app",
      taskId: "task-1",
      sessionId: "session-1",
      threadId: "thread-1",
      runId: "plugin_runtime:content-factory-app:task-1",
      startSequence: 7,
      events: [
        {
  ...
  it("同时输出标准 Agent Runtime execution events 供共享 projection 消费", () => {
    const events = buildPluginStandardRuntimeEvents({

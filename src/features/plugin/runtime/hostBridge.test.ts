...
vi.mock("@/lib/api/plugins", () => ({
  selectPluginDirectory: pluginApiMocks.selectPluginDirectory,
}));
...
describe("PluginHostBridge", () => {
  afterEach(() => {
    ...
    pluginApiMocks.selectPluginDirectory.mockReset();
  ...
  it("构建 host snapshot 时应携带 Lime 主题 token 和 Host 上下文", () => {
    ...
    const theme = buildPluginThemePayload();
    const snapshot = buildPluginHostSnapshot({
      appId,
      entryKey,
      displayName: "内容工厂",
  ...
  it("只信任当前 iframe、runtime origin 和匹配 appId 的标准消息", () => {
    ...
    expect(
      isTrustedPluginBridgeMessage(trusted, {
    ...
    expect(
      isTrustedPluginBridgeMessage(wrongOrigin, {
        appWindow: frame.contentWindow,
        runtimeOrigin,
        appId,
        entryKey,
    ...
    expect(
      isTrustedPluginBridgeMessage(wrongProtocol, {
        appWindow: frame.contentWindow,
        runtimeOrigin,
        appId,
        entryKey,
    ...
    expect(
      isTrustedPluginBridgeMessage(missingEntryKey, {
        appWindow: frame.contentWindow,
        runtimeOrigin,
        appId,
        entryKey,
    ...
    expect(
      isTrustedPluginBridgeMessage(wrongEntryKey, {
        appWindow: frame.contentWindow,
        runtimeOrigin,
        appId,
        entryKey,
  ...
  it("lime.ui selectDirectory 应调用宿主封装目录选择命令", async () => {
    pluginApiMocks.selectPluginDirectory.mockResolvedValue({
      path: "/Users/example/plugin",
      cancelled: false,
    });
    const { frame, postMessage } = buildFrame();
    const bridge = new PluginHostBridge({
    ...
    expect(pluginApiMocks.selectPluginDirectory).toHaveBeenCalledWith({
      title: "选择应用目录",
    });
    expect(postMessage).toHaveBeenCalledWith(
      expect.objectContaining({
    ...
    try {
      ...
      expect(postMessage).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          requestId: "req-1",
          payload: expect.objectContaining({
            subscriptionId: "plugin-subscription-1",
            taskId: "task-1",
            bridgeAction: "contentFactoryProduction",
          }),
        }),
  ...
  it("订阅 task 时应同时接入 AgentRuntime event bus 并转发运行时事件", async () => {
    ...
    try {
      ...
      expect(listenRuntimeEventMock).toHaveBeenCalledWith(
        "plugin_runtime:content-factory-app:task-1",
        expect.any(Function),
      );
      ...
      expect(postMessage).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          payload: expect.objectContaining({
            ...
            taskId: "task-1",
            bridgeAction: "contentFactoryProduction",
            runtimeEventName: "plugin_runtime:content-factory-app:task-1",
            events: [
              expect.objectContaining({

...
import { buildPluginProvenance } from "../sdk/provenance";
...
interface AdapterCapabilityHostOptions {
  ...
  store?: InMemoryPluginCapabilityStore;
  now?: () => string;
}
...
export class AdapterCapabilityHost implements CapabilityHost {
  ...
  private readonly store: InMemoryPluginCapabilityStore;
  ...
  constructor(options: AdapterCapabilityHostOptions) {
    ...
    this.store = options.store ?? new InMemoryPluginCapabilityStore();
  ...
  async runEntry(entryKey: string): Promise<PluginRunResult> {
    ...
    const run: PluginRunRecord = {
      ...
      storageKeys: [],
      taskIds: [],
      provenance: buildPluginProvenance({
        preview: this.preview,
        entryKey,
        runId,
      }),
    ...
    const storageEntry = await sdk.storage.set(`runs/${runId}`, {
      ...
      status: "running",
      title: entry.title,
      adapter: "local-plugin-capability-store",
    });
  ...
  async uninstall(params: {
    ...
    const storageTargets = this.getStorageEntries({ appId }).map((entry) =>
      refTarget(
        ...
        "Plugin adapter storage entry.",
      ),
    );
    const artifactTargets = this.getArtifacts({ appId }).map((artifact) =>
      refTarget(`artifact:${artifact.id}`, "Plugin adapter artifact."),
    );
    const evidenceTargets = this.getEvidence({ appId }).map((item) =>
      refTarget(`evidence:${item.id}`, "Plugin adapter evidence."),
    );
    const taskTargets = this.getTasks({ appId }).map((task) =>
      refTarget(`task:${task.taskId}`, "Plugin adapter task."),
    );
    const dataTargets = [
      ...
      retryTask: async (task) => {
        ...
        if (!sourceTask) {
          throw new PluginCapabilityError({
            code: "TASK_NOT_FOUND",
            message: `Plugin task ${taskId} was not found.`,
            appId: this.preview.identity.appId,
            entryKey: provenance.entryKey,
  ...
  private submitHostResponse(
    ...
    if (!this.store.getTask(input.taskId)) {
      throw new PluginCapabilityError({
        code: "TASK_NOT_FOUND",
        message: `Plugin task ${input.taskId} was not found.`,
        appId: this.preview.identity.appId,
        entryKey: provenance.entryKey,
        capability: "lime.agent",
      });
  ...
  private completeTask(taskId: string, result: unknown): PluginTaskRecord {
  ...
  private assertRealAdapterEnabled(): void {
    ...
    throw new PluginCapabilityError({
      ...
      message: "Plugin real adapter host is disabled.",
      appId: this.preview.identity.appId,
    });
  }
  ...
  private assertCapabilityEnabled(
    ...
    throw new PluginCapabilityError({
      ...
      message: `${capability} is not available through the Plugin adapter host.`,
      appId: this.preview.identity.appId,
      entryKey,
      capability,
    });

...
const agentSkillPaths = agentSkillIds.map(
...
for (const token of requiredWorkbenchTokens) {
  assert(
    ...
    `app.workbench.yaml missing token: ${token}`,
  );
}
...
for (const connectorId of [
  ...
  assert(
    connectorRegistry.connectors?.some(
...
assert(
  resources.entries?.some(
    (entry) => entry.activationKey === "content_article_generate",
  ),
...
assert(
  schema.properties?.appId?.const === "content-factory-app",
  "schema appId const mismatch",
);
assert(
...
const samplePatch = JSON.parse(
  await readFile(
    path.join(root, "examples/workspace-patch.sample.json"),
    "utf8",
  ),
);
assert(
  samplePatch.appId === "content-factory-app",
  "sample patch appId mismatch",
);
assert(
...
assert(
  samplePatch.objects.some(
    (object) => typeof object.source?.documentText === "string",
  ),
...
const runtimeRequest = JSON.parse(
  await readFile(
    path.join(root, "examples/runtime-request.sample.json"),
    "utf8",
  ),
);
const {
...
const fullRuntimeResult = runContentFactoryTask({
  ...
  topic: "内容工厂校验",
  audience: "运营团队",
  hostManagedGeneration: runtimeRequest.hostManagedGeneration,
});
assert(
...
const fullRuntimeArticle = fullRuntimeResult.patch.objects.find(
  (object) => object.ref?.kind === "articleDraft",
);
assert(
  String(fullRuntimeArticle?.source?.documentText ?? "").includes(
    "人才选聘不能只看简历关键词",
  ) && fullRuntimeArticle?.source?.articleGenerationStatus === "complete",
  "full runtime articleDraft must come from host-managed generation",
);
...
const runtimeProgressEvents =
  buildContentFactoryWorkerProgressEvents(runtimeRequest);
assert(
...
connectorAuditEvents.forEach((event) => {
  assert(
    ...
    "worker audit progress must use runtime.event envelope",
  );
  assert(
    event.payload?.stepId === "research",
    "connector audit event must bind research step",
  );
  assert(
    event.payload?.connectorRef === "web-research",
    "connector audit event must bind web-research connector",
  );
  assert(
    event.payload?.toolName === "WebSearch",
    "connector audit event must name WebSearch",
  );
  assert(
    event.payload?.auditOnly === true,
    "connector audit event must be audit-only",
  );
});
...
artifactProgressEvents.forEach((event, index) => {
  assert(
    event.kind === "runtime.event",
    "worker progress must use runtime.event envelope",
  );
  assert(
...
assert(
  String(article.source?.documentText ?? "").includes(
    "人才选聘不能只看简历关键词",
  ) &&
    String(article.source?.documentText ?? "").includes(
      "## 用任务验证真实能力",
...
for (const locale of ["zh-CN", "zh-TW", "en-US", "ja-JP", "ko-KR"]) {
  ...
  assert(
    ...
    `locale ${locale} missing app.displayName`,
  );
  assert(

...
test("content.factory.generate outputs a full Article Workspace patch", () => {
  const result = runContentFactoryTask({
    ...
    topic: "内容工厂工作台",
    audience: "运营团队",
    hostManagedGeneration: withHostManagedGeneration({}).hostManagedGeneration,
  });
  ...
  assert.ok(
    result.patch.objects.some((object) => object.ref.kind === "contentBrief"),
  );
  assert.ok(
    result.patch.objects.some((object) => object.ref.kind === "articleDraft"),
  );
  assert.ok(
    result.patch.objects.some(
  ...
  assert.ok(
    result.patch.objects.some(
      (object) => object.ref.kind === "deliveryChecklist",
    ),
  );
  ...
  assert.match(
    result.patch.workerEvidence[0].researchRounds[0].query,
    /内容工厂工作台/,
  );
  ...
  const article = result.patch.objects.find(
    (object) => object.ref.kind === "articleDraft",
  );
  assert.equal(article?.source.documentText, HOST_GENERATED_MARKDOWN);
  assert.equal(article?.source.articleGenerationStatus, "complete");
});
...
test("host worker request returns artifact snapshot response", () => {
  ...
  assert.match(article.source.processMarkdown ?? "", /待执行检索/);
...
test("host worker request emits paragraph-level artifact progress events", () => {
  ...
  const finalPatch =
    response.artifacts[0].metadata.contentFactoryWorkspacePatch;
  ...
  assert.equal(
    artifactProgressEvents[0].payload.artifact.metadata.complete,
    false,
  );
  assert.equal(
    artifactProgressEvents[0].payload.artifact.metadata.streamSequence,
    1,
  );
...
test("host managed generation output overrides deterministic article draft text", () => {
  ...
  assert.equal(
    article?.source.hostManagedGeneration?.provider,
    "test-provider",
  );
...
test("content.factory.generate host request fails closed without generated article text", () => {
  const request = {
    schemaVersion: "content-factory.worker-request.v1",
    appId: "content-factory-app",
    sessionId: "session-factory-missing-001",
    workspaceId: "workspace-main",
    turnId: "turn-factory-missing-001",
    taskId: "task-factory-missing-001",
    taskKind: "content.factory.generate",
    prompt: "用内容工厂生成一篇关于团队知识库治理的文章",
    expectedOutput: {
      artifactKind: "content_factory.workspace_patch",
    },
    runtime: {
  ...
  assert.deepEqual(buildContentFactoryWorkerProgressEvents(request), []);
  const response = handleContentFactoryWorkerRequest(request);
...
test("content.image.generate selects image grid as the primary surface", () => {
  ...
  assert.equal(
    imageObject?.source.images[0]?.cache?.status,
    "pending_executor",
  );
  assert.match(
    imageObject?.source.images[0]?.cache?.relativePath ?? "",
    /\.lime\/plugins\/content-factory-app\/sessions\/session-image-001\/tasks\//,
  );
  assert.equal(
...
test("content.video.storyboard.generate declares video cache executor contracts", () => {
  ...
  assert.equal(storyboardObject?.source.shots[0]?.cache?.mimeType, "video/mp4");
  assert.match(

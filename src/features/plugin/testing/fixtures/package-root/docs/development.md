...
- 如果宿主无可用 provider、生成失败，或当前 backend 没返回 `hostManagedGenerationResult`，`content.article.generate` / `content.factory.generate` 必须 fail closed，不得使用 deterministic fallback 生成合同样例或伪造文章正文。

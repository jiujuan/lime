...
```bash
PLUGIN_SIGNING_PRIVATE_KEY_PEM="$PRIVATE_KEY_PEM" npm run release:sign -- \
  --package-url https://updates.limeai.run/plugins/content-factory-app-2.2.2.lapp \
  --package-hash sha256:<package-sha256> \
  --manifest-hash sha256:<manifest-sha256> \
  --release-id plugin-release-xxxx \
  --signature-ref sigstore:content-factory-app@2.2.2 \
  --public-key-id plugin-root-2026 \
  --out app.signature.yaml \
  --trust-root-out plugin-signature-trust-root.json
```

`app.signature.yaml` 可以随发布包进入 LimeCore 上传 / release 创建流程，LimeCore 会从 `signature.package` 派生 `signatureProof`；`plugin-signature-trust-root.json` 只用于配置 LimeCore / OEM runtime 的 `pluginSignatureTrustRoots`，不能放进 remote catalog 让 App 自签通过。生产私钥只能通过环境变量或本地密钥文件传入，不得提交到仓库。
...
- 发布到 remote catalog 时必须提供 package hash、manifest hash、`signatureRef` 和 `signatureProof`；catalog 只提供 proof，不能自带可信公钥。`signatureProof.publicKeyId` 必须命中 Lime 宿主运行时配置或 bootstrap 快照中的 `pluginSignatureTrustRoots`，且签名 payload 必须绑定 `appId / version / packageUrl / packageHash / manifestHash / releaseId / signatureRef / signatureProof.publicKeyId / signatureProof.algorithm / signatureProof.signedAt`，安装审查生成 `releaseEvidence.signatureVerificationStatus=verified` 后才允许确认安装。
...
- Lime 宿主可信根只允许来自 `window.__LIME_OEM_CLOUD__.pluginSignatureTrustRoots`、`window.__LIME_BOOTSTRAP__.pluginSignatureTrustRoots` 或 `window.__LIME_BOOTSTRAP__.plugins.signatureTrustRoots`。可信根字段支持 `publicKeyId / algorithm / publicKey / appIds / notBefore / notAfter / revoked / revokedAt`；日期字段格式错误时应按无可信根处理。

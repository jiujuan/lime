        ...
        return [
          {
            ...
            targetObjectKind: normalizeText(
              item.targetObjectKind ?? item.target_object_kind,
              "",
            ),
            outputField: normalizeText(
              item.outputField ?? item.output_field,
              "",
            ),
...
function declaredHostManagedGenerationFallback(request) {
  ...
  if (
    ...
    !taskRequiresGeneratedArticleText(normalizeText(request.taskKind, ""))
  ) {
    return null;
  }
  const requests = Array.isArray(declaration.requests)
...
function hostManagedGenerationFromRequest(request) {
  return (
    (hasHostManagedGenerationResult(request.hostManagedGeneration)
      ...
      : null) ??
    (hasHostManagedGenerationResult(
      request.runtime?.hostManagedGenerationResult,
    )
      ? normalizeHostManagedGeneration(
          request.runtime?.hostManagedGenerationResult,
        )
      : null) ??
...
function generatedDocumentTextFromHost(hostManagedGeneration) {
  ...
  const output = hostManagedGeneration.outputs.find(
    (item) =>
      (item.targetObjectKind === "articleDraft" || !item.targetObjectKind) &&
      (item.outputField === "documentText" ||
        item.outputField === "finalMarkdown" ||
        !item.outputField) &&
      item.content,
  );
...
function taskRequiresGeneratedArticleText(taskKind) {
  return ["content.article.generate", "content.factory.generate"].includes(
    normalizeText(taskKind, ""),
  );
}
...
function normalizeHostWorkerRequest(input) {
  ...
  if (
    taskRequiresGeneratedArticleText(taskKind) &&
...
function workspacePatchArtifact({
  artifactId,
  patch,
  metadata = {},
  status = "ready",
...
function workflowConnectorRequestedEvents(context, patch) {
  if (
    !["content.article.generate", "content.factory.generate"].includes(
      context.taskKind,
    ) ||
  ...
  return searchRequests.map((request, index) =>
    runtimeEventEnvelope("workflow.connector.requested", {
      ...
      connectorExpectedOutput: normalizeText(
...
function buildArticleObject(context) {
  ...
  const hasGeneratedDocumentText = Boolean(generatedDocumentText);
  const articleTitle =
    markdownTitleFromDocumentText(generatedDocumentText) ||
    context.topic ||
    `${context.channel}文章草稿`;
  const documentText = generatedDocumentText;
  const excerpt = markdownExcerptFromDocumentText(documentText) || "";
  const generationMetadata = context.hostManagedGeneration
    ? {
        ...
        outputIds: context.hostManagedGeneration.outputs
  ...
  return {
    ...
    title: articleTitle,
    status: hasGeneratedDocumentText ? "needs_review" : "draft",
    summary: hasGeneratedDocumentText
      ? `围绕“${articleTitle || context.topic}”生成首版${context.channel}正文，可继续编辑、审稿和发布。`
      : "等待宿主托管生成正文后进入编辑、审稿和发布流程。",
    previewArtifactId: artifactIds[0],
    source: {
      ...sourceBase({
      ...
      articleGenerationStatus: hasGeneratedDocumentText
        ? "complete"
...
export function buildContentFactoryWorkerProgressEvents(input = {}) {
  ...
  const artifactEvents = prefixes.map((prefix, index) => {
    const partialPatch = setArticleDocumentText(
      cloneJson(patch),
      prefix,
      "streaming",
    );

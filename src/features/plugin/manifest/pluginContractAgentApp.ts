...
export function buildPluginRightSurfaceFromAgentAppManifest(
  manifest: NormalizedAppManifest,
): PluginRightSurfaceContract {
  return convertAgentRightSurface(buildAgentAppRightSurfaceContract(manifest));
}
  ...
  return {
    ...
    workflows: (manifest.workflows ?? []).map((workflow) => {
      ...
      return {
        ...
        hookPolicy: normalizeAgentAppWorkflowHookPolicy(workflow),
    ...
    clis: clisFromAgentApp(manifest),
    hooks: hooksFromAgentApp(manifest),
...
function normalizeAgentAppWorkflowHookPolicy(

...
function normalizeManifestVersion(
  ...
  throw new Error(`Unsupported Plugin manifest version: ${version}`);
}
...
function normalizeProfiles(manifest: AppManifest): PluginProfile[] {
  const profiles = new Set<PluginProfile>();

...
import {
  PLUGIN_HOST_FLAGS_STORAGE_KEY,
  resolvePluginHostFlags,
} from "./featureFlag";
...
describe("Plugin feature flags", () => {
  ...
  it("默认关闭 Lab 和所有运行能力", () => {
    expect(resolvePluginHostFlags()).toMatchObject({
      labEnabled: false,
  ...
  it("开启 labEnabled 时只打开 P0 只读链路", () => {
    expect(resolvePluginHostFlags({ labEnabled: true })).toMatchObject({
      labEnabled: true,
  ...
  it("开启 mockSdkEnabled 时应进入 P1 mock SDK 但仍不启用真实运行时", () => {
    expect(resolvePluginHostFlags({ mockSdkEnabled: true })).toMatchObject({
      labEnabled: true,
      localPackageEnabled: true,
      projectionEnabled: true,
      readinessEnabled: true,
  ...
  it("生产构建不能通过 override、env 或 localStorage 启用 mock SDK", () => {
    ...
    vi.stubEnv("VITEST", "");
    vi.stubEnv("VITE_LIME_PLUGIN_MOCK_SDK", "1");
  ...
  it("开启 realAdapterEnabled 时应进入 P2 adapter 链路但仍不启用 UI/worker runtime", () => {
    expect(
      resolvePluginHostFlags({ realAdapterEnabled: true }),
    ).toMatchObject({
      labEnabled: true,
      localPackageEnabled: true,
      projectionEnabled: true,
  ...
  it("开启 uiRuntimeEnabled 时应进入 P3 UI host 但仍不启用 worker runtime", () => {
    expect(resolvePluginHostFlags({ uiRuntimeEnabled: true })).toMatchObject({
      labEnabled: true,
      localPackageEnabled: true,
      projectionEnabled: true,
      readinessEnabled: true,
  ...
  it("开启 workerRuntimeEnabled 时应进入 P4.2 workflow runtime 但不自动启用 adapter", () => {
    expect(
      resolvePluginHostFlags({ workerRuntimeEnabled: true }),
    ).toMatchObject({
      labEnabled: true,
      localPackageEnabled: true,
      projectionEnabled: true,
  ...
  it("开启 cloudBootstrapEnabled 时只进入 P5 bootstrap 输入层，不自动运行 App", () => {
    expect(
      resolvePluginHostFlags({ cloudBootstrapEnabled: true }),
    ).toMatchObject({
      labEnabled: true,
      localPackageEnabled: true,
      projectionEnabled: true,

...
function buildReadyManagerFixture() {
  const flags = resolvePluginHostFlags({
  ...
  const flow = evaluatePluginLabInstallFlow({
    preview,
    setup,
    flags,
  ...
  const primary = flow.installedState ?? buildInstalledPluginState({ preview, setup });
...
async function renderPanel(params?: {
  ...
  evidence?: Parameters<typeof PluginManagerPanel>[0]["evidence"];
}) {
...
describe("PluginManagerPanel", () => {
  ...
  it("从 repository states 渲染多 App 列表并支持选择", async () => {
    ...
    expect(
      container.querySelector('[data-testid="plugin-manager-repository-list"]'),
    ).not.toBeNull();
    expect(
      container.querySelectorAll('[data-testid^="plugin-manager-repository-app-"]'),
    ).toHaveLength(2);
    expect(container.textContent).toContain("plugin.lab.manager.repositorySummary:2");
  ...
  it("entry launcher 和 lifecycle actions 绑定当前选中的 App state", async () => {
    ...
    expect(
      container.querySelector('[data-testid="plugin-manager-selected-app"]')?.textContent,
    ).toContain("fixture:content-factory-playbook-app");

    const launchButton = container.querySelector(
      '[data-testid="plugin-manager-launch-entry-dashboard"]',
    ) as HTMLButtonElement | null;
    await act(async () => {
      launchButton?.click();
    });
  ...
  it("展示 cleanup rehearsal JSON summary 并保留 selected app 身份", async () => {
    ...
    const evidence: Parameters<typeof PluginManagerPanel>[0]["evidence"] = {
        ...
        targets: [
          {
            ...
            kind: "path",
            value:
              "<LimeAppData>/plugins/installed/content-factory-playbook-app.json",
            exists: "unknown",
            safeToDelete: true,
            reason: "Installed Plugin state snapshot.",
            disposition: "delete",
          },
          {
            ...
            kind: "namespace",
            value:
              "<LimeAppData>/plugins/storage/content-factory-playbook-app",
            exists: "unknown",
            safeToDelete: true,
    ...
    expect(
      container.querySelector('[data-testid="plugin-manager-residual-audit"]'),
    ).not.toBeNull();
    expect(
      container.querySelector('[data-testid="plugin-manager-residual-pending"]')
        ?.textContent,
    ).toContain("plugin.lab.manager.evidence.residual.pendingDeletion:1");
    expect(
      container.querySelector('[data-testid="plugin-manager-residual-blocked"]')
        ?.textContent,
    ).toContain("plugin.lab.manager.evidence.residual.blockedOutOfScope:1");
  });
});

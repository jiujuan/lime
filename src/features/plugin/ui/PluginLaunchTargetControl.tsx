...
interface PluginLaunchTargetControlProps {
  policy: PluginLaunchTargetPolicy;
  selectedTargetId: string | null;
  onModeChange: (mode: PluginLaunchTargetMode) => void;
  onSelectedTargetIdChange: (targetId: string | null) => void;
}

type PluginLaunchTargetTranslate = (
  key: string,
  params?: Record<string, string | number>,
) => string;

function resolveTargetLabel(
  option: PluginRightSurfaceLaunchTargetOption,
  index: number,
  translate: PluginLaunchTargetTranslate,
): string {
  return (
    option.label ??
    translate("plugin.apps.launchTarget.targetFallback", { index: index + 1 })
  );
}

export function PluginLaunchTargetControl({
  policy,
  selectedTargetId,
  onModeChange,
  onSelectedTargetIdChange,
}: PluginLaunchTargetControlProps) {
  const { t } = useTranslation("agent");
  const translate = useMemo<PluginLaunchTargetTranslate>(() => {
    const baseTranslate = t as unknown as PluginLaunchTargetTranslate;
  ...
  return (
    <div
      ...
      data-testid="plugins-launch-target-policy"
    >
      <div className="min-w-0">
        <p className="text-xs font-semibold text-[color:var(--lime-text-strong)]">
          {translate("plugin.apps.launchTarget.label")}
        </p>
        {!policy.rightSurfaceAvailable ? (
          ...
          >
            {translate("plugin.apps.launchTarget.rightSurfaceUnavailable")}
          </p>
        ) : currentTarget ? (
          <p
            className="mt-0.5 truncate text-xs text-[color:var(--lime-text-muted)]"
            data-testid="plugins-launch-target-current"
            title={currentTarget.description ?? currentTarget.label}
          >
            {translate("plugin.apps.launchTarget.targetHint", {
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        targetLabels.length > 1 ? (
          <select
            ...
            aria-label={translate("plugin.apps.launchTarget.targetSelect")}
            value={selectValue}
            onChange={(event) =>
        ...
        <div className="inline-flex rounded-full border border-[color:var(--lime-surface-border)] bg-[color:var(--lime-surface)] p-1">
          <button
            ...
            data-testid="plugins-launch-target-standalone"
          >
            {translate("plugin.apps.launchTarget.standalone")}
          </button>
          <button
            type="button"
            className={`h-8 rounded-full px-3 text-xs font-semibold transition ${
          ...
          >
            {translate("plugin.apps.launchTarget.rightSurface")}
          </button>
        </div>
      </div>
    </div>

...
import type { PluginCloudReleaseEvidenceCatalogSource } from "../install/cloudReleaseEvidence";
import type {
  PluginHostFunctionStatus,
...
import {
  ...
  type PluginReadinessIssueCategory,
  type PluginReadinessIssueCategorySummary,
...
export type AppCenterActionLabelKey =
  | "plugin.apps.center.action.open"
  | "plugin.apps.center.action.install"
...
export interface AppCenterItem {
  ...
  hostLifecycle?: PluginHostLifecycleSnapshot;
...
export interface AppCenterHostLifecycleSummary {
  ...
  primaryIssueCategory: PluginReadinessIssueCategory | null;
...
function getAppDescription(
  installedState: InstalledPluginState | undefined,
  cloudApp: CloudBootstrapApp | undefined,
): string {
  return (
...
function getAppTitle(
  ...
  return (
    ...
    (installedState
      ? resolveInstalledPluginDisplayName(installedState)
      : null) ??
    appId
  );
}
...
export function buildAppCenterItems(params: {
  ...
  convertLocalFileSrc?: ConvertLocalFileSrc;
  hostLifecycleSnapshots?: PluginHostLifecycleSnapshot[];
}): AppCenterItem[] {
    ...
    .map((appId) => {
      ...
      const hostLifecycle =
        ...
        (params.hostLifecycleSnapshots === undefined && installedState
          ? buildPluginHostLifecycleForInstalledState(installedState)
          : undefined);
...
export function getActionLabelKey(
  ...
  if (item.installedState) {
    if (item.statusKind === "disabled") {
      return "plugin.apps.center.action.enable";
    }
    if (item.registrationBlocked) {
      return "plugin.apps.center.action.activate";
    }
    if (canOneClickUpdate(item)) {
      return "plugin.apps.center.action.updateOneClick";
    }
    return "plugin.apps.center.action.open";
  }
  if (item.statusKind === "registration") {
    return "plugin.apps.center.action.activate";
  }
  if (item.statusKind === "update") {
    return "plugin.apps.center.action.update";
  }
  if (item.statusKind === "installable" || !item.installedState) {
    return "plugin.apps.center.action.install";
  }
  if (item.statusKind === "disabled") {
    return "plugin.apps.center.action.enable";
  }
...
const HOST_LIFECYCLE_STATUS_LABEL_KEYS: Record<
  ...
  string
> = {
  ready: "plugin.apps.center.host.status.ready",
  "needs-setup": "plugin.apps.center.host.status.needsSetup",
  blocked: "plugin.apps.center.host.status.blocked",
  delisted: "plugin.apps.center.host.status.delisted",
  planned: "plugin.apps.center.host.status.planned",
};
...
const READINESS_ISSUE_CATEGORIES = new Set<PluginReadinessIssueCategory>([
  "legacy",
  "package",
  "cloud",
  "runtime",
...
function normalizeReadinessIssueCategory(
  ...
  return READINESS_ISSUE_CATEGORIES.has(
    normalized as PluginReadinessIssueCategory,
  )
    ? (normalized as PluginReadinessIssueCategory)
    : null;
}
...
function normalizeServerIssueCategories(
  lifecycle: PluginHostLifecycleSnapshot,
...
export function buildAppCenterHostLifecycleSummary(
  ...
  const issueCategories =
    serverIssueCategories.length > 0
      ...
      : summarizePluginReadinessIssueCategories(lifecycle.blockers);
  ...
  return {
    ...
    primaryIssueCategory:
      ...
      getPrimaryPluginReadinessIssueCategory(issueCategories),
    issueCategories,

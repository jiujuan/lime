...
describe("installed Plugin plugin projection", () => {
  it("应把已安装 Plugin 投影成插件 contract 与可激活 registry", () => {
  ...
  it("坏 manifest 应 fail closed 并记录 skippedAppIds", () => {
    ...
    expect(projectPluginContractsFromInstalledPlugins([broken])).toEqual({
      contracts: [],
      skippedAppIds: ["broken-app"],
    });
    expect(projectPluginRegistryFromInstalledPlugins([broken])).toMatchObject(
      {
        contracts: [],
        skippedAppIds: ["broken-app"],
        projectionInputs: [],

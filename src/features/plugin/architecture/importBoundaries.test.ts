...
const PLUGIN_ROOT = path.resolve(process.cwd(), "src/features/plugin");
...
function readModuleSources(moduleName: string): Array<{
  ...
  return collectProductionSourceFiles(path.join(PLUGIN_ROOT, moduleName)).map(
    (absolutePath) => ({
      absolutePath,
...
function readPluginProductionSources(): ReturnType<typeof readModuleSources> {
  return collectProductionSourceFiles(PLUGIN_ROOT).map((absolutePath) => ({
    absolutePath,
    relativePath: toPosixPath(path.relative(process.cwd(), absolutePath)),
    source: readFileSync(absolutePath, "utf8"),
  }));
...
describe("Plugin v2 import boundaries", () => {
  ...
  it("UI 不直接触碰 Desktop Host invoke、旧桌面宿主 API 或 DevBridge，只能走 plugins API gateway", () => {
    const files = readModuleSources("ui");

    expectNoForbiddenPattern(
      files,
      LEGACY_HOST_IMPORT_PATTERN,
      "UI 组件不能直接依赖旧桌面宿主 API；统一经 src/lib/api/plugins.ts。",
    );
    expectNoForbiddenPattern(
      files,
      /from\s+["']@\/lib\/desktop-host(?:\/|["'])/,
      "UI 组件不能直接依赖 Desktop Host API；统一经 src/lib/api/plugins.ts。",
    );
    expectNoForbiddenPattern(
      files,
      /from\s+["']@\/lib\/dev-bridge["']/,
      "UI 组件不能直接依赖 DevBridge；统一经 src/lib/api/plugins.ts。",
    );
    expectNoForbiddenPattern(
      files,
      /\b(?:safeInvoke|invoke)\s*\(/,
  ...
  it("Runtime / SDK 不依赖 Shell adapter，避免 standalone 反向污染核心运行时", () => {
    ...
    expectNoForbiddenPattern(
      files,
      /from\s+["'][^"']*(?:\/shell|features\/plugin\/shell)[^"']*["']/,
      "Runtime / SDK 只能读取 RuntimeProfile / capability contract，不能 import shell adapter。",
    );
  });

  it("Plugin production code 不直接 import 旧桌面宿主 API", () => {
    const files = readPluginProductionSources();

    expectNoForbiddenPattern(
      files,
      LEGACY_HOST_DIRECT_IMPORT_PATTERN,
      "Plugin current 生产代码不能直接 import 旧桌面宿主 API；宿主能力必须经 Desktop Host / App Server adapter 或 packaging artifact seam。",
    );
  });

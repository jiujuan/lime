...
function buildTaskRecord(input: { title: string }): PluginTaskRecord {
  return {
    taskId: "task-1",
    traceId: "trace-1",
    appId: "content-factory-app",
...
describe("P18 typed Capability SDK contract", () => {
  ...
  it("应把旧边界错误映射为稳定 SDK error code", () => {
    const readinessError = new PluginCapabilityError({
        ...
        "lime.agent": {
          startTask: (request) => {
            ...
            return buildTaskRecord({
              title:
                typeof args?.title === "string" ? args.title : "Plugin task",
            });
          },
        },
      }),

...
class FakeBridgeWindow {
  readonly parent = {
    postMessage: vi.fn(
      (message: LimePluginBridgeClientMessage, targetOrigin: string) => {
  ...
  readonly posts: Array<{
    message: LimePluginBridgeClientMessage;
    targetOrigin: string;
  }> = [];
  ...
  emit(
    data: LimePluginBridgeClientMessage,
...
function hostResponse(
  request: LimePluginBridgeClientMessage,
  payload: unknown,
...
function latestBridgeRequest(fake: FakeBridgeWindow): LimePluginBridgeClientMessage {
  ...
  it("应支持 app ready、snapshot、theme、visibility 和 Host action", async () => {
    ...
    fake.emit(
      hostResponse(selectDirectoryRequest, {
        ok: true,
        value: { path: "/Users/example/plugin", cancelled: false },
      }),
    );

...
function supportsManifestRuntime(manifest: NormalizedAppManifest): boolean {
  return (
    ...
    compatiblePluginStandardVersions.includes(manifest.manifestVersion)
  );
}
  ...
  if (registrationBlocked) {
    blockers.push({
      ...
      message: cloud.registrationHint
        ? `Cloud Plugin requires registration: ${cloud.registrationHint}.`
        : "Cloud Plugin requires registration before install or launch.",
    });

...
import type { PluginSetupState, HostCapabilityProfile } from "../types";
  ...
  it("应接受 v0.11 manifest 和 install runtime 作为当前 Host 标准", () => {
    const rawManifest = parseManifest({
      ...
      install: {
        ...
        runtime: { minVersion: currentPluginHostRuntimeVersion },
        standalone: { shell: "lime-app-shell" },
        runtimeBacked: {
          requires: "lime-runtime",
          minVersion: currentPluginHostRuntimeVersion,
        },
      },
    });
    const manifest = normalizeManifest(rawManifest);

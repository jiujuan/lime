...
export type PluginHostFunctionKey =
  | "appCenterPublishing"
  | "packageInspection"
  | "installReview"
  | "readinessGate"
...
export type PluginHostFunctionStatus =
  | "ready"
  | "needs-setup"
  | "blocked"
  | "delisted"
...
export type PluginRightSurfaceTabKind =
  | "articleWorkspace"
  | "file"
  | "evidence"
  | "terminal"
...
export type PluginArticleWorkspacePaneKind =
  | "artifact"
  | "inspector"
  | "runtime"
  | "evidence"
...
export interface PluginHostFunctionState {
  key: PluginHostFunctionKey;
  status: PluginHostFunctionStatus;
  currentOwner: "app-server" | "desktop-host" | "claw" | "plugin-host";
  blockers: string[];
  followUps: string[];
}
...
export interface PluginArticleWorkspaceObject {
  kind: string;
  title: string;
  defaultPane: PluginArticleWorkspacePaneKind;
  artifactKind?: string;
  primary: boolean;
}
...
export interface PluginRightSurfaceContract {
  dock: "right";
  physicalDockCount: 1;
  defaultActiveTab: PluginRightSurfaceTabKind | null;
  supportedTabs: PluginRightSurfaceTabKind[];
  articleWorkspace: {
    enabled: boolean;
    objects: PluginArticleWorkspaceObject[];
    panes: PluginArticleWorkspacePaneKind[];
    rendererKinds: string[];
  };
  historyRestore: {
    enabled: boolean;
    defaultTab: PluginRightSurfaceTabKind | null;
    defaultPane: PluginArticleWorkspacePaneKind | null;
...
export interface PluginHostLifecycleIssueCategorySummary {
  category: string;
  count: number;
  codes: string[];
}
...
export interface PluginHostLifecycleSnapshot {
  ...
  appCenterStatus: PluginHostFunctionStatus;
  ...
  issueCategories?: PluginHostLifecycleIssueCategorySummary[];
  generatedAt: string;
}

export interface BuildPluginHostLifecycleSnapshotParams {
...
const DEFAULT_RIGHT_SURFACE_TABS: PluginRightSurfaceTabKind[] = [
  "articleWorkspace",
  "file",
  "evidence",
  "terminal",
...
): PluginArticleWorkspacePaneKind {
...
export function buildPluginTaskRuntimeContract(
  manifest: NormalizedAppManifest,
...
export function buildPluginRightSurfaceContract(
  manifest: NormalizedAppManifest,
...
function createFunctionState(params: {
  key: PluginHostFunctionKey;
  status: PluginHostFunctionStatus;
  currentOwner: PluginHostFunctionState["currentOwner"];
  blockers?: string[];
  followUps?: string[];
...
export function buildPluginHostLifecycleSnapshot(
  params: BuildPluginHostLifecycleSnapshotParams,
): PluginHostLifecycleSnapshot {
  const { manifest, readiness, installedState } = params;
  const generatedAt = params.generatedAt ?? new Date().toISOString();
  const rightSurface = buildPluginRightSurfaceContract(manifest);
  const taskRuntime = buildPluginTaskRuntimeContract(manifest);
  ...
  const functions: PluginHostFunctionState[] = [
    createFunctionState({
      key: "appCenterPublishing",
      status: appCenterStatus,
      currentOwner: "plugin-host",
      blockers:

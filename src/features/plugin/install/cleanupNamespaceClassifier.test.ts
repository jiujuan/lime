...
import {
  ...
  listPluginCleanupNamespaceGroups,
...
describe("Plugin P17.3 cleanup namespace classifier", () => {
  it("应把 cleanup plan 归类为 package / setup / storage / artifact / export namespace", () => {
    const { preview } = buildInstalledStateFixture();

    const groups = listPluginCleanupNamespaceGroups(preview.cleanupPlan);
    ...
    expect(preview.cleanupPlan.exportPaths[0]?.value).toBe(
      "<LimeAppData>/plugins/exports/content-factory-app",
    );
  });
  ...
  it("keep-data 应保留 appData namespace，delete-data 应删除同一组 appData namespace", () => {
    ...
    const keepData = classifyPluginCleanupNamespaceTargets({
      state,
      cleanupPlan: preview.cleanupPlan,
      strategy: "keep-data",
    });
    const deleteData = classifyPluginCleanupNamespaceTargets({
      state,
      cleanupPlan: preview.cleanupPlan,

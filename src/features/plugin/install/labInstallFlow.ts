...
import {
  ...
  type PluginRuntimePackageLoadResult,
...
export type PluginLabInstallFlowStage =
  | "source-selected"
  | "package-reviewed"
  | "package-verified"
  | "installed"
...
export type PluginLabInstallFlowStatus =
...
function cleanupTargetCount(plan: AppCleanupPlan): number {
  return listPluginCleanupNamespaceGroups(plan).reduce(
    (count, group) => count + group.targets.length,
    0,
  );
}

function uninstallBranchPreview(plan: AppCleanupPlan): PluginLabInstallFlowResult["uninstallPreview"] {
  const groups = listPluginCleanupNamespaceGroups(plan);
...
export function buildPluginLabResolvedSetupState(
  projection: PluginProjection,
): PluginSetupState {
  const setup: PluginSetupState = {
    knowledgeBindings: {},
    skills: {},
    tools: {},
    artifactTypes: {},
...
export function evaluatePluginLabInstallFlow(
  params: EvaluatePluginLabInstallFlowParams,
  ...
  const completedStages: PluginLabInstallFlowStage[] = [
    "source-selected",
    "package-reviewed",
  ];
  const packageCache =
    params.packageCache ?? new InMemoryPluginPackageCacheRepository();
  const installedStore =
    params.installedStore ?? new InMemoryInstalledPluginStateStore();

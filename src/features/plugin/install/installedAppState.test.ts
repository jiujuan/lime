...
describe("InstalledPluginState P10", () => {
  ...
  it("store 应支持 upsert、get、list、disable 和 remove", () => {
    const { preview, setup } = buildPreviewWithSetup();
    const store = new InMemoryInstalledPluginStateStore();
    const state = buildInstalledPluginState({ preview, setup, installedAt: now, updatedAt: now });
  ...
  it("cleanup plan 应包含 installed state snapshot 路径", () => {
    ...
    expect(preview.cleanupPlan.installedStatePaths[0]?.value).toBe(
      "<LimeAppData>/plugins/installed/content-factory-app.json",
    );
  });
});

describe("InstalledPluginState P11 local persistence", () => {
  it("应把 installed state 与 setup state 写入本地 persistence driver 并可恢复", async () => {
    ...
    expect(snapshot["<LimeAppData>/plugins/installed/content-factory-app.json"]).toContain(
      '"schemaVersion": 1',
    );
    expect(snapshot["<LimeAppData>/plugins/setup/content-factory-app.json"]).toContain(
      '"project_knowledge": true',
    );
  ...
  it("应提供浏览器本地存储 driver 作为 Lab persistence 适配层", async () => {
    ...
    const { preview, setup } = buildPreviewWithSetup();
    const state = buildInstalledPluginState({ preview, setup, installedAt: now, updatedAt: now });
    const repository = new LocalInstalledPluginStateRepository({
      driver: new BrowserLocalStoragePluginPersistenceDriver({
        keyPrefix: "test.plugin.persistence:",
      }),
    });
    ...
    expect(
      window.localStorage.getItem(
        "test.plugin.persistence:<LimeAppData>/plugins/installed/content-factory-app.json",
      ),
  ...
  it("应隔离坏文件并返回 load issue，而不是恢复为可运行状态", async () => {
    ...
    await driver.writeText("<LimeAppData>/plugins/installed/broken.json", "{bad-json");

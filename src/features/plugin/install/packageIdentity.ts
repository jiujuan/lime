...
export function stableStringifyPluginValue(value: unknown): string {
  ...
  if (value && typeof value === "object") {
    return `{${Object.entries(value as Record<string, unknown>)
      ...
      .map(([key, entry]) => `${JSON.stringify(key)}:${stableStringifyPluginValue(entry)}`)
      .join(",")}}`;
  }
...
export function buildPackageIdentity(params: {
  ...
  const manifestHash = buildPluginManifestHash(params.manifest);
...
export function buildPluginManifestHash(manifest: unknown): string {
  return `manifest-fnv1a-${fnv1a(stableStringifyPluginValue(manifest))}`;
}

export function buildPluginPackageHash(params: {
  manifest: unknown;
  sourceUri?: string;
}): string {

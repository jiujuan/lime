...
import {
  ...
  type PluginDeleteDataExecutionGate,
  type PluginLifecycleUninstallRehearsalDescriptor,
} from "./lifecycleAction";
import {
  executePluginDeleteData,
  type PluginDeleteDataFileSystemPort,
} from "./deleteDataExecutor";

const now = "2026-05-15T00:00:00.000Z";
const pluginDataRoot = "<LimeAppData>/plugins";

function buildDeleteDataDescriptor(): PluginLifecycleUninstallRehearsalDescriptor {
  ...
  return buildPluginLifecycleUninstallRehearsalDescriptor({
    state,
...
function buildAllowedGate(
  ...
  const gate = buildPluginDeleteDataExecutionGate({
    descriptor,
    confirmationPhrase: buildPluginDeleteDataConfirmationPhrase(descriptor),
    generatedAt: now,
  });
...
function buildMemoryFileSystem(options: {
  ...
  const port: PluginDeleteDataFileSystemPort = {
...
describe("Plugin delete-data executor", () => {
  it("应只删除 path / namespace 目标，并把 ref 作为 evidence 保留", async () => {
    ...
    const result = await executePluginDeleteData({
      descriptor,
      gate,
  ...
  it("应拒绝路径穿越和 data root 外目标", async () => {
        ...
        pendingDeletionTargets: [
          {
            ...
            kind: "path",
            value:
              "<LimeAppData>/plugins/storage/content-factory-app/../other-app",
    ...
    } satisfies PluginLifecycleUninstallRehearsalDescriptor;
    ...
    expect(result.blockers).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({ code: "TARGET_OUTSIDE_PLUGIN_DATA_ROOT" }),
      ]),
    );
  ...
  it("descriptor 与 gate 不匹配时应阻断执行", async () => {
    const descriptor = buildDeleteDataDescriptor();
    const mismatchedGate: PluginDeleteDataExecutionGate = {
      allowed: true,
      appId: "other-app",

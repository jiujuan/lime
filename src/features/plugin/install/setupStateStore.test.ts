...
describe("Plugin setup state store P9", () => {
  it("应把 binding records 聚合为 PluginSetupState", () => {
  ...
  it("store 应支持 upsert、查询、删除和按 app 清理", () => {
    const store = new InMemoryPluginSetupStateStore();
  ...
  it("setup state 接入 preview 后应消除 needs-setup 状态", () => {
    ...
    expect(preview.cleanupPlan.setupStatePaths[0]?.value).toBe(
      "<LimeAppData>/plugins/setup/content-factory-app.json",
    );
  });
});

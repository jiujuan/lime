...
import type { PluginCloudReleaseSignatureVerificationStatus } from "./cloudReleaseEvidence";

export interface VerifyCloudReleaseSignatureParams {
  app: CloudBootstrapApp;
  trustRoots?: PluginCloudReleaseSignatureTrustRoot[];
...
export async function verifyCloudReleaseSignature({
  ...
  trustRoots = [],
  crypto: cryptoApi,
}: VerifyCloudReleaseSignatureParams): Promise<PluginCloudReleaseSignatureVerificationStatus> {
  const proof = app.signatureProof;
  if (!proof) {
...
export type { PluginCloudReleaseSignatureProof };

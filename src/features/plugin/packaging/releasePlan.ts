...
export type PluginReleaseChannel = "dev" | "internal" | "stable";

export interface PluginStandaloneReleaseInput {
...
export interface PluginStandaloneReleaseGate {
  code:
...
export interface PluginStandaloneReleasePlan {
  schemaVersion: 1;
  appId: string;
  channel: PluginReleaseChannel;
  target: PluginPackageDescriptor["target"];
  ...
  gates: PluginStandaloneReleaseGate[];
  blockers: PluginStandaloneReleaseGate[];
  warnings: PluginStandaloneReleaseGate[];
}

function gate(
  code: PluginStandaloneReleaseGate["code"],
  message: string,
  severity: PluginStandaloneReleaseGate["severity"] = "blocker",
  details?: unknown,
): PluginStandaloneReleaseGate {
  return { code, message, severity, details };
}

function hasUpdaterConfig(input: PluginStandaloneReleaseInput): boolean {
  if (!input.updater?.enabled) return false;
  return Boolean(input.updater.endpoint?.trim());
}

function hasRollbackPlan(input: PluginStandaloneReleaseInput): boolean {
...
export function buildStandaloneReleasePlan(
  input: PluginStandaloneReleaseInput,
): PluginStandaloneReleasePlan {
  const descriptor = input.descriptor;
  const target = descriptor.target;
  const gates: PluginStandaloneReleaseGate[] = [];

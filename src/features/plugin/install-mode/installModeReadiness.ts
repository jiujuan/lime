...
function shellKindForMode(mode: PluginInstallMode): string {
  if (mode === "standalone") {
...
function unsupportedWebHostIssue(mode: PluginInstallMode): ReadinessIssue {
  return {
    code: "INSTALL_MODE_UNSUPPORTED",
    severity: "blocker",
    message: "web_host is reserved in Plugin v2 and is not launchable in the Lime desktop client yet.",
    kind: "install-mode",
    key: mode,
    required: true,

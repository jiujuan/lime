...
export interface AgentUiTeamWorkbenchItemTarget {
  sessionId?: string;
  threadId?: string;
  runId?: string;
  turnId?: string;
...
export interface AgentUiTeamWorkbenchViewItem {
  id: string;
  ...
  chips: string[];
  attention: boolean;
  action: AgentUiTeamWorkbenchItemAction | null;
  target: AgentUiTeamWorkbenchItemTarget;
}

export interface AgentUiTeamWorkbenchViewSection {
  surface: AgentUiSurface;
  label: string;
  description: string;
  total: number;
  attentionCount: number;
  latestItems: AgentUiTeamWorkbenchViewItem[];
  primaryItem: AgentUiTeamWorkbenchViewItem | null;
}

export interface AgentUiTeamWorkbenchViewModel {
  total: number;
  attentionCount: number;
  sections: AgentUiTeamWorkbenchViewSection[];
}
...
function isTeamWorkbenchEvent(event: AgentUiProjectionEvent): boolean {
  return Boolean(
...
function buildTarget(
  event: AgentUiProjectionEvent,
): AgentUiTeamWorkbenchItemTarget {
...
export function buildAgentUiTeamWorkbenchViewModel(
  ...
  const sections = AGENT_UI_TEAM_WORKBENCH_SURFACE_DEFINITIONS.map(
    (definition) => {
      ...
      } satisfies AgentUiTeamWorkbenchViewSection;
    },

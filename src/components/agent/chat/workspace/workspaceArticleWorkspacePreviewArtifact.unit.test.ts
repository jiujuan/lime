...
import { buildWorkspaceArticleWorkspaceViewModel } from "./workspaceArticleWorkspaceModel";
...
describe("workspaceArticleWorkspacePreviewArtifact", () => {
  it("应把文档对象投影为 source-backed markdown preview artifact", () => {
    const viewModel = buildWorkspaceArticleWorkspaceViewModel(articleWorkspace);
  ...
  it("应把图片对象投影为 media preview artifact", () => {
    const imageProfile: WorkspaceArticleWorkspace = {
      ...articleWorkspace,
      selectedObjectRef: {
        appId: "content-factory-app",
        kind: "imageGenerationSet",
    ...
    const viewModel = buildWorkspaceArticleWorkspaceViewModel(imageProfile);
  ...
  it("图片对象没有远程 URL 时应优先使用本地缓存文件打开预览", () => {
    ...
    expect(artifact).toMatchObject({
      ...
      meta: expect.objectContaining({
        ...
        articleWorkspaceImage: expect.objectContaining({
          id: "artifact-image-local",
          localPath: "/tmp/lime-content-factory/local-image.png",
          sourcePath: "/tmp/lime-content-factory/local-image.png",
        }),
  ...
  it("应把视频分镜对象投影为 storyboard preview artifact", () => {
    const storyboardProfile: WorkspaceArticleWorkspace = {
      ...articleWorkspace,
      selectedObjectRef: {
        appId: "content-factory-app",
    ...
    const viewModel =
      buildWorkspaceArticleWorkspaceViewModel(storyboardProfile);
    ...
    const artifact = buildWorkspaceArticleWorkspacePreviewArtifact({
      ...
      object: viewModel.selectedObject,
      preview: viewModel.selectedPreview,
      articleWorkspace: storyboardProfile,
      now: 100,
    });
  ...
  it("应把交付检查清单投影为 checklist preview artifact", () => {
    const checklistProfile: WorkspaceArticleWorkspace = {
      ...articleWorkspace,
      selectedObjectRef: {
        appId: "content-factory-app",
    ...
    const viewModel = buildWorkspaceArticleWorkspaceViewModel(checklistProfile);
    ...
    const artifact = buildWorkspaceArticleWorkspacePreviewArtifact({
      ...
      object: viewModel.selectedObject,
      preview: viewModel.selectedPreview,
      articleWorkspace: checklistProfile,
      now: 100,
    });

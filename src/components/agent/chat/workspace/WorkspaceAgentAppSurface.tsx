...
interface WorkspaceAgentAppSurfaceProps {
  activeContainerId?: string | null;
  onCloseSurface?: (surface: WorkspaceAgentAppSurfaceDescriptor) => void;
  onSelectSurface?: (surface: WorkspaceAgentAppSurfaceDescriptor) => void;
  surface?: WorkspaceAgentAppSurfaceDescriptor | null;
  surfaces?: readonly WorkspaceAgentAppSurfaceDescriptor[];
}

type AgentAppSurfaceTranslation = (
  key: string,
  options?: Record<string, unknown>,
) => string;

export function WorkspaceAgentAppSurface({
  activeContainerId,
  onCloseSurface,
  onSelectSurface,
  surface,
  surfaces,
}: WorkspaceAgentAppSurfaceProps): ReactElement {
  const { t } = useTranslation("agent");
  const dynamicT = t as AgentAppSurfaceTranslation;
  const surfaceList = useMemo(
    () =>
      normalizeWorkspaceAgentAppSurfaces(
        surfaces ?? (surface ? [surface] : []),
      ),
    [surface, surfaces],
  );
  const activeSurface = selectWorkspaceAgentAppSurfaceDescriptor(
    surfaceList,
    activeContainerId,
  );
  ...
  if (!activeSurface) {
    return (
      <section
        ...
        data-testid="workspace-agent-app-surface"
      />
    );
  }
      ...
      {surfaceList.length > 1 ? (
        ...
        >
          {surfaceList.map((item) => {
            ...
            const closeLabel = `${dynamicT("agentChat.agentAppSurface.closeTab")} ${item.title}`;
            return (
              <div
              ...
              >
                <button
                  ...
                  data-testid={`workspace-agent-app-surface-tab-${item.containerId}`}
                  role="tab"
                ...
                <button
                  ...
                  title={closeLabel}
                  data-testid={`workspace-agent-app-surface-close-${item.containerId}`}
...
function WorkspaceAgentAppSurfaceFrame({
  active,
  surface,
}: {
  active: boolean;
  ...
  const viewId = useMemo(
    () => `agent-app-surface-${sanitizeViewId(surface.containerId)}`,
    [surface.containerId],
  );
    ...
    >
      <header className="flex h-11 shrink-0 items-center gap-2 border-b border-[color:var(--lime-surface-border)] px-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="truncate text-xs text-[color:var(--lime-text-muted)]">
            {state?.isLoading
              ? dynamicT("agentChat.agentAppSurface.loading")
              : dynamicT("agentChat.agentAppSurface.ready")}
          </div>
        </div>
      </header>
      <div className="relative min-h-0 flex-1 bg-white">
        ...
        {!hostAvailable ? (
          <SurfaceOverlay
            tone="warning"
            title={dynamicT("agentChat.agentAppSurface.hostUnavailableTitle")}
            body={dynamicT("agentChat.agentAppSurface.hostUnavailableBody")}
          />
        ) : !state && !errorText ? (
          <SurfaceOverlay
            title={dynamicT("agentChat.agentAppSurface.loadingTitle")}
            body={dynamicT("agentChat.agentAppSurface.loadingBody")}
          />
        ) : null}
        {errorText ? (
          <SurfaceOverlay
            tone="error"
            title={dynamicT("agentChat.agentAppSurface.loadFailedTitle")}
            body={errorText}
          />
        ) : null}
      </div>
...
function normalizeWorkspaceAgentAppSurfaces(
  surfaces: readonly WorkspaceAgentAppSurfaceDescriptor[],

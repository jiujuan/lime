...
export interface WorkspacePluginIntentSource {
  appId: string;
  appName?: string;
...
export interface WorkspacePluginIntentMatch {
  appId: string;
  appName: string;
  manifest: NormalizedAppManifest;
  intentKey: string;
  ...
  source: "plugin_manifest_intent" | "plugin_manifest_default";
}

interface AgentRuntimeIntentDeclaration {
  key: string;
  source: WorkspacePluginIntentMatch["source"];
  mode?: string;
  taskKind?: string;
  workflowKey?: string;
  outputArtifactKind?: string;
...
function buildFallbackIntentDeclaration(
  ...
  return {
    key: "default",
    source: "plugin_manifest_default",
...
export function resolveWorkspacePluginIntent(
  sourceText: string,
  ...
  let bestMatch: (WorkspacePluginIntentMatch & { score: number }) | null =
    null;
...
export function buildPluginIntentRequestMetadata(
  match: WorkspacePluginIntentMatch,
): Record<string, unknown> {
  const pluginIntent: Record<string, unknown> = {
    source: match.source,
    app_id: match.appId,
  ...
  if (match.taskKind) {
    pluginIntent.task_kind = match.taskKind;
  }
  if (match.workflowKey) {
    pluginIntent.workflow_key = match.workflowKey;
  }
  if (match.outputArtifactKind) {
    pluginIntent.output_artifact_kind = match.outputArtifactKind;
  }
  if (match.rightSurface) {
    pluginIntent.right_surface = match.rightSurface;
  }

  return {
    plugin_intent: pluginIntent,
    ...(match.rightSurface
      ? {
          right_surface: {
...
export function buildPluginIntentSystemPrompt(
  match: WorkspacePluginIntentMatch,
): string {
  ...
  return [
    ...
    `Plugin: ${match.appName} (${match.appId})`,
    ...
    "必须按该 Plugin intent 执行业务，不要调用 skill_search、SkillTool 或其他 Skill 搜索/执行链路来替代这个 App。",

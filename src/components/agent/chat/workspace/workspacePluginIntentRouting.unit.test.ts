...
import {
  buildPluginIntentRequestMetadata,
...
describe("workspacePluginIntentRouting", () => {
  it("应从已安装 Plugin manifest intents 匹配自然语言触发", () => {
    const match = resolveWorkspacePluginIntent(
      "帮我用创作工作台生成一篇文章和配图",
      [createInstalledApp()],
    );
  ...
  it("内容工厂写文章应优先命中文章任务 intent，而不是泛化生成任务", () => {
    const match = resolveWorkspacePluginIntent("用内容工厂写一篇公众号文章", [
      {
  ...
  it("内容工厂 intent 应来自调用方提供的 manifest source", () => {
    const match = resolveWorkspacePluginIntent("@内容工厂 写一篇公众号文章", [
      {
        appId: contentFactoryManifest.appId,
        appName: contentFactoryManifest.displayName,
        manifest: contentFactoryManifest,
  ...
  it("应构造通用 harness metadata 和系统提示，阻断 Skill 搜索替代 App", () => {
    const match = resolveWorkspacePluginIntent("creator workbench plan", [
      createInstalledApp(),
    ]);
    expect(match).not.toBeNull();

    const metadata = buildPluginIntentRequestMetadata(match!);
    ...
    expect(buildPluginIntentSystemPrompt(match!)).toContain(
      "本轮请求已命中 Plugin manifest intent。",
    );
    expect(buildPluginIntentSystemPrompt(match!)).not.toContain("已安装");
    expect(buildPluginIntentSystemPrompt(match!)).toContain("skill_search");
  });

  it("未声明 rightSurface 时不应写入空的 right_surface metadata", () => {
    const metadata = buildPluginIntentRequestMetadata({
      appId: "minimal-app",
      appName: "Minimal App",
      manifest: {

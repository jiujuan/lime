...
import { buildWorkspacePluginProductProfileFromActivation } from "./workspacePluginProductProfile";

describe("workspacePluginProductProfile", () => {
  it("无显式激活时应返回 null", () => {
    expect(
  ...
  it("contract 缺失或 product workspace 未启用时应 fail closed", () => {
  ...
  it("应从 selected object 构造最小 Product Profile", () => {
    ...
    const profile = buildWorkspacePluginProductProfileFromActivation({
      activationContext: {
        sessionId: "session-main",
        ...
        openedTabs: ["productProfile", "files"],
        source: "user",
      },
      contracts: [plugin],

...
describe("workspaceProductProfileWorkerEvidence", () => {
  it("应读取 worker 深度详情并按更新时间倒序排列", () => {
    const evidence = readWorkspaceProductProfileWorkerEvidence([
      {
  ...
  it("没有显式 worker evidence 时应保留旧 source artifact fallback", () => {
    const evidence = buildWorkspaceProductProfileWorkerEvidenceFromThreadRead({
      sourceArtifacts: [
        {
          artifactRef: "artifact-workspace-patch-1",

...
function contentPartMetadataFromThreadToolItem(
  item: AgentThreadItem,
  toolCall: HistoryToolCall,
): Record<string, unknown> | undefined {
  const metadata =
...
function sortProcessContentParts(parts: ContentPart[]): ContentPart[] {
  return parts
    .map((part, index) => ({
      part,
      index,
...
const appendUniqueMessageImage = (
  images: MessageImage[],
  image: MessageImage,
) => {
...
const appendHistoryMessageAttachments = (
  images: MessageImage[],
  message: unknown,
) => {

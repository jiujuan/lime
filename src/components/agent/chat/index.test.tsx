...
describe("AgentChatPage 任务中心初始会话标签", () => {
  ...
  it("自动首条技能提交期间不应恢复本地旧标签覆盖新提交流程", async () => {
    localStorage.setItem(
      TASK_CENTER_OPEN_TAB_IDS_STORAGE_KEY,
      JSON.stringify({
        "workspace-test": ["topic-old"],
      }),
    );
    const switchTopic = vi.fn(async () => "success");
    const prompt =
    ...
    expect(
      sharedSendMessageMock.mock.calls.some((call) => call[0] === prompt),
    ).toBe(true);
    expect(switchTopic).not.toHaveBeenCalled();
  });

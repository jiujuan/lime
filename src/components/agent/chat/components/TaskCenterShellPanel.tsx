...
import {
  TaskCenterShellTerminal,
  type TaskCenterShellTabState,
  type TaskCenterShellTerminalHandle,
} from "./TaskCenterShellTerminal";
...
interface TaskCenterShellTab {
  id: string;
  state: TaskCenterShellTabState;
}

function createDefaultShellTabState(index: number): TaskCenterShellTabState {
  return {
    errorText: null,
    ready: false,
    shell: null,
...
export function TaskCenterShellPanel({
  ...
  const terminalRefs = React.useRef<
    Record<string, TaskCenterShellTerminalHandle | null>
  >({});
  const [tabs, setTabs] = React.useState<TaskCenterShellTab[]>(() => [
    {
      id: "shell-tab-1",
  ...
  const updateTabState = React.useCallback(
    (tabId: string, state: Partial<TaskCenterShellTabState>) => {
      setTabs((currentTabs) =>
        currentTabs.map((tab) =>
          tab.id === tabId
            ? {
  ...
  const startNewSession = React.useCallback(() => {
    ...
    const nextTab: TaskCenterShellTab = {
      ...
      <div className="flex h-full flex-col">
        ...
        <div className="flex h-9 shrink-0 items-center border-b border-slate-200 bg-white">
          <div
            ...
            data-testid="task-center-shell-tabs"
          >
            {tabs.map((tab) => {
              ...
              return (
                <div
                  ...
                  data-testid="task-center-shell-tab"
                >
                  <button
                    type="button"
                    ...
                    data-testid={`task-center-shell-tab-button-${tab.id}`}
                  >
                  ...
                  <button
                    type="button"
                    className="inline-flex h-5 w-5 shrink-0 items-center justify-center rounded text-slate-400 transition hover:bg-slate-200 hover:text-slate-700"
                    aria-label={agentText(
                    ...
                    data-testid={`task-center-shell-tab-close-${tab.id}`}
                  >
                    <X className="h-3 w-3" />
                  </button>
                </div>
          ...
          <ShellActionButton
            testId="task-center-shell-list-files"
          ...
          <ShellActionButton
            testId="task-center-shell-view-file"
          ...
          <ShellActionButton
            testId="task-center-shell-git-status"
            label={agentText(
              "agentChat.navbar.shell.gitStatus",
              "查看 Git 状态",
            )}
            disabled={!activeTab?.state.ready}
          ...
          <ShellActionButton
            testId="task-center-shell-clear"
          ...
          <span
            className="mr-2 hidden max-w-[180px] truncate text-[11px] text-slate-400 sm:inline"
            data-testid="task-center-shell-status"
          >
        ...
        <div
          className="min-h-0 flex-1 overflow-hidden bg-white px-3 py-2"
          onClick={handleTerminalClick}
          onDoubleClick={handleResize}
          data-testid="task-center-shell-output"
        >

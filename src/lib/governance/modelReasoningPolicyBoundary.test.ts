...
const MODEL_REASONING_POLICY_FIELDS = [
  "supports_reasoning_summaries",
  "default_reasoning_level",
  "supported_reasoning_levels",
...
describe("Model reasoning policy boundary", () => {
  it("只暴露 Codex 式 reasoning policy 字段", () => {
    const source = readRepoFile(REASONING_POLICY_SOURCE);
    const inputBlock = requireMatch(
      source,
      /export interface ModelReasoningPolicyInput \{\n(?<body>[\s\S]*?)\n\}/u,
      "ModelReasoningPolicyInput",
    );
    ...
    expect(extractTypeFieldNames(inputBlock)).toEqual(
      MODEL_REASONING_POLICY_INPUT_FIELDS,
    );
    expect(extractTypeFieldNames(policyBlock)).toEqual(
      MODEL_REASONING_POLICY_FIELDS,
    );
  });

  it("不从 capability summary、runtime features 或 picker/catalog 字段推断 reasoning policy", () => {
    const source = readRepoFile(REASONING_POLICY_SOURCE);

    for (const field of FORBIDDEN_INFERENCE_FIELDS) {
      expect(
        source,
        `modelReasoningPolicy 不应读取 ${field} 推断 reasoning effort`,
  ...
  it("保持纯 owner，不依赖 registry、旧 Lime 窄枚举、bridge 或 UI", () => {
    ...
    for (const token of FORBIDDEN_DEPENDENCY_TOKENS) {
      expect(
        source,
        `modelReasoningPolicy 不应依赖 ${token}`,
      ).not.toContain(token);
    }
  });
});

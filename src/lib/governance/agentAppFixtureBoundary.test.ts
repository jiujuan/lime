...
function isAllowedTestOnlyPath(path: string): boolean {
  ...
  return (
    normalized.includes("/src/features/agent-app/testing/") ||
...
describe("agentAppFixtureBoundary", () => {
  it("生产源码不得恢复 Agent App fixture 目录或导入 test-only fixture", () => {
    const violations = listSourceFiles(SRC_ROOT)
      ...
      .flatMap((path) => {
        ...
        return [
          source.includes("features/agent-app/fixtures")
            ? `${relativePath}: legacy fixture path`
            : null,
          source.includes("agent-app/testing/fixtures") ||

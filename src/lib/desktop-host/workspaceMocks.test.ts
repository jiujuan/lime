...
describe("workspaceMocks", () => {
  ...
  it("保留仍被显式测试夹具依赖的 workspace mock", () => {
    expect(workspaceMocks).toHaveProperty("get_or_create_default_project");
    expect(workspaceMocks).toHaveProperty("workspace_get_projects_root");
  });
});

...
describe("agentAppRuntime api", () => {
  ...
  it("runtime task facade 遇到非 runtime 响应形状时应 fail closed", async () => {
    ...
    await expect(
      ...
      "agent_app_runtime_submit_host_response returned an error envelope",
    );
  });
  ...
  it("runtime task facade 遇到 error envelope 时应 fail closed", async () => {
    ...
    await expect(
      ...
      "agent_app_runtime_start_task returned an error envelope",
    );
    ...
    await expect(
      ...
      "agent_app_runtime_cancel_task returned an error envelope",
    );
    ...
    ).rejects.toThrow("agent_app_runtime_get_task returned an error envelope");

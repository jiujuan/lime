...
9. 组织 harness 的结果只能回到 `internal/roadmap/`、`internal/exec-plans/`、telemetry、artifact 和 evidence 主链。
...
后续 `internal/roadmap/skill-forge/` 必须做到：
...
6. 把 Codex `/goal` 参考单独留在 `internal/research/codex-goal/`；Skill Forge roadmap 只引用它来解释长期目标推进，不把它写成 Skill Forge 的一部分。

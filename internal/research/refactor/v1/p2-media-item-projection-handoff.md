...
下一刀应补 progressive media renderer、其它非 sidecar/sourcePath owner 的完整读取链和非 media 长 IO / 子进程级 transport kill；不要在 `src/components/agent/chat/**` 继续补 provider wire 媒体识别，也不要在 Aster vendor 或 provider wire 里加 UI 字段。

...
本轮已补 `refId` / sidecar `sourceUri` / relativePath-only `sidecarRef` 的 read owner normalization：GUI hydrate 会把 App Server contentParts 的 `ref_id/refId` 透传到 `MessageMediaReference.refId`，Workbench preview read params 可用非 sidecar 展示 URI 上的 `refId`、`sidecar://` `sourceUri` 或 relativePath-only `sidecarRef` 进入同一 `agentSession/media/read` current 链；direct URI / `previewUrl` / `sourcePath` owner 仍不重复读取，inline `data:` 仍 fail closed。

本轮继续补 artifact/cache owner read skeleton：`session_media_refs.rs` 统一收集 / 匹配 `agentSession/media/read` known refs，artifact/cache metadata 中带 `sidecarRef` 且 context MIME / kind 明确为 image/audio/video 时，可通过 `artifact://...`、`artifactRef/artifactId`、`cacheRef/contentRef/outputRef` 或 sidecar ref alias 进入同一 digest/range/stream/cancel 读取链；普通 text/markdown artifact 不会被误当 media。

下一刀应补图片生成 / 插件 host-managed generation 的真实 GUI / Playwright 自然度验收，或回到 P2 Plugin / Skills / MCP 的 Playwright 自然度验收与有副作用自动触发策略；不要在 `src/components/agent/chat/**` 继续补 provider wire 媒体识别，也不要在 Aster vendor 或 provider wire 里加 UI 字段。

2026-07-08 MCP 补充：`npm run smoke:mcp-workspace-plugin-runtime-electron-fixture` 已补真实 Electron Workspace 点击骨架，覆盖 page-level `harness-status-panel` 点击“准备 MCP”后经 preload `app_server_handle_json_lines` 执行 `agentSession/toolInventory/read`、candidate `mcpServer/start`、caller-scoped `mcpTool/listForContext`、显式 `mcpTool/callWithCaller`，并证明 default list proof 不自动调用工具。该证据仍是 Gate B skeleton，不替代 Playwright 自然度验收、插件安装/选择全链路或有副作用自动触发策略。

2026-07-08 MCP 生产 Harness 补充：`plugin_mcp_targets` 已进入前端 typed inventory contract 与 fail-closed shape guard，生产 `HarnessExtensionToolInventorySections` 已展示插件 MCP target 的 plugin/caller/server/tool、runtime/prepare 状态、prepare/call proof 与 default list proof，五语言文案已补齐；`inventoryClient` / `HarnessStatusPanel.tools` / `useWorkspaceHarnessInventoryRuntime` 定向 Vitest 27 tests passed。该补充只关闭生产 Harness target 可见性骨架，不替代 Playwright 自然度验收、插件安装/选择全链路或有副作用自动触发策略。
...
| Workbench preview target | `src/components/agent/chat/components/MessageAssistantBody.tsx`、`AgentChatWorkspace.tsx`、`workspace/mediaReferencePreviewArtifacts.ts`、`workspace/mediaReferencePreviewStreamingProgress.ts`、`workspace/mediaReferencePreviewLiveDrain.ts`、`workspace/useWorkspaceMediaReferencePreviewRuntime.ts`、`src/lib/api/appServerEventBus.ts`、`src/lib/api/fileSystem.ts`、`electron/main.ts` | `media_reference` 卡片可发出 `MessagePreviewTarget.kind = "media_reference"`；`mediaReferencePreviewArtifacts.ts` 会把 direct media URI、`previewUrl`、绝对 `sourcePath`、`sourceUri` 变成 media artifact，并负责 sidecar read params、range / chunked object URL assembly、progress artifact、`shouldContinue` 中断钩子与 metadata fallback；`mediaReferencePreviewStreamingProgress.ts` 负责 `media.read.chunk` progress notification 解析和 `sessionId / streamId / uri / offset / eventId` fail-closed 去重；`mediaReferencePreviewLiveDrain.ts` 通过 App Server EventBus `includeRecent` 在 request pending 期间消费 mirrored transient notification；`useWorkspaceMediaReferencePreviewRuntime.ts` 负责 App Server client lazy read、object URL registry、同一 preview artifact 替换时释放旧 URL、组件卸载清理、request token、迟到 sidecar read / live progress fail-closed、object URL 数量 / bytes 预算、progress artifact、`AbortSignal` 接线和 Canvas Workbench preview 接线；Electron `asset://` 只读协议已支撑 dev renderer 加载本地 sourcePath 图片，不展开 inline payload；App Server `agentSession/media/read` skeleton 已能按 session 已知 sidecar ref 读取 base64 bytes / range window 并校验 full-file digest，前端 AppServerClient 网关已接入；没有 direct/source owner 但可读的 `sidecar://` media reference 会按 bounded range window 分片读取并组装 object URL media preview artifact，首段有效 range 后先写 loading/progress markdown artifact，读取失败、partial range、不连续 offset、digest 漂移、超限、并发过期或组件卸载时 fail-closed 到 markdown metadata fallback / no-op；`$/cancelRequest` 会在 `agentSession/media/read` sidecar range read / digest loop 中 fail-closed；`agentSession/media/read stream=true` 可在 response 前发 `media.read.chunk` / `media.read.completed` transient notifications；前端 chunked preview 已消费同次 response `notifications` 和 live bridge/event-drain mirrored `media.read.chunk` 驱动 progress artifact；`runtime/session_media_refs.rs` 已支持 artifact/cache media sidecar owner alias；`AgentChatWorkspace.tsx` 只做 hook 接线；App Server `mediaTaskArtifact/image|audio/complete` 已能把当前 session 的 data/file/workspace-local/受控远程 URL 输出写成 media sidecar owner facts；图片 worker 直写 data URL 或 remote URL 输出已能补 sidecar owner facts；direct live shell process cancel/terminate 骨架已在 `lime-agent` tool orchestrator 落地 | 后续补 GUI / Playwright 自然度、业务协作入口和其它网络长 IO owner 证据 |
...
5. GUI hydrate / streaming sync / MessageList render（skeleton done / current Electron fixture skeleton validated）
   ...
   - Message preview target 已能打开 media reference artifact：direct media URI、`previewUrl`、绝对 `sourcePath`、`sourceUri` 走 `mediaReferencePreviewArtifacts.ts` media preview；没有 source owner 的 `sidecar://` 若可由当前 session `agentSession/media/read` 读取，则按 bounded range window 分片组装 object URL media preview artifact，否则走 metadata fallback；chunked sidecar read 首段有效 range 后先写 loading/progress artifact，完整 object URL 成功后用同一 artifact id 替换；current Electron fixture 已验证 `agent_message.contentParts.media -> GUI media card -> Canvas Workbench sourcePath image preview` 骨架；media task completion data/file/remote URL sidecar 写入骨架、worker data URL / remote URL 输出 sidecar 写入骨架、tool result carried owner facts synthetic projection 骨架、worker task store -> session read model enrich 骨架、range/object URL lifecycle、chunked read assembly 骨架、helper owner split、streaming progress parser owner split、preview runtime hook owner split、preview runtime 取消 / object URL 预算骨架、progress artifact 骨架、App Server client / renderer client AbortSignal wait-detach 骨架、large-media page-window / toolbar 骨架、server-side media read cancellation 骨架、media/read streaming transport 骨架、同次 response notification progress consumption 骨架、live bridge/event-drain progressive media renderer 骨架、artifact/cache media owner read 骨架和 direct live shell process cancel/terminate 骨架已补；其它网络长 IO / live Provider owner 证据后续补。
...
Media server-side cancellation skeleton 验证：

```bash
cargo test --manifest-path "lime-rs/Cargo.toml" -p app-server request_cancel -- --nocapture
cargo test --manifest-path "lime-rs/Cargo.toml" -p app-server read_bytes_range_verified_with_cancel -- --nocapture
cargo test --manifest-path "lime-rs/Cargo.toml" -p app-server media_read_requests_spawn_transport_task_for_cancellation -- --nocapture
cargo test --manifest-path "lime-rs/Cargo.toml" -p app-server reads_known_media_sidecar -- --nocapture
```

结果：request cancellation 2 tests passed；sidecar cancellable range read 1 test passed；transport task dispatch guard 1 test passed；既有 media sidecar read / range read 2 tests passed。该步声明 `$/cancelRequest` 已能在 `agentSession/media/read` sidecar IO 中 fail-closed 到 JSON-RPC `REQUEST_CANCELLED`，但不声明真正 streaming transport、progressive media renderer、其它非 sidecar/sourcePath owner 完整读取或非 media 长 IO / 子进程级 transport kill 完成。

Media read streaming transport skeleton 验证：
...
npm run test:contracts
rustfmt --edition 2021 --check "lime-rs/crates/app-server/src/runtime/session_media_reader.rs" "lime-rs/crates/app-server/src/runtime/sidecar_store.rs" "lime-rs/crates/app-server/src/processor/agent_session.rs" "lime-rs/crates/app-server/src/processor/dispatch.rs" "lime-rs/crates/app-server/src/lib.rs" "lime-rs/crates/app-server-protocol/src/protocol/v0/agent_session.rs" "lime-rs/crates/app-server/src/processor/tests/request_cancel.rs"
```

结果：`@limecloud/app-server-client` 59 tests passed；`check:protocol-types` 无漂移；App Server client contract 287 checks passed；`npm run test:contracts` 完整通过；scoped rustfmt 通过。独立 target Rust 定向 filters 已通过：`media_read_streaming_request_emits_chunk_notifications`、`streams_known_media_sidecar_chunks_before_completed_event`、`stream_bytes_range_verified_emits_chunks_and_keeps_full_digest`、`media_read_streaming_transport_requires_stream_flag`。该步声明 `agentSession/media/read stream=true` 已通过 App Server JSON-RPC current transport 在 response 前输出 transient `media.read.chunk` / `media.read.completed` notification；不声明前端 progressive media renderer、其它非 sidecar/sourcePath owner 完整读取或非 media 长 IO / 子进程级 transport kill 完成。

Media read response notification progress consumption 验证：

```bash
npx vitest run "src/components/agent/chat/workspace/mediaReferencePreviewArtifacts.unit.test.ts" "src/components/agent/chat/workspace/useWorkspaceMediaReferencePreviewRuntime.test.tsx"
npx eslint "src/components/agent/chat/workspace/mediaReferencePreviewArtifacts.ts" "src/components/agent/chat/workspace/mediaReferencePreviewArtifacts.unit.test.ts" "src/components/agent/chat/workspace/useWorkspaceMediaReferencePreviewRuntime.ts" "src/components/agent/chat/workspace/useWorkspaceMediaReferencePreviewRuntime.test.tsx" --max-warnings 0
./node_modules/.bin/tsc --noEmit --project "tsconfig.renderer.json" --pretty false
```

结果：Vitest 2 files / 25 tests passed；scoped ESLint 通过；renderer typecheck 通过。该步声明前端 media preview owner 已消费同次 App Server client response notifications 里的 `media.read.chunk` 并驱动 progress artifact；不声明 live bridge/event-drain progressive media renderer、其它非 sidecar/sourcePath owner 完整读取或非 media 长 IO / 子进程级 transport kill 完成。

Media read live bridge/event-drain progress consumption 验证：

```bash
npx vitest run "src/lib/api/appServerEventBus.unit.test.ts" "src/lib/api/appServer.test.ts" "src/components/agent/chat/workspace/mediaReferencePreviewArtifacts.unit.test.ts" "src/components/agent/chat/workspace/useWorkspaceMediaReferencePreviewRuntime.test.tsx"
npx eslint "src/lib/api/appServerClient.ts" "src/lib/api/appServerEventBus.ts" "src/lib/api/appServerEventBus.unit.test.ts" "src/lib/api/appServer.test.ts" "src/lib/api/agentRuntime/appServerEventStreamRouting.ts" "src/components/agent/chat/workspace/mediaReferencePreviewArtifacts.ts" "src/components/agent/chat/workspace/mediaReferencePreviewStreamingProgress.ts" "src/components/agent/chat/workspace/mediaReferencePreviewLiveDrain.ts" "src/components/agent/chat/workspace/useWorkspaceMediaReferencePreviewRuntime.ts" "src/components/agent/chat/workspace/mediaReferencePreviewArtifacts.unit.test.ts" "src/components/agent/chat/workspace/useWorkspaceMediaReferencePreviewRuntime.test.tsx" --max-warnings 0
./node_modules/.bin/tsc --noEmit --project "tsconfig.renderer.json" --pretty false
npm run test:contracts
npm run verify:gui-smoke
```

结果：Vitest 4 files / 61 tests passed；scoped ESLint 通过；renderer typecheck 通过；`test:contracts` 通过；`verify:gui-smoke` 通过，Electron smoke 输出 `renderer loaded`、`app-server initialized protocol=appserver.v0 version=1.95.0`、`claw workbench shell ready`、`memory settings ready`。该步声明 `AppServerClient.drainEvents({ includeRecent })`、`AppServerEventBus.includeRecent` 与 `workspace/mediaReferencePreviewLiveDrain.ts` 已复用 current App Server event-drain 机制，在 media read request pending 期间消费 mirrored `media.read.chunk` transient notification 并驱动同一 progress artifact；session / stream / uri / offset mismatch、duplicate eventId、superseded preview 和 unmount 均 fail closed。仍不声明其它非 sidecar/sourcePath owner 完整读取或非 media 长 IO / 子进程级 transport kill 完成。

Media read owner normalization 验证：

```bash
npm test -- --run "src/components/agent/chat/hooks/agentThreadMessageContentParts.unit.test.ts" "src/components/agent/chat/workspace/mediaReferencePreviewArtifacts.unit.test.ts"
npx eslint "src/components/agent/chat/types.ts" "src/components/agent/chat/hooks/agentThreadMessageContentParts.ts" "src/components/agent/chat/hooks/agentThreadMessageContentParts.unit.test.ts" "src/components/agent/chat/workspace/mediaReferencePreviewArtifacts.ts" "src/components/agent/chat/workspace/mediaReferencePreviewArtifacts.unit.test.ts"
npm run typecheck
```

结果：Vitest 2 files / 24 tests passed；scoped ESLint 通过；renderer + node typecheck 通过。该步声明 `MessageMediaReference.refId`、App Server contentParts `ref_id/refId` hydrate、非 sidecar 展示 URI + `refId`、sidecar `sourceUri`、relativePath-only `sidecarRef` 三种 read-param owner normalization 已接到 current `agentSession/media/read`；direct URI / `previewUrl` / `sourcePath` owner 仍不重复读取，inline `data:` 仍 fail closed。artifact/cache owner 读取与 direct live shell process cancel/terminate 骨架已由后续刀补齐；仍不声明其它网络长 IO、live Provider 或跨重启恢复证据完成。

Non-media process kill skeleton 验证：

```bash
cargo fmt --manifest-path "lime-rs/Cargo.toml" --all --check
cargo test --manifest-path "lime-rs/Cargo.toml" -p lime-agent live_shell_process_is_terminated_by_turn_cancel_token --lib -- --nocapture
cargo test --manifest-path "lime-rs/Cargo.toml" -p lime-agent tool_orchestrator --lib -- --nocapture
```

结果：fmt check 通过；单项 cancel test 1 passed；`tool_orchestrator` 定向集 16 tests passed。该步声明 `lime-agent` direct live shell process 已消费 turn `CancellationToken`，cancel 后调用 `LocalExecutionProcessHandle::terminate()`，metadata 写入 `executionProcessCancellation=turn_cancel`，final outcome fail closed，live process registry finish snapshot 为 `terminated`。该步不声明 live Provider、网络 IO、所有 App Server request、外部 backend 子进程跨重启恢复或 GUI Playwright 自然度验收完成。

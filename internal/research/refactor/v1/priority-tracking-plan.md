...
> 本轮补充：第三十八刀 request/tool/context consumer 主链、contract gate、runtime current fixture、P2 App Server lifecycle owner、`lime-agent` typed event construction owner、active state gate、approval pending transition、process/terminal correlation、approval resolved terminal transition、Evidence export correlation、action/tool identity split、RuntimeCore `action/respond` lifecycle guard、RuntimeState/Aster pending resume 证据、App Server RuntimeBackend bridge 证据与 Tool / Approval / Sandbox final combo evidence 已完成；bridge fixture 已接入 live `ExecutionProcessServer`，并通过正式 `ExecutionBackend::start_turn -> action.required -> RuntimeBackend::respond_action(ToolConfirmation) -> Aster pending future -> tool.result/message.delta/turn.completed` 回归。Context / Token fragment typed owner 已精修，App Server `context_packet` consumer 已消费 `ContextFragmentEnvelope` 并输出 `fragmentEnvelope` telemetry；Context Evidence export 已消费 turn runtime metadata / event payload 中的 `context_packet_telemetry`，输出不含 preview 正文和 sha256 的 context decision summary；session compaction 已把真实 `sidecarRef` 透传到 next-turn `ContextPacket` metadata，并把 `context.compaction.completed` materialize 成 read model `context_compaction` Item；Plugin / Skills / MCP 已新增 `runtimeCapabilities` snapshot owner并接入 projected plugin manifest，App Center plugin projection 已消费该 snapshot 生成 skill/tool/MCP runtime capability projection，详情页技能展示也已迁到 projection consumer，不再直接读旧 `manifest.skillRefs`；Realtime / Media / Collaboration 已新增 RuntimeCore `RuntimeContentPart` / media reference owner，按 MIME 归类 image/audio/video/file，非文本内容只能带 reference；第二十二刀已让支持 MIME 的 LLM image/audio 输出经 `llm_protocol` 映射为 `message.delta` + `contentPart/contentParts` typed payload；第二十三刀已新增 `RuntimeMessageDeltaContent`，统一 `text`、`contentPart`、`contentParts` 的 `message.delta` 内容 payload owner；第二十四刀已新增 owner-backed payload parser，解析 Item projection 后续要消费的 `message.delta` payload，并校验 `contentPart` / `contentParts` alias 一致；inline data URL、unsupported MIME 或 missing MIME 继续保持 `runtime.event` generic。P3 第五次 upstream range check 已完成：Codex 新增 configWarning owner 收敛与 safety buffering `retry_model` adopt-now 信号，conditional dotenv 记为 desktop-adapt/watch，同时回滚第四次 `interleaved response items`，opencode 无新增；本轮已新增 [p2-codex-fifth-signal-handoff.md](./p2-codex-fifth-signal-handoff.md)，把 App Server warning owner、provider `retry_model` parser 和 Desktop Host env overlay watch 拆成可接管施工单；当前 App Server `configWarning` protocol / processor emitter / warning list container / frontend typed consumer / GUI presentation 已完成，safety buffering parser / typed payload owner、provider stream typed event contract、runtime event envelope、OpenAI Responses / Aster compat SSE capture、App Server read model projection 与 GUI reliability diagnostics projection 已完成；下一次 upstream 从 Codex `8268cbfb0e5f39cb4efff928264fe8f29ddacafb` 与 opencode `eb6ff0c1e049e5dfb6f61eb74f925c0a8007490c` 起算。
> 本轮骨架补充（2026-07-07）：按“先快速完成骨架，然后回头完成细节”执行；`memory_prompt.rs` 已把 memory summary 写入 sidecarRef skeleton，且明确不保存 `memory_soul_prompt_context`、Style Pack 或 full system prompt；`thread_item_projection/agent_message.rs` 已消费 RuntimeCore `RuntimeMessageDeltaContent::from_payload(...)`，media-only `message.delta.contentPart/contentParts` 可进入 `agent_message.contentParts`，同 `itemId` text/media delta 可合并，alias mismatch 与 inline `data:` media fail closed。已跑 `rustfmt --check` 与 scoped `git diff --check`；App Server 定向 `cargo test` 已在 `/tmp/lime-codex-media-item-target` 通过，`media_only_delta_creates_agent_message_content_parts` 为 `1 passed`；protocol/generated 已导出并验证 `AgentThreadContentReference` / `AgentThreadMessageContentPart`；MessageList / StreamingRenderer media reference skeleton 已完成，新增 projection helper 单测覆盖 sidecar reference 转换与 inline `data:` fail closed，相关 GUI media Vitest 50 tests passed；`media-reference` GUI smoke 已验证 media card、chat-canvas Workbench preview skeleton 与 read model `contentParts`。Plugin runtime consumer 已由后续刀补上 skill prompt / MCP guidance 骨架，下一刀优先继续 binary sidecar read / full preview detail，或接 MCP runtime import；不在 Aster vendor 继续补业务代码。
> 本轮骨架追加（2026-07-07）：继续按“先快速完成骨架，然后回头完成细节”执行；App Server `context_media.rs` 已把 turn `AgentInput.attachments` 中的非 inline media URI 转成 `ContextPacket::media_reference(...)`，写入 `media_prompt_context` 和 `context_packet_telemetry`，inline `data:` media fail closed，不读取二进制、不把媒体正文塞进 prompt。`memory_prompt` 回归已补跑通过，`media_context` 2 tests、`context_packet` 4 tests、`memory_prompt` 14 tests 均 green。App Server plugin runtime consumer 已把 workflow-scoped skill prompt policy 接到 Agent Skills selection、session prompt 与 telemetry，`plugin_runtime` 8 tests green；MCP binding 已进入 `plugin_mcp_targets` inventory prepare request skeleton，`tool_inventory` 8 tests green；前端 `mcpApi.executePrepareRequests(...)` 已能顺序执行 candidate `prepareRequests`，复用 App Server current `mcpServer/importFromApp`、`mcpServer/start` 与 `mcpTool/listForContext`，API 定向测试 18 tests green；Workspace Harness 已接 `prepareMcpTargets` 手动 trigger，执行 candidate 后复用 `refreshToolInventory()` 刷新 inventory，hook / GUI 定向测试 32 tests green；自动触发、GUI smoke 与 caller-scoped 可调用闭环仍 pending。下一刀从 caller-scoped MCP callable proof、plugin package skill registry install path、binary sidecar read / full preview detail，或 Phase 6 provider/reply loop skeleton 里选，不再重复补已完成的 context / plugin prompt skeleton。
> Soul 对齐补充：Interaction Soul 不新增 runtime、transcript 或 UI 句库；它只作为 `memory_soul_prompt_context`、tool lifecycle metadata、collaboration facts、risk facts 和 GUI projection data contract 进入 `Thread -> Turn -> Item/read model` 主链。四个 built-in Style Pack seed 已迁到 `src/lib/soul/style-profiles/packs/*.json` manifest，并由 `builtInProfiles.ts` registry loader 与 App Server `runtime/soul/style_profile.rs` 校验 / 消费后输出完整 response contract、transcript surface contract 与 few-shot anchors，覆盖工具前/中/后、正文细节和结尾建议；前端 `manifest.ts` / `registry.ts` 已支持合并 built-in 与 installed `local_import/cloud_download` manifests，resolver / directive composer / 设置页 selector 消费同一 registry，缺失已选 profile 会显示中性提示而不是静默选中默认 built-in；`memory.soul.style_profile_id` 配置层已改为 registry profile id 字符串，不再用四 built-in enum 或旧 alias 映射限制风格包；deterministic transcript golden 已证明同一工具 facts 下四风格文本不同且 fact tokens 不漂移；`soul-style` Electron current fixture 已通过 `--soul-style-profile` 覆盖四个 built-in profile，证明 current pack id、lifecycle contract 和 profile-specific transcript 进入真实 GUI/read model；`AgentRuntimeStrip` 已接入 collaboration facts / Soul metadata contract；`runtime_status.rs` 旧 profile-specific title rewrite 已删除；旧共享 `com.lime.builtin.default` 和旧 `sassy_cute_executor` alias mapping 按 `dead / forbidden-to-restore` 处理。
...
| P2 深层能力 | `in-progress / app-server-tool-lifecycle-owner-done / lime-agent-event-owner-done / active-state-gate-done / approval-pending-transition-done / process-correlation-done / terminal-correlation-done / approval-resolved-terminal-done / evidence-export-correlation-done / action-tool-id-split-done / action-respond-lifecycle-guard-done / runtime-state-aster-resume-evidence-done / runtimebackend-bridge-evidence-done / tool-approval-sandbox-combo-evidence-done / context-fragment-owner-refined / context-packet-consumer-done / context-evidence-export-done / session-compaction-sidecar-and-item-done / memory-sidecar-skeleton-done / memory-regression-done / media-input-reference-skeleton-done / media-gui-preview-smoke-validated / binary-sidecar-read-app-server-skeleton-wired / binary-sidecar-read-runtime-test-sigterm / full-binary-preview-pending / plugin-runtime-capability-owner-done / plugin-app-center-projection-consumer-done / plugin-runtime-capability-schema-identity-guard-done / plugin-app-center-detail-consumer-done / skill-prompt-runtime-consumer-validated / mcp-guidance-runtime-consumer-validated / mcp-prepare-request-skeleton-done / mcp-prepare-execution-api-done / mcp-prepare-trigger-pending / runtime-content-owner-done / llm-media-content-event-done / message-delta-content-owner-done / message-delta-content-parser-done / gui-media-smoke-validated / binary-sidecar-read-app-server-skeleton-wired / binary-sidecar-read-runtime-test-sigterm / full-binary-preview-pending / soul-style-pack-manifest-registry-done / soul-app-server-manifest-consumer-done / soul-runtime-strip-consumer-done / soul-runtime-status-profile-switch-removed / codex-fifth-signal-handoff-ready / config-warning-app-server-owner-done / config-warning-frontend-consumer-done / config-warning-gui-presentation-done / provider-safety-owner-done / provider-safety-runtime-event-envelope-done / provider-safety-provider-sse-capture-done / provider-safety-read-model-done / provider-safety-gui-projection-done` | [p2-runtime-skeleton.md](./p2-runtime-skeleton.md) 已固定 Tool / Approval / Sandbox、Context / Token / Compaction、Plugin / Skills / MCP、Realtime / Media / Collaboration 的 owner、first code slice、禁止路径和最小验证；Tool / Approval / Sandbox 已完成 App Server external runtime event lifecycle owner、`lime-agent` typed event construction owner、active state gate、approval pending terminal transition、process lifecycle correlation、terminal metadata correlation、approval resolved terminal transition、Evidence export correlation 第一刀、action id / tool id 解耦前置刀、RuntimeCore `action/respond` lifecycle guard、RuntimeState/Aster pending future resume 证据、App Server `RuntimeBackend::respond_action` bridge evidence，以及 governance / contract / runtime fixture / projection 组合证据；Context / Token / Compaction 已完成 RuntimeCore fragment typed owner 精修，App Server `context_packet` 已从手写 token 截断迁到 `ContextFragmentEnvelope` / sidecar reference consumer，App Server `evidence/export` 已输出 context decision summary且不泄漏 preview 正文或 sha256，session compaction 已贯通真实 sidecar source 并 materialize 成 `context_compaction` read model Item，media reference GUI smoke 已验证 preview skeleton；Plugin / Skills / MCP 已完成 App Server plugin package `runtimeCapabilities` owner，并让 App Center plugin projection 优先消费 snapshot 生成 skill/tool/MCP runtime capability projection，schema gate 已拒绝 pluginId/version 与当前 projection app 不匹配的 snapshot，详情页技能展示已消费 projection 而非旧 `manifest.skillRefs`，App Server skill prompt runtime consumer 已把 workflow-scoped skill policy 接到 Agent Skills selection / session prompt / telemetry，MCP binding 已进入 App Server inventory `plugin_mcp_targets` prepare request skeleton，前端 API helper 已能执行 candidate prepare requests，但 GUI / 自动触发仍 pending；Realtime / Media / Collaboration 已完成 RuntimeCore content/media reference typed owner、LLM media output typed RuntimeEvent、`RuntimeMessageDeltaContent` payload owner、payload parser、App Server Item skeleton、protocol/generated、projection/hydrate、MessageList renderer skeleton 与 `media-reference` GUI smoke，binary sidecar read / full preview detail 仍 pending；Soul style pack seed 已迁到 manifest registry，App Server Soul prompt owner 已消费同一 manifest，Soul runtime strip 已消费 collaboration facts，旧 runtime status profile switch 已删除；Codex fifth signal handoff 已固定 App Server `configWarning`、provider safety buffering `retry_model`、Desktop Host startup env overlay 的 owner / 禁止路径 / 最小验证；`configWarning` App Server protocol / processor emitter / warning list container / frontend typed consumer / GUI presentation 已完成，`model-provider::safety` parser / typed payload owner、provider stream typed event contract、runtime event envelope、OpenAI Responses / Aster compat SSE 捕获、App Server read model projection 与 reliability panel projection 已完成；下一步转入 MCP prepare trigger、plugin skill registry install path、binary sidecar read / full preview detail，或 Phase 6 provider/reply loop skeleton，不再把 bridge、组合证据或 GUI media smoke 当 blocker |
...
| ---------- | ----------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------- | ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------ | --------------- | ------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-07-07 | P2 Plugin skill/MCP runtime consumer skeleton                                             | `skill-prompt-runtime-consumer-validated / mcp-guidance-runtime-consumer-validated / mcp-prepare-request-skeleton-done / mcp-prepare-execution-api-done / mcp-prepare-trigger-pending`      | `lime-rs/crates/app-server/src/runtime_backend/plugin_runtime_context.rs`；`lime-rs/crates/app-server/src/runtime_backend.rs`；`lime-rs/crates/app-server/src/runtime_backend/tool_inventory.rs`；`lime-rs/crates/app-server/src/runtime_backend/agent_skills_context.rs`；`lime-rs/crates/app-server/src/runtime_backend/agent_skills_telemetry.rs`；`lime-rs/crates/app-server/src/runtime_backend/request_context/session_config.rs`；`lime-rs/crates/app-server/src/runtime_backend/tests/session_skill_context.rs`；`lime-rs/crates/app-server/src/runtime_backend/tests/tool_inventory.rs`；`src/components/agent/chat/workspace/workspacePluginActivation.ts`；`src/components/agent/chat/workspace/workspacePluginActivation.unit.test.ts`；`src/lib/api/mcp.ts`；`src/lib/api/mcpTypes.ts`；`src/lib/api/mcp.test.ts`；`src/lib/api/mcp.failClosed.test.ts`；`internal/research/refactor/v1/priority-tracking-plan.md`；`internal/roadmap/astermigration/refactor-v1-impact-audit.md`；`internal/roadmap/astermigration/aster-capability-intake-execution-plan.md` | 继续避让 Aster vendor、provider/reply loop、MCP server import owner、plugin package skill registry install path、Media Item / Workbench GUI 热区；本刀不把 MCP guidance / prepare request skeleton / API helper 写成 MCP runtime-complete                                                                                                                                                                                                                                                                                                                                                                                                                                                            | App Server `plugin_runtime_context` 消费 plugin activation / harness runtimeCapabilities，workflow-scoped skill prompt policy 已接入 Agent Skills selection、session prompt 与 telemetry；`tool_inventory` 已把 MCP binding 归一化为 `plugin_mcp_targets`，消费 MCP server status / tool snapshot 后输出 `available` / `server_stopped` / `server_available_tool_missing` / `server_missing`、`expectedToolName`、`serverRunning`、`toolListRequest`、`prepareStatus` 与 candidate `prepareRequests` skeleton；`mcpApi.executePrepareRequests(...)` 已按 candidate request 顺序调用 App Server current `mcpServer/importFromApp`、`mcpServer/start`、`mcpTool/listForContext`，未知方法、非 candidate 和 malformed params fail closed 且不回退 `safeInvoke`。验证：`cargo test --manifest-path "lime-rs/Cargo.toml" -p app-server plugin_runtime -- --nocapture` 8 passed；`CARGO_TARGET_DIR="/tmp/lime-plugin-mcp-target" cargo test --manifest-path "lime-rs/Cargo.toml" -p app-server tool_inventory --lib -j 2` 8 passed；`npx vitest run "src/components/agent/chat/workspace/workspacePluginActivation.unit.test.ts"` 15 passed；`npx vitest run "src/lib/api/mcp.test.ts" "src/lib/api/mcp.failClosed.test.ts"` 18 passed；scoped `git diff --check` passed。下一刀：GUI / runtime trigger 触发 `executePrepareRequests`，或 plugin package skill registry install path。 |
| 2026-07-07 | P2 Codex fifth signal `configWarning` frontend typed consumer / GUI presentation          | `frontend-typed-consumer-done / gui-presentation-done / typecheck-green / contracts-green / gui-smoke-green`                                                              | `src/lib/api/appServer.ts`；`src/lib/api/appServerClient.ts`；`src/lib/api/appServerConstants.ts`；`src/lib/api/appServerResponse.ts`；`src/lib/api/appServerTypes.ts`；`src/lib/api/appServerConfigWarnings.ts`；`src/lib/api/appServerConfigWarnings.test.ts`；`src/lib/api/appServerResponse.test.ts`；`src/App.tsx`；`src/components/AppServerConfigWarningToastBridge.tsx`；`src/components/AppServerConfigWarningToastBridge.test.tsx`；`src/i18n/resources/{zh-CN,zh-TW,en-US,ja-JP,ko-KR}/common.json`；`src/i18n/agentChatCopy.ts`；`src/components/agent/chat/utils/agentChatCopy.ts`；`src/lib/soul/interactionCopy.ts`；`src/lib/soul/interactionCopy.unit.test.ts`；`internal/research/refactor/v1/p2-codex-fifth-signal-handoff.md`；`internal/research/refactor/v1/p2-runtime-skeleton.md`；`internal/research/refactor/v1/completion-audit.md`；`internal/research/refactor/v1/priority-tracking-plan.md`；`internal/research/refactor/v1/quality-fixture-matrix.md` | 继续避让 `lime-rs/crates/app-server/**`、`lime-rs/crates/model-provider/**`、`lime-rs/vendor/aster-rust/**`、`src/components/agent/chat/hooks/**`、`src/lib/soul/style-profiles/**`、`src/lib/api/memoryConfigTypes.ts` 与 safety buffering read model / GUI projection 热区；本刀不恢复 `.rules` reader、不接 provider SSE 捕获、不改 AgentChat 内联诊断                                                                                                                                                                                                                                                                                                                                                                                                                         | App Server response projector 已从 JSON-RPC success/error path 提取 typed `configWarnings`；`AppServerClient.request(...)` 通过 warning sink 发布并保留 method / phase / requestId context；GUI 全局 toast bridge 消费 typed warning，五语言文案齐全，renderer 不读 rules/config，不解析 warning 文案。验证：定向 Vitest 48 tests passed；`npm run typecheck`、相关 ESLint / Prettier、`npm run i18n:check`、`npm run test:contracts`、`npm run verify:gui-smoke` 均通过；GUI smoke 证明 renderer loaded、App Server initialized、Claw workbench shell ready、memory settings ready。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| 2026-07-07 | P2 Codex fifth signal safety buffering read model projection                              | `read-model-done / gui-presentation-done-by-follow-up / app-server-test-green / frontend-related-hotzone-red`                                                             | `lime-rs/crates/app-server/src/runtime/read_model.rs`；`lime-rs/crates/app-server/src/runtime/tests/read_model/messages_diagnostics.rs`；`src/lib/api/agentRuntime/types.ts`；`src/lib/api/agentRuntime/types.d.ts`；`internal/research/refactor/v1/p2-codex-fifth-signal-handoff.md`；`internal/research/refactor/v1/priority-tracking-plan.md`；`internal/research/refactor/v1/completion-audit.md`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | 继续避让 `lime-rs/vendor/aster-rust/**`、`lime-rs/crates/agent/**`、`lime-rs/crates/model-provider/**`、`src/components/agent/chat/**`、`src/lib/api/appServer*.ts` 与 GUI presentation 热区；本刀不在 Aster 继续补代码，不改 AgentChat 预览/StreamingRenderer 失败用例                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | App Server read model 把 `provider_safety_buffering` RuntimeEvent 投到 `thread_read.diagnostics.provider_safety_buffering_count`、`latest_provider_safety_buffering` 与 `runtime_summary.latestProviderSafetyBuffering`，只暴露 provider/model/use_cases/reasons/retry/fallback/source/backend 等精选字段，不透传 raw `runtimeEvent`。验证：`cargo test --manifest-path "lime-rs/Cargo.toml" -p app-server read_session_projects_provider_safety_buffering_into_diagnostics -- --nocapture` 1 passed；`cargo fmt --manifest-path "lime-rs/Cargo.toml" -p app-server -- --check` passed；scoped `git diff --check` passed。`npm run test:related -- src/lib/api/agentRuntime/types.ts src/lib/api/agentRuntime/types.d.ts` 当时失败 5 tests，落在 `AgentChatPage` bootstrap / preview 与 `StreamingRenderer` 缓存/首帧热区，不属于本刀写集；GUI projection 缺口已由后续 safety buffering GUI projection 切片关闭。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| 2026-07-07 | P2 Codex fifth signal safety buffering GUI projection                                     | `gui-presentation-done / frontend-tests-green / app-server-test-green / inputbar-rich-restore-green / aggregate-fixture-green`                                             | `src/components/agent/chat/components/AgentThreadProviderSafetyBufferingCard.tsx`；`src/components/agent/chat/components/AgentThreadReliabilityPanel.tsx`；`src/components/agent/chat/utils/providerSafetyBufferingDiagnostic.ts`；`src/components/agent/chat/utils/threadReliabilityDiagnosticText.ts`；`src/components/agent/chat/components/AgentThreadReliabilityPanel.test.tsx`；`src/components/agent/chat/components/AgentThreadReliabilityPanel.memory.test.tsx`；`src/i18n/resources/{zh-CN,zh-TW,en-US,ja-JP,ko-KR}/agentRuntime.json`；`internal/research/refactor/v1/p2-codex-fifth-signal-handoff.md`；`internal/research/refactor/v1/priority-tracking-plan.md`                                                                                                                                                                                                                                                                                                        | 继续避让 `src/components/agent/chat/utils/agentChatCopy.ts`、`src/i18n/resources/*/common.json`、App Server runtime/backend、Aster compat provider、provider parser 与 `lime-rs/vendor/aster-rust/**`；Inputbar rich restore 已由第 75.6 节转绿，本刀仍不改 raw provider capture、不修 AgentChat preview / StreamingRenderer 旁路失败用例                                                                                                                                                                                                                                                                                                                                                                          | Reliability panel 新增 provider safety buffering 诊断卡，并让复制诊断包消费 `thread_read.diagnostics.latest_provider_safety_buffering`；GUI 只读 read model sample，展示 provider/model、retry model、fallback header model、use cases、reasons、source、buffering UI 状态，不解析 raw `runtimeEvent`、provider wire、Aster sentinel 或 legacy `fasterModel`。用户可见文案覆盖五语言。验证：`XDG_CACHE_HOME="/tmp/lime-codex-node-cache-provider-safety-projection" npm test -- "src/components/agent/chat/components/AgentThreadReliabilityPanel.test.tsx" "src/components/agent/chat/components/AgentThreadReliabilityPanel.memory.test.tsx"` 18 tests passed；`CARGO_HOME="/tmp/lime-codex-cargo-home-provider-safety-projection" CARGO_TARGET_DIR="/tmp/lime-codex-provider-safety-projection-target" CARGO_INCREMENTAL=0 CARGO_PROFILE_DEV_DEBUG=0 CARGO_PROFILE_TEST_DEBUG=0 CARGO_BUILD_JOBS=4 cargo test --manifest-path "lime-rs/Cargo.toml" -p app-server provider_safety_buffering -- --nocapture` 1 matching read model test passed；`inputbar-rich-restore` 单场景 passed；`npm run smoke:agent-runtime-current-fixture` 聚合 passed，`liveProviderUsed=false`，详见第 75.6 节。 |
| 2026-07-07 | P2 Codex fifth signal OpenAI Responses safety buffering SSE capture                       | `provider-sse-capture-done / read-model-done-by-follow-up / gui-presentation-done-by-follow-up`                                                                           | `lime-rs/vendor/aster-rust/crates/aster/src/providers/formats/openai_responses.rs`；`lime-rs/vendor/aster-rust/crates/aster/src/providers/openai.rs`；`lime-rs/vendor/aster-rust/crates/aster/src/agents/agent.rs`；`lime-rs/crates/agent/src/request_tool_policy/aster_reply_adapter.rs`；`lime-rs/crates/agent/src/request_tool_policy/aster_reply_adapter/tests.rs`；`internal/research/refactor/v1/p2-codex-fifth-signal-handoff.md`；`internal/research/refactor/v1/priority-tracking-plan.md`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | 继续避让 `lime-rs/crates/model-provider/src/**`、`lime-rs/crates/app-server/src/runtime_backend/**`、`packages/agent-runtime-projection/**`、`src/components/agent/chat/hooks/**`、`src/components/agent/chat/**`、`src/lib/api/**`、`src/lib/governance/**`、`scripts/**`、`electron/**`；本刀不接 GUI projection，不改 safety owner，不碰前端 hook 热区                                                                                                                                                                                                                                                                                                                                                                                                                         | Aster OpenAI Responses parser 在 raw SSE JSON 含对象型 `safety_buffering` 时发出 invisible provider stream sentinel；`safety_buffering=false` 不发事件。OpenAI provider 把 legacy safety buffering headers 带入 parser；Aster agent loop 遇 sentinel 只向外 yield，不写 conversation history；Lime Aster adapter 消费 sentinel 并调用 `RuntimeReplyProviderStreamEvent::safety_buffering_from_response_event(...)`，输出 typed `ProviderStreamEvent`。验证：`aster-core` / `lime-agent` check 与两个定向单测均通过，Cargo home / target 均在 `/tmp`；详见第 73 节；后续 read model 与 GUI presentation 已由本计划同日 follow-up 关闭。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| 2026-07-07 | P2 Codex fifth signal provider stream runtime event envelope                              | `runtime-event-envelope-done / provider-sse-capture-done-by-follow-up / read-model-done-by-follow-up / gui-presentation-done-by-follow-up`                                | `lime-rs/crates/agent-runtime/src/reply_stream.rs`；`lime-rs/crates/agent/src/protocol.rs`；`lime-rs/crates/agent/src/request_tool_policy/agent_reply_stream.rs`；`lime-rs/crates/app-server/src/runtime_backend/tool_events.rs`；`internal/research/refactor/v1/p2-codex-fifth-signal-handoff.md`；`internal/research/refactor/v1/priority-tracking-plan.md`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | 只读 `lime-rs/crates/model-provider/src/provider_stream.rs`；继续避让 `lime-rs/crates/agent/src/tools/skill_search_tool.rs`、`packages/agent-runtime-projection/**`、`src/components/agent/chat/**`、`src/lib/api/**`、`src/lib/governance/**`、`scripts/**`、`electron/**`；本刀不捕获真实 provider SSE / Responses event，不接 GUI                                                                                                                                                                                                                                                                                                                                                                                                                                              | `agent-runtime::RuntimeReplyStreamEvent` 新增 `ProviderStreamEvent(RuntimeReplyProviderStreamEvent)` envelope；`lime-agent` 新增 `AgentEvent::ProviderStreamEvent` 并在 reply stream policy 中把 provider typed event 投到 agent DTO；App Server mapper 对该 DTO 输出 allowlist 后的 `provider_safety_buffering` RuntimeEvent，未知 provider stream kind fail-closed 到 `runtime.event`，payload 只消费 typed `payload_json_value()`，不解析 raw `retry_model` / `fasterModel`。验证：`CARGO_HOME="/tmp/lime-codex-cargo-home-model-provider" CARGO_TARGET_DIR="/tmp/lime-codex-provider-stream-runtime-event-target" CARGO_BUILD_JOBS=4 cargo fmt --manifest-path "lime-rs/Cargo.toml" -p agent-runtime -p lime-agent -p app-server --check`；同环境 `cargo test --manifest-path "lime-rs/Cargo.toml" -p agent-runtime reply_stream -- --nocapture` 3 tests passed；同环境 `cargo test --manifest-path "lime-rs/Cargo.toml" -p lime-agent provider_stream_reply_event_projects_agent_event -- --nocapture` 1 test passed；同环境 `cargo test --manifest-path "lime-rs/Cargo.toml" -p lime-agent agent_event_provider_stream_event_serializes_current_payload -- --nocapture` 1 test passed；同环境 `cargo test --manifest-path "lime-rs/Cargo.toml" -p app-server provider_stream_event -- --nocapture` 2 tests passed；后续 SSE capture、read model 与 GUI presentation 已由同日 follow-up 关闭。 |
| 2026-07-07 | P2 Codex fifth signal `configWarning` processor config-load emitter                       | `processor-config-load-emitter-done / warning-list-owner-done / rules-owner-gap-documented / app-server-test-green`                                                       | `lime-rs/crates/app-server/src/processor/config_warning.rs`；`lime-rs/crates/app-server/src/processor/mod.rs`；`lime-rs/crates/app-server/src/processor/dispatch.rs`；`lime-rs/crates/app-server/src/processor/tests.rs`；`lime-rs/crates/app-server/src/processor/tests/config_warning.rs`；`internal/research/refactor/v1/p2-codex-fifth-signal-handoff.md`；`internal/research/refactor/v1/p2-runtime-skeleton.md`；`internal/research/refactor/v1/completion-audit.md`；`internal/research/refactor/v1/priority-tracking-plan.md`                                                                                                                                                                                                                                                                                                                                                                                                                                                | 继续避让 `lime-rs/crates/app-server/src/runtime/**`、`lime-rs/crates/app-server/src/runtime_backend/**`、`lime-rs/crates/model-provider/src/provider_stream.rs`、`lime-rs/crates/tool-runtime/**`、`electron/**`、`src/lib/api/**`、`packages/agent-runtime-projection/**`、`src/components/agent/chat/**`、`src/lib/governance/**`、`lime-rs/vendor/aster-rust/**`；本刀不接 bridge/GUI consumer；Lime 当前没有 `.rules` owner，standalone rules / exec-policy reload 只作为 future design gap                                                                                                                                                                                                                                                                                   | App Server processor current 主链新增只读 config warning probe：现有 `config.yaml` / legacy `config.json` 解析失败时生成 typed `configWarning`；`initialize` 与 `agentSession/turn/start` 均把 notification 合并进当前 JSON-RPC response path，turn-start 保持 connection-scoped，不走 broadcast、不进 GUI 旁路。精修后 `ConfigWarningProvider` 返回 `Vec<JsonRpcNotification>`，`RpcDispatch` 支持批量追加 notification，并新增 initialize 多 warning 回归，为后续多个 config source 共用同一容器留出接口。验证：`cargo fmt --manifest-path "lime-rs/Cargo.toml" -p app-server --check`、scoped `git diff --check` 通过；`CARGO_TARGET_DIR="/tmp/lime-app-server-config-warning-target" CARGO_BUILD_JOBS=4 cargo test --manifest-path "lime-rs/Cargo.toml" -p app-server config_warning -- --nocapture` 6 tests passed。当时剩余的 frontend consumer / GUI presentation 已由本计划同日后续切片关闭。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| 2026-07-07 | P2 Codex fifth signal `configWarning` protocol contract                                   | `protocol-contract-done / superseded-by-runtime-emitter-and-gui-consumer`                                                                                                 | `lime-rs/crates/app-server-protocol/src/protocol/v0/method_names.rs`；`lime-rs/crates/app-server-protocol/src/protocol/v0/server_notification.rs`；`lime-rs/crates/app-server-protocol/src/protocol/v0/catalog.rs`；`lime-rs/crates/app-server-protocol/src/protocol/v0/tests/catalog.rs`；`lime-rs/crates/app-server-protocol/src/protocol/v0/schema_types.rs`；`lime-rs/crates/app-server-protocol/src/schema_export/registry.rs`；`lime-rs/crates/app-server-protocol/schema/json/**`；`packages/app-server-client/src/generated/protocol-types.ts`；`internal/research/refactor/v1/p2-codex-fifth-signal-handoff.md`；`internal/research/refactor/v1/priority-tracking-plan.md`；`internal/research/refactor/v1/p2-runtime-skeleton.md`；`internal/research/refactor/v1/completion-audit.md`                                                                                                                                                                                     | 继续避让 `lime-rs/crates/app-server/src/runtime/**`、`lime-rs/crates/app-server/src/runtime_backend/**`、`lime-rs/crates/model-provider/src/provider_stream.rs`、`electron/**`、`src/lib/api/**`、`packages/agent-runtime-projection/**`、`src/components/agent/chat/**`、`src/lib/governance/**`、`lime-rs/vendor/aster-rust/**`；本刀不接 runtime emitter、不接 bridge/GUI consumer、不碰 Aster 迁移热区                                                                                                                                                                                                                                                                                                                                                                        | App Server protocol 已新增 Codex-style `configWarning` notification contract：`METHOD_CONFIG_WARNING`、`ServerNotification::ConfigWarning(ConfigWarningNotification)`、`TextPosition/TextRange`、method catalog、schema registry、schema fixture 与 generated TS 均同步。验证：`notification_round_trips_config_warning_payload`、`schema_registry_matches_declared_type_names`、`schema_fixtures_match_generated_output`、隔离 target catalog 测试、`npm run check:protocol-types` 通过。当时剩余的 App Server emitter 与 frontend/GUI consumer 已由后续切片关闭；response-path connection 归属以 processor / frontend consumer 验证为准。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| 2026-07-07 | P2 Codex fifth signal handoff                                                             | `handoff-ready / config-warning-gui-consumer-done / provider-safety-owner-done / provider-stream-consumer-done / provider-read-model-done / provider-gui-projection-done` | `internal/research/refactor/v1/p2-codex-fifth-signal-handoff.md`；`internal/research/refactor/v1/README.md`；`internal/research/refactor/v1/p2-runtime-skeleton.md`；`internal/research/refactor/v1/priority-tracking-plan.md`；`internal/research/refactor/v1/quality-fixture-matrix.md`；`internal/research/refactor/v1/completion-audit.md`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | 只读审计 `lime-rs/crates/model-provider/src/provider_stream.rs`、App Server runtime/backend、Electron、frontend API、projection package 与 AgentChat GUI；继续避让 `electron/**`、`packages/agent-runtime-projection/**`、`src/components/agent/chat/**`、`src/lib/governance/**`、`lime-rs/vendor/aster-rust/**`；本刀不接 Desktop Host env overlay                                                                                                                                                                                                                                                                                                                                                                                                                              | 第五次 Codex 信号施工单已更新：App Server `configWarning` owner 已完成 protocol contract、config-load emitter、warning list container、frontend typed consumer 与 GUI presentation；safety buffering 已完成 provider parser / typed payload owner、provider stream typed event、RuntimeEvent envelope、OpenAI Responses / Aster compat SSE capture、App Server read model projection 与 reliability diagnostics projection；conditional dotenv 仍只作为 Desktop Host sidecar startup env overlay watch，不照搬 CLI `CODEX_HOME` / `arg0`。Lime 当前没有 `.rules` current owner，standalone `.rules` / exec-policy reload 只能作为 future design gap；GUI presentation 缺口已由第 75 节关闭，下一刀补 current fixture / GUI smoke 证据。验证：scoped 文档 whitespace / conflict marker check。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| 2026-07-07 | P2 Codex fifth signal safety buffering RuntimeEvent payload owner                         | `provider-safety-buffering-runtime-payload-owner-done / provider-stream-consumer-done-by-follow-up`                                                                       | `lime-rs/crates/model-provider/src/safety.rs`；`internal/research/refactor/v1/priority-tracking-plan.md`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | 继续避让 `lime-rs/crates/model-provider/src/provider_stream.rs`、`lime-rs/crates/app-server/src/runtime/**`、`lime-rs/crates/app-server/src/runtime_backend/**`、`lime-rs/crates/agent/src/**`、`packages/agent-runtime-projection/**`、`src/components/agent/chat/**`、`src/lib/api/**`、`src/lib/governance/**`、`scripts/**`、`electron/**`；本刀不接 provider stream，不写 App Server RuntimeEvent storage/read model/GUI projection                                                                                                                                                                                                                                                                                                                                          | `model-provider::safety` 新增 `ProviderSafetyBufferingUpdate`、`parse_safety_buffering_update(...)`、`safety_buffering_enabled_header(...)` 与 `to_runtime_event_payload(...)`，让后续 RuntimeEvent 直接消费 camelCase typed payload：`retryModel`、`fallbackHeaderModel`、`source`、`showBufferingUi`、`useCases`、`reasons`；payload 不暴露 raw `retry_model` / `faster_model` / `fasterModel`，避免 provider stream 接入后再次解释 wire 字段。验证：`CARGO_HOME="/tmp/lime-codex-cargo-home-model-provider" CARGO_TARGET_DIR="/tmp/lime-codex-model-provider-safety-target" CARGO_BUILD_JOBS=4 cargo fmt --manifest-path "lime-rs/Cargo.toml" -p model-provider --check`、同环境 `cargo test --manifest-path "lime-rs/Cargo.toml" -p model-provider safety -- --nocapture` 21 tests 通过、同环境 `cargo test --manifest-path "lime-rs/Cargo.toml" -p model-provider -- --nocapture` 33 tests 通过。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| 2026-07-07 | P2 Codex fifth signal safety buffering response-event owner                               | `provider-safety-buffering-response-event-owner-done / provider-stream-consumer-done-by-follow-up`                                                                        | `lime-rs/crates/model-provider/src/safety.rs`；`internal/research/refactor/v1/priority-tracking-plan.md`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | 继续避让 `lime-rs/crates/model-provider/src/provider_stream.rs`、`lime-rs/crates/app-server/src/runtime/**`、`lime-rs/crates/app-server/src/runtime_backend/**`、`lime-rs/crates/agent/src/**`、`packages/agent-runtime-projection/**`、`src/components/agent/chat/**`、`src/lib/api/**`、`src/lib/governance/**`、`scripts/**`、`electron/**`；本刀不夹写 provider stream / App Server runtime / read model / GUI projection                                                                                                                                                                                                                                                                                                                                                     | `model-provider::safety` 新增 `SAFETY_BUFFERING_RESPONSE_EVENT_FIELD` 与 `parse_safety_buffering_response_event(...)`，把 Responses event 的 `safety_buffering` 字段提取也封进 current safety owner；provider stream 后续只调用 owner，不自行解释 raw event object / boolean false / `retry_model`。验证：`CARGO_HOME="/tmp/lime-codex-cargo-home-model-provider" CARGO_TARGET_DIR="/tmp/lime-codex-model-provider-safety-target" CARGO_BUILD_JOBS=4 cargo fmt --manifest-path "lime-rs/Cargo.toml" -p model-provider --check`、同环境 `cargo test --manifest-path "lime-rs/Cargo.toml" -p model-provider safety -- --nocapture` 23 tests 通过、同环境 `cargo test --manifest-path "lime-rs/Cargo.toml" -p model-provider -- --nocapture` 35 tests 通过；Cargo home / target 均在 `/tmp`。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| 2026-07-07 | P2 Codex fifth signal safety owner test split                                             | `safety-owner-size-guard-done / provider-stream-consumer-done-by-follow-up`                                                                                               | `lime-rs/crates/model-provider/src/safety.rs`；`lime-rs/crates/model-provider/src/safety/tests.rs`；`internal/research/refactor/v1/priority-tracking-plan.md`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | 继续避让 `lime-rs/crates/model-provider/src/provider_stream.rs`、`lime-rs/crates/app-server/src/runtime/**`、`lime-rs/crates/app-server/src/runtime_backend/**`、`lime-rs/crates/agent/src/**`、`packages/agent-runtime-projection/**`、`src/components/agent/chat/**`、`src/lib/api/**`、`src/lib/governance/**`、`scripts/**`、`electron/**`；本刀不改 provider stream 行为                                                                                                                                                                                                                                                                                                                                                                                                     | 将 `safety.rs` 内联测试拆到 `safety/tests.rs`，`safety.rs` 从 778 行降到 351 行，避免继续向接近 800 行的 owner 文件追加新业务逻辑；现有 safety parser / RuntimeEvent payload / response-event owner 行为不变。验证：`CARGO_HOME="/tmp/lime-codex-cargo-home-model-provider" CARGO_TARGET_DIR="/tmp/lime-codex-model-provider-safety-target" CARGO_BUILD_JOBS=4 cargo fmt --manifest-path "lime-rs/Cargo.toml" -p model-provider --check`、同环境 `cargo test --manifest-path "lime-rs/Cargo.toml" -p model-provider safety -- --nocapture` 23 tests 通过、同环境 `cargo test --manifest-path "lime-rs/Cargo.toml" -p model-provider -- --nocapture` 35 tests 通过；Cargo home / target 均在 `/tmp`。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| 2026-07-07 | P2 Codex fifth signal safety buffering typed runtime payload refine                       | `provider-safety-buffering-typed-runtime-payload-done / provider-stream-consumer-done-by-follow-up`                                                                       | `lime-rs/crates/model-provider/src/safety.rs`；`lime-rs/crates/model-provider/src/safety/tests.rs`；`internal/research/refactor/v1/priority-tracking-plan.md`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | 继续避让 `lime-rs/crates/model-provider/src/provider_stream.rs`、`lime-rs/crates/app-server/src/runtime/**`、`lime-rs/crates/app-server/src/runtime_backend/**`、`lime-rs/crates/agent/src/**`、`packages/agent-runtime-projection/**`、`src/components/agent/chat/**`、`src/lib/api/**`、`src/lib/governance/**`、`scripts/**`、`electron/**`；本刀不接 provider stream / App Server RuntimeEvent / read model / GUI projection                                                                                                                                                                                                                                                                                                                                                  | `model-provider::safety` 新增 `ProviderSafetyBufferingRuntimeEventPayload` 与 `SAFETY_BUFFERING_RUNTIME_EVENT_KIND`，`ProviderSafetyBufferingUpdate::runtime_event_payload(...)` 返回 typed payload，`to_runtime_event_payload(...)` 只负责序列化同一 typed payload；测试覆盖 explicit null 在 RuntimeEvent payload 中保持 typed null 且不暴露 raw `retry_model` / `faster_model`。验证：`CARGO_HOME="/tmp/lime-codex-cargo-home-model-provider" CARGO_TARGET_DIR="/tmp/lime-codex-model-provider-safety-typed-payload-target" CARGO_BUILD_JOBS=4 cargo fmt --manifest-path "lime-rs/Cargo.toml" -p model-provider --check`、同环境 `cargo test --manifest-path "lime-rs/Cargo.toml" -p model-provider safety -- --nocapture` 24 tests 通过、同环境 `cargo test --manifest-path "lime-rs/Cargo.toml" -p model-provider -- --nocapture` 36 tests 通过；Cargo home / target 均在 `/tmp`。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| 2026-07-07 | P2 Codex fifth signal safety buffering source enum refine                                 | `provider-safety-buffering-source-enum-done / provider-stream-consumer-done-by-follow-up`                                                                                 | `lime-rs/crates/model-provider/src/safety.rs`；`lime-rs/crates/model-provider/src/safety/tests.rs`；`internal/research/refactor/v1/priority-tracking-plan.md`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | 继续避让 `lime-rs/crates/model-provider/src/provider_stream.rs`、`lime-rs/crates/app-server/src/runtime/**`、`lime-rs/crates/app-server/src/runtime_backend/**`、`lime-rs/crates/agent/src/**`、`packages/agent-runtime-projection/**`、`src/components/agent/chat/**`、`src/lib/api/**`、`src/lib/governance/**`、`scripts/**`、`electron/**`；本刀不接 provider stream / App Server RuntimeEvent / read model / GUI projection                                                                                                                                                                                                                                                                                                                                                  | `ProviderSafetyBufferingRetryModelSource` 派生 `Serialize` 并按 snake_case 序列化，`ProviderSafetyBufferingRuntimeEventPayload.source` 从 `&'static str` 改为该 enum；typed payload consumer 不再持有 source 字符串，JSON 仍输出 `payload_retry_model` / `explicit_null` 等稳定值。验证：`CARGO_HOME="/tmp/lime-codex-cargo-home-model-provider" CARGO_TARGET_DIR="/tmp/lime-codex-model-provider-safety-source-enum-target" CARGO_BUILD_JOBS=4 cargo fmt --manifest-path "lime-rs/Cargo.toml" -p model-provider --check`、同环境 `cargo test --manifest-path "lime-rs/Cargo.toml" -p model-provider safety -- --nocapture` 24 tests 通过、同环境 `cargo test --manifest-path "lime-rs/Cargo.toml" -p model-provider -- --nocapture` 36 tests 通过；Cargo home / target 均在 `/tmp`。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| 2026-07-07 | P2 Codex fifth signal safety buffering stream-ready runtime payload helper                | `provider-safety-buffering-stream-ready-payload-done / provider-stream-consumer-done-by-follow-up`                                                                        | `lime-rs/crates/model-provider/src/safety.rs`；`lime-rs/crates/model-provider/src/safety/tests.rs`；`internal/research/refactor/v1/priority-tracking-plan.md`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | 继续避让 `lime-rs/crates/model-provider/src/provider_stream.rs`、`lime-rs/crates/app-server/src/runtime/**`、`lime-rs/crates/app-server/src/runtime_backend/**`、`lime-rs/crates/agent/src/**`、`packages/agent-runtime-projection/**`、`src/components/agent/chat/**`、`src/lib/api/**`、`src/lib/governance/**`、`scripts/**`、`electron/**`；本刀不接 provider stream / App Server RuntimeEvent / read model / GUI projection                                                                                                                                                                                                                                                                                                                                                  | 新增 `parse_safety_buffering_runtime_event_payload(...)`，把完整 Responses event + headers + provider/model 直接投影成 `ProviderSafetyBufferingRuntimeEventPayload`；新增 `ProviderSafetyBufferingRuntimeEventPayload::to_json_value()`，让 JSON 序列化也归属 typed payload owner，`Update::to_runtime_event_payload(...)` 仅委托它。测试覆盖 legacy header fallback 的 stream-ready payload、typed payload 自身 JSON 输出与 `safety_buffering=false` 不产出 payload。验证：`CARGO_HOME="/tmp/lime-codex-cargo-home-model-provider" CARGO_TARGET_DIR="/tmp/lime-codex-model-provider-safety-stream-payload-target" CARGO_BUILD_JOBS=4 cargo fmt --manifest-path "lime-rs/Cargo.toml" -p model-provider --check`、同环境 `cargo test --manifest-path "lime-rs/Cargo.toml" -p model-provider safety -- --nocapture` 26 tests 通过、同环境 `cargo test --manifest-path "lime-rs/Cargo.toml" -p model-provider -- --nocapture` 38 tests 通过；Cargo home / target 均在 `/tmp`。                                                                                                                                                                                                                                                                                                                                                                                                                         |
| 2026-07-07 | P2 Codex fifth signal provider stream safety buffering event contract                     | `provider-stream-safety-buffering-event-contract-done / runtime-consumer-pending`                                                                                         | `lime-rs/crates/model-provider/src/provider_stream.rs`；`lime-rs/crates/model-provider/src/safety.rs`；`lime-rs/crates/model-provider/src/safety/tests.rs`；`internal/research/refactor/v1/priority-tracking-plan.md`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | 继续避让 `lime-rs/crates/app-server/src/runtime/**`、`lime-rs/crates/app-server/src/runtime_backend/**`、`lime-rs/crates/agent/src/**`、`packages/agent-runtime-projection/**`、`src/components/agent/chat/**`、`src/lib/api/**`、`src/lib/governance/**`、`scripts/**`、`electron/**`；本刀只接 model-provider provider stream contract，不接 App Server RuntimeEvent storage/read model/GUI projection                                                                                                                                                                                                                                                                                                                                                                          | `provider_stream.rs` 新增 `RuntimeReplyProviderStreamEvent::SafetyBuffering` 与 `safety_buffering_from_response_event(...)`，provider stream 侧直接消费 safety owner 的 typed payload，不再自行解释 `retry_model`、legacy header 或 RuntimeEvent JSON 字段；新增 `runtime_event_kind()` 与 `payload_json_value()` 作为后续 runtime consumer 的窄出口。验证：`CARGO_HOME="/tmp/lime-codex-cargo-home-model-provider" CARGO_TARGET_DIR="/tmp/lime-codex-model-provider-provider-stream-safety-target" CARGO_BUILD_JOBS=4 cargo fmt --manifest-path "lime-rs/Cargo.toml" -p model-provider --check`、同环境 `cargo test --manifest-path "lime-rs/Cargo.toml" -p model-provider provider_stream -- --nocapture` 9 tests 通过、同环境 `cargo test --manifest-path "lime-rs/Cargo.toml" -p model-provider safety -- --nocapture` 28 tests 通过、同环境 `cargo test --manifest-path "lime-rs/Cargo.toml" -p model-provider -- --nocapture` 40 tests 通过；Cargo home / target 均在 `/tmp`。                                                                                                                                                                                                                                                                                                                                                                                                                |
| 2026-07-07 | P2 第二十九刀：Plugin runtime skill prompt / MCP guidance consumer                        | `skill-prompt-runtime-consumer-validated / mcp-guidance-runtime-consumer-validated / superseded-by-76-mcp-target-skeleton`                                                | `lime-rs/crates/app-server/src/runtime_backend/plugin_runtime_context.rs`；`lime-rs/crates/app-server/src/runtime_backend/agent_skills_context.rs`；`lime-rs/crates/app-server/src/runtime_backend/agent_skills_telemetry.rs`；`lime-rs/crates/app-server/src/runtime_backend/request_context/session_config.rs`；`lime-rs/crates/app-server/src/runtime_backend/tests/session_skill_context.rs`；`internal/research/refactor/v1/priority-tracking-plan.md`；`internal/research/refactor/v1/p2-runtime-skeleton.md`；`internal/research/refactor/v1/quality-fixture-matrix.md`；`internal/research/refactor/v1/completion-audit.md`                                                                                                                                                                                                                                                                                                                                                    | 继续避让 `lime-rs/crates/app-server/src/runtime/**`、`lime-rs/crates/agent/src/**`、`packages/agent-runtime-projection/**`、`src/components/agent/chat/**`、`src/lib/api/**`、`src/lib/governance/**`、`scripts/**`、`electron/**` 并行热区；本刀不导入 MCP runtime、不声明 MCP tools 可用、不改 GUI media / Workbench 热区                                                                                                                                                                                                                                                                                                                                                                                                                         | App Server `plugin_runtime_context` 消费 plugin activation / harness runtime capabilities，workflow-scoped skill prompt policy 可进入 Agent Skills selection、session system prompt 和 telemetry；activation plugin mismatch fail closed；MCP binding 只渲染 guidance，并明确需要 runtime MCP status/tool listing 确认后才能声明工具可用。验证：`TMPDIR="/tmp/lime-codex-rust-tmp-plugin-runtime" XDG_CACHE_HOME="/tmp/lime-codex-xdg-cache" npm_config_cache="/tmp/lime-codex-npm-cache" CARGO_HOME="/tmp/lime-codex-cargo-home-media-context" CARGO_TARGET_DIR="/tmp/lime-codex-media-context-target" CARGO_INCREMENTAL=0 CARGO_PROFILE_DEV_DEBUG=0 CARGO_PROFILE_TEST_DEBUG=0 CARGO_PROFILE_TEST_STRIP=none CARGO_BUILD_JOBS=1 cargo test --manifest-path "lime-rs/Cargo.toml" -p app-server plugin_runtime -- --nocapture` 通过，7 tests；第 76 节已补 `plugin_mcp_targets` prepare request skeleton 与 `tool_inventory` 验证。                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| P2     |   10 | Context / Token / Compaction                     | `context-packet-consumer-done / context-evidence-export-done / session-compaction-sidecar-and-item-done / memory-sidecar-skeleton-done / memory-regression-done / media-input-reference-skeleton-done / media-gui-preview-smoke-validated / binary-sidecar-read-app-server-skeleton-wired / binary-sidecar-read-runtime-test-sigterm / full-binary-preview-pending`                                        | `ContextFragmentEnvelope` / budget decision / sidecar-reference owner 已落到 RuntimeCore；preview 同时受 char policy 与 `max_model_visible_tokens` 约束，zero-preview + sidecar 明确为 `reference_only`；App Server `context_packet` admitted packet 已消费 envelope 并输出 `fragmentEnvelope` telemetry；App Server `evidence/export` 已输出 context decision summary；session compaction 已把真实 `sidecarRef` 贯通到 packet metadata，并把 compaction 结果 materialize 成 read model `context_compaction` Item；memory summary sidecarRef 已进入 `ContextPacket.metadata` 骨架，且不保存 `memory_soul_prompt_context`、Style Pack 或 full system prompt；turn attachment media URI 已进入 `media_reference` ContextPacket telemetry，inline `data:` media fail closed；`media-reference` GUI smoke 已通过；下一刀只剩 binary sidecar read / full preview detail | `cargo test --manifest-path "lime-rs/Cargo.toml" -p runtime-core context_fragments -- --nocapture`；`cargo test --manifest-path "lime-rs/Cargo.toml" -p app-server context_packet -- --nocapture`；`cargo test --manifest-path "lime-rs/Cargo.toml" -p app-server export_evidence_pack_includes_context_fragment_summary_from_turn_metadata -- --nocapture`；`cargo test --manifest-path "lime-rs/Cargo.toml" -p app-server compact_agent_session_writes_session_context_artifact -- --nocapture`；`cargo test --manifest-path "lime-rs/Cargo.toml" -p app-server compact_agent_session_injects_next_turn_session_context_packet -- --nocapture`；`cargo test --manifest-path "lime-rs/Cargo.toml" -p app-server memory_prompt -- --nocapture` 14 tests；`cargo test --manifest-path "lime-rs/Cargo.toml" -p app-server media_context -- --nocapture` 2 tests；`npm run smoke:claw-chat-current-fixture -- --scenario media-reference --timeout-ms 240000 --evidence-dir "/tmp/lime-codex-media-reference-evidence" --prefix "claw-chat-current-fixture-media-reference"` passed |
| P2     |   11 | Plugin / Skills / MCP                            | `plugin-app-center-projection-consumer-done / skill-prompt-runtime-consumer-validated / mcp-guidance-runtime-consumer-validated / mcp-prepare-request-skeleton-done / mcp-prepare-execution-api-done / mcp-prepare-trigger-skeleton-done / mcp-runtime-callable-proof-pending` | `runtimeCapabilities` snapshot owner 已落到 `plugin_packages/runtime_capabilities.rs` 并由 projected manifest 消费；前端 App Center projection / detail sections 已优先消费 snapshot；App Server runtime consumer 已把 workflow-scoped skill prompt policy 接到 Agent Skills selection、session prompt 与 telemetry；MCP binding 已进入 App Server inventory `plugin_mcp_targets` prepare request skeleton；前端 API `mcpApi.executePrepareRequests(...)` 已能顺序执行 candidate import / start / listForContext；Workspace Harness 已接手动 prepare trigger 并在执行后刷新 inventory；下一刀只剩 caller-scoped 可调用证明 / GUI smoke / 自动触发，或 plugin package skill registry install path | `cargo test --manifest-path "lime-rs/Cargo.toml" -p app-server runtime_capabilities -- --nocapture`；`npx vitest run "src/features/plugin/projection/projectApp.test.ts" "src/features/plugin/schema/schemaGate.test.ts"`；`cargo test --manifest-path "lime-rs/Cargo.toml" -p app-server plugin_runtime -- --nocapture` 8 tests；`CARGO_TARGET_DIR="/tmp/lime-plugin-mcp-target" cargo test --manifest-path "lime-rs/Cargo.toml" -p app-server tool_inventory --lib -j 2` 8 tests；`npx vitest run "src/lib/api/mcp.test.ts" "src/lib/api/mcp.failClosed.test.ts"` 18 tests；`npx vitest run "src/components/agent/chat/workspace/useWorkspaceHarnessInventoryRuntime.test.tsx" "src/components/agent/chat/components/HarnessStatusPanel.tools.test.tsx" "src/lib/api/mcp.test.ts" "src/lib/api/mcp.failClosed.test.ts"` 32 tests；scoped ESLint passed；`npm run typecheck -- --pretty false` passed；后续补 caller-scoped callable proof / GUI smoke 或 plugin skill registry tests |
| P2     |   12 | Realtime / Media / Collaboration                 | `runtime-content-owner-done / llm-media-content-event-done / message-delta-content-owner-done / message-delta-content-parser-done / app-server-agent-message-contentparts-skeleton-validated / protocol-generated-skeleton-validated / projection-hydrate-skeleton-done / message-list-renderer-skeleton-validated / gui-media-smoke-validated / binary-sidecar-read-app-server-skeleton-wired / binary-sidecar-read-runtime-test-sigterm / full-binary-preview-pending` | `runtime-core/src/runtime_content.rs` 已固定 `RuntimeContentPart` / `RuntimeContentReference` / `RuntimeMediaKind` owner、`RuntimeMessageDeltaContent` payload owner 和 payload parser；`llm_protocol::runtime_event_from_llm_event(...)` 已把文本、支持 MIME 的 LLM image/audio 输出统一接成 owner-backed `message.delta` payload；App Server `thread_item_projection/agent_message.rs` 已消费 parser 并输出 `agent_message.contentParts` skeleton，media-only delta、同 `itemId` text/media 合并、alias mismatch 和 inline `data:` fail closed 均有 owner 单测；protocol/generated 已导出 `AgentThreadContentReference` / `AgentThreadMessageContentPart`；projection package、GUI hydrate/streaming sync 与 MessageList renderer skeleton 已接入；`media-reference` GUI smoke 已验证 GUI media card、Workbench preview skeleton 与 read model `contentParts`，且不暴露 inline payload；下一刀按 [p2-media-item-projection-handoff.md](./p2-media-item-projection-handoff.md) 接 binary sidecar read / full preview detail                                             | `cargo test --manifest-path "lime-rs/Cargo.toml" -p runtime-core runtime_content -- --nocapture`；`cargo test --manifest-path "lime-rs/Cargo.toml" -p runtime-core llm_ -- --nocapture`；`rustfmt --edition 2021 --check "lime-rs/crates/app-server/src/runtime/thread_item_projection/agent_message.rs"`；`cargo test --manifest-path "lime-rs/Cargo.toml" -p app-server media_only_delta_creates_agent_message_content_parts -- --nocapture` 1 test passed；`cargo test --manifest-path "lime-rs/Cargo.toml" -p app-server-protocol generated_fixture_tree_is_stable_and_canonical_json -- --nocapture` 1 test passed；`cargo test --manifest-path "lime-rs/Cargo.toml" -p app-server-protocol schema_fixtures_match_generated_output -- --nocapture` 1 test passed；`npm run check:protocol-types` 通过；GUI media Vitest 5 files / 50 tests passed；`npm run smoke:claw-chat-current-fixture -- --scenario media-reference --timeout-ms 240000 --evidence-dir "/tmp/lime-codex-media-reference-evidence" --prefix "claw-chat-current-fixture-media-reference"` passed；后续 binary sidecar read / full preview detail                                                                                                                                                          |
...
如果 App Server runtime/backend 仍是并行脏热区，下一刀应转 Plugin skill/MCP runtime consumer、Media Item projection 或 full media preview，而不是夹写 runtime 热区。
...
如果 App Server runtime/backend 仍是并行脏热区，应优先等待移交或转 Plugin skill/MCP runtime consumer、Media Item projection 或 full media preview 的已释放写集，不继续在前端 UI 里补 runtime 推断。
...
若 App Server runtime/backend 仍被并行进程持有，则优先转 Plugin skill/MCP runtime consumer、Media Item projection 或 full media preview 的已释放写集。
...
1. App Server runtime 热区释放后，接 full media preview / binary sidecar read 或 MCP runtime import。
2. Plugin / Skills / MCP 热区释放后，接 MCP runtime status / tool listing / import contract。
...
## 70. P2 Codex fifth signal safety buffering stream-ready runtime payload helper

### 70.1 目标

第 66-69 节已经分别固定 Responses event extraction、typed update、RuntimeEvent payload struct 和 source enum。本节补齐 provider stream 接线前的最后一个 owner 侧 helper：让 consumer 可以从完整 Responses event + headers + provider/model 一步拿到 `ProviderSafetyBufferingRuntimeEventPayload`，避免后续 `provider_stream.rs` 或 App Server RuntimeEvent 接线重新拼 raw provider 字段、legacy header fallback 和 RuntimeEvent 字段名。

`provider_stream.rs`、App Server runtime/backend、projection package 与 AgentChat GUI 仍是并行脏热区，本刀继续只改 `model-provider::safety` owner，不夹写 stream/runtime/read model/GUI。
...
- 新增 `parse_safety_buffering_runtime_event_payload(...)`：
  - 输入完整 Responses event、response headers、provider、model。
  - 内部委托 `parse_safety_buffering_response_event(...)`，继续由 safety owner 处理 `retry_model`、explicit null、missing fallback header 和 `safety_buffering=false`。
  - 输出 `ProviderSafetyBufferingRuntimeEventPayload` typed struct，不输出 raw `retry_model` / `faster_model` / `fasterModel`。
- 新增 `ProviderSafetyBufferingRuntimeEventPayload::to_json_value()`，让 JSON 序列化归属 typed payload 自身；`ProviderSafetyBufferingUpdate::to_runtime_event_payload(...)` 只保留为委托 helper，不再直接调用 `serde_json::to_value(...)`。
- 新增 stream-ready payload 回归：
  - object event + legacy header fallback 能产出 provider/model 归属、`retryModel` / `fallbackHeaderModel` 和 `LegacyHeader` source。
  - typed payload 自身能序列化出稳定 JSON source，且不暴露 raw `retry_model`。
  - `safety_buffering=false` 不产出 RuntimeEvent payload。
- 只读对照 Codex safety buffering 事件链后确认：Codex 会在 `safety_buffering` object 出现时发事件，`show_buffering_ui=false` 的隐藏/陈旧处理属于下游 presentation/runtime 状态机，不在 provider parser 隐式吞掉。
...
```bash
CARGO_HOME="/tmp/lime-codex-cargo-home-model-provider" CARGO_TARGET_DIR="/tmp/lime-codex-model-provider-safety-stream-payload-target" CARGO_BUILD_JOBS=4 cargo fmt --manifest-path "lime-rs/Cargo.toml" -p model-provider --check
CARGO_HOME="/tmp/lime-codex-cargo-home-model-provider" CARGO_TARGET_DIR="/tmp/lime-codex-model-provider-safety-stream-payload-target" CARGO_BUILD_JOBS=4 cargo test --manifest-path "lime-rs/Cargo.toml" -p model-provider safety -- --nocapture
CARGO_HOME="/tmp/lime-codex-cargo-home-model-provider" CARGO_TARGET_DIR="/tmp/lime-codex-model-provider-safety-stream-payload-target" CARGO_BUILD_JOBS=4 cargo test --manifest-path "lime-rs/Cargo.toml" -p model-provider -- --nocapture
```

结果：

```text
model-provider safety tests: 26 passed
model-provider full crate tests: 38 passed
```
...
1. `provider_stream.rs` 直接调用 `parse_safety_buffering_runtime_event_payload(...)`，产出 provider stream typed event。
2. App Server runtime 写入 turn-scoped RuntimeEvent，payload 直接消费 `ProviderSafetyBufferingRuntimeEventPayload` 序列化结果。
3. read model / GUI 只消费 projection，不解析 raw provider event 或 legacy `fasterModel`。

### 70.5 分类

- `current`：`parse_safety_buffering_runtime_event_payload(...)` 是 provider stream 接线前的 safety buffering RuntimeEvent payload helper，仍归属 `model-provider::safety` owner。
- `current`：`ProviderSafetyBufferingRuntimeEventPayload` / `ProviderSafetyBufferingRetryModelSource` 继续是 payload 与 source 的 typed owner。
- `current`：`ProviderSafetyBufferingRuntimeEventPayload::to_json_value()` 是 typed payload 的 JSON 序列化 owner；`ProviderSafetyBufferingUpdate::to_runtime_event_payload(...)` 仅为兼容 helper。
- `compat`：旧 safety buffering headers 只作为 provider parser 边界输入。
- `deprecated`：raw provider event 字段、`retry_model` 与 legacy `faster_model` / `fasterModel` 不进入 RuntimeEvent consumer。
- `dead`：旧 `lime-rs/src/**`、旧 Tauri wrapper、旧 `agent_runtime_*` production surface 继续 forbidden-to-restore / retired guard-only。

## 71. P2 Codex fifth signal provider stream safety buffering event contract

### 71.1 目标

第 70 节已经把完整 Responses event + headers + provider/model 到 typed payload 的投影收进 `model-provider::safety`。本节把这个 owner 接到 `provider_stream.rs` 的 current reply stream contract，产出 provider stream 侧可传递的 typed event wrapper，避免后续 runtime consumer 直接读取 raw `safety_buffering` event、`retry_model` 或 legacy header。

本刀只触碰 `model-provider` contract 层；不接 App Server RuntimeEvent storage、read model 或 GUI projection。
...
- 新增 `RuntimeReplyProviderStreamEvent`：
  - 当前包含 `SafetyBuffering(ProviderSafetyBufferingRuntimeEventPayload)`。
  - 该类型不引入 Aster 类型，也不绑定 `lime-agent` 的 runtime protocol。
- 新增 `RuntimeReplyProviderStreamEvent::safety_buffering_from_response_event(...)`：
  - 输入 `RuntimeReplyStreamRequest`、完整 Responses event 和 response headers。
  - provider / model 归属来自 current `RuntimeReplyStreamRequest` provider handle。
  - safety buffering 语义继续委托 `parse_safety_buffering_runtime_event_payload(...)`，stream contract 不重新解释 `retry_model`、explicit null、missing fallback header 或 `safety_buffering=false`。
- 新增 `runtime_event_kind()` 与 `payload_json_value()`，让下一层 runtime consumer 有窄出口，不需要碰 typed payload 内部字段或 raw provider wire。
- 新增 provider stream 单测：
  - object event + legacy header fallback 能产出 `RuntimeReplyProviderStreamEvent::SafetyBuffering`。
  - `safety_buffering=false` 不产出 stream event。
  - JSON payload 不暴露 raw `retry_model` / `faster_model`。
...
```bash
CARGO_HOME="/tmp/lime-codex-cargo-home-model-provider" CARGO_TARGET_DIR="/tmp/lime-codex-model-provider-provider-stream-safety-target" CARGO_BUILD_JOBS=4 cargo fmt --manifest-path "lime-rs/Cargo.toml" -p model-provider --check
CARGO_HOME="/tmp/lime-codex-cargo-home-model-provider" CARGO_TARGET_DIR="/tmp/lime-codex-model-provider-provider-stream-safety-target" CARGO_BUILD_JOBS=4 cargo test --manifest-path "lime-rs/Cargo.toml" -p model-provider provider_stream -- --nocapture
CARGO_HOME="/tmp/lime-codex-cargo-home-model-provider" CARGO_TARGET_DIR="/tmp/lime-codex-model-provider-provider-stream-safety-target" CARGO_BUILD_JOBS=4 cargo test --manifest-path "lime-rs/Cargo.toml" -p model-provider safety -- --nocapture
CARGO_HOME="/tmp/lime-codex-cargo-home-model-provider" CARGO_TARGET_DIR="/tmp/lime-codex-model-provider-provider-stream-safety-target" CARGO_BUILD_JOBS=4 cargo test --manifest-path "lime-rs/Cargo.toml" -p model-provider -- --nocapture
```

结果：

```text
model-provider provider_stream tests: 9 passed
model-provider safety-filter tests: 28 passed
model-provider full crate tests: 40 passed
```
...
1. `lime-rs/crates/agent/**` 或 App Server runtime 持有者接 `RuntimeReplyProviderStreamEvent::SafetyBuffering`，写入 turn-scoped RuntimeEvent。
2. App Server runtime/read model 绑定 session/thread/turn/provider/model 归属，不直通 raw provider payload。
3. GUI 只消费 read model / projection，不解析 raw provider event 或 legacy `fasterModel`。

### 71.5 分类

- `current`：`RuntimeReplyProviderStreamEvent::SafetyBuffering` 是 provider reply stream 的 safety buffering typed event contract。
- `current`：`parse_safety_buffering_runtime_event_payload(...)` / `ProviderSafetyBufferingRuntimeEventPayload` 仍是 wire parser 与 RuntimeEvent payload owner。
- `compat`：旧 safety buffering headers 只作为 provider parser 边界输入。
- `deprecated`：raw provider event 字段、`retry_model` 与 legacy `faster_model` / `fasterModel` 不进入 RuntimeEvent consumer。
- `dead`：旧 `lime-rs/src/**`、旧 Tauri wrapper、旧 `agent_runtime_*` production surface 继续 forbidden-to-restore / retired guard-only。

## 72. P2 Codex fifth signal provider stream runtime event envelope

### 72.1 目标

第 71 节已把 safety buffering typed event 放进 `model-provider::provider_stream` contract。本节把该 typed event 接到 reply stream envelope、`lime-agent` DTO 和 App Server RuntimeEvent mapper，先封住 provider event -> RuntimeEvent 的 current 合同，避免后续 read model / GUI 为了展示 safety buffering 再直接解析 provider raw payload。

本刀不捕获真实 provider SSE / Responses event，也不接 read model / GUI presentation；这些仍是下一刀主缺口。
...
- `agent-runtime::RuntimeReplyStreamEvent<E>` 新增 `ProviderStreamEvent(RuntimeReplyProviderStreamEvent)`，让 provider stream typed event 不必伪装成普通 Aster / agent event。
- `lime-agent::AgentEvent` 新增 `ProviderStreamEvent { runtime_event_kind, payload }`，payload 由 `RuntimeReplyProviderStreamEvent::payload_json_value()` 生成，不重新解释 `retry_model`、legacy header、`faster_model` 或 `fasterModel`。
- `stream_agent_reply_once(...)` 在 reply stream policy 中把 provider stream event 投成 `AgentEvent::ProviderStreamEvent`，再沿现有 `on_event` 回调进入 App Server backend。
- App Server `runtime_backend::tool_events` 对 `ProviderStreamEvent` 特判，并用 allowlist 输出 RuntimeEvent kind；safety buffering 当前会写成 `provider_safety_buffering` RuntimeEvent，未知 provider stream kind fail-closed 到 `runtime.event`，payload 中保留 `backend=runtime` 与 `runtimeEvent` 诊断副本。
...
CARGO_HOME="/tmp/lime-codex-cargo-home-model-provider" CARGO_TARGET_DIR="/tmp/lime-codex-provider-stream-runtime-event-target" CARGO_BUILD_JOBS=4 cargo test --manifest-path "lime-rs/Cargo.toml" -p agent-runtime reply_stream -- --nocapture
CARGO_HOME="/tmp/lime-codex-cargo-home-model-provider" CARGO_TARGET_DIR="/tmp/lime-codex-provider-stream-runtime-event-target" CARGO_BUILD_JOBS=4 cargo test --manifest-path "lime-rs/Cargo.toml" -p lime-agent provider_stream_reply_event_projects_agent_event -- --nocapture
CARGO_HOME="/tmp/lime-codex-cargo-home-model-provider" CARGO_TARGET_DIR="/tmp/lime-codex-provider-stream-runtime-event-target" CARGO_BUILD_JOBS=4 cargo test --manifest-path "lime-rs/Cargo.toml" -p lime-agent agent_event_provider_stream_event_serializes_current_payload -- --nocapture
CARGO_HOME="/tmp/lime-codex-cargo-home-model-provider" CARGO_TARGET_DIR="/tmp/lime-codex-provider-stream-runtime-event-target" CARGO_BUILD_JOBS=4 cargo test --manifest-path "lime-rs/Cargo.toml" -p app-server provider_stream_event -- --nocapture
```

结果：

```text
agent-runtime reply_stream tests: 3 passed
lime-agent reply stream bridge test: 1 passed
lime-agent provider stream DTO test: 1 passed
app-server provider stream RuntimeEvent mapper tests: 2 passed
```

Cargo home 与 target 均在 `/tmp` 下；没有使用用户 cache 目录。`lime-agent` 定向测试仍有既有 `agent_tools/execution/tests.rs` 的 `WorkspaceToolSurface` unused import warning，不属于本刀写集。

### 72.4 下一刀

下一步回到 runtime 主链，按增量排序：

1. 在 provider current / compat stream 边界捕获真实 SSE / Responses safety buffering event，调用 `RuntimeReplyProviderStreamEvent::safety_buffering_from_response_event(...)`，而不是在 App Server 或 GUI 侧解析 raw provider JSON。
2. App Server read model 把 `provider_safety_buffering` 绑定 session/thread/turn/provider/model，投成 diagnostic / metadata，不污染 assistant text。
3. GUI 只消费 read model / projection 展示 buffering / retry model，不解析 raw `retry_model` 或 legacy header。

### 72.5 分类

- `current`：`RuntimeReplyStreamEvent::ProviderStreamEvent` 是 provider reply stream typed event 的 current envelope。
- `current`：`AgentEvent::ProviderStreamEvent` 是 `lime-agent` 到 App Server backend 的 DTO 边界。
- `current`：App Server `provider_safety_buffering` RuntimeEvent mapper 是 RuntimeEvent projection 入口。
- `compat`：Aster compat provider backend 仍是当前实际 provider 执行 adapter；它只能承接捕获 typed provider event 的迁移边界，不得重新解释 safety buffering wire 语义。
- `deprecated`：raw provider `retry_model`、legacy `faster_model` / `fasterModel` 不进入 App Server / GUI consumer。
- `dead`：旧 `lime-rs/src/**`、旧 Tauri wrapper、旧 `agent_runtime_*` production surface 继续 forbidden-to-restore / retired guard-only。

## 73. P2 Codex fifth signal OpenAI Responses safety buffering SSE capture

### 73.1 目标

第 72 节已经把 provider stream typed event 接到 RuntimeEvent envelope 与 App Server mapper。本节补齐下一条主缺口：在当前实际 provider 执行边界 Aster compat / OpenAI Responses stream 中捕获 raw SSE `safety_buffering`，并把它投到已有 typed provider stream event，不让 App Server、read model 或 GUI 重新解析 raw provider JSON。

本刀只接 provider SSE capture 与 Lime adapter 投影；不接 safety buffering read model / GUI projection，不修改 `model-provider::safety` owner，不碰前端 hook 热区。
...
- Aster OpenAI Responses parser 在 raw SSE JSON 顶层 `safety_buffering` 为 object 时发出 invisible provider stream sentinel；`safety_buffering=false` 不发 sentinel。
- sentinel 使用 `SystemNotification + MessageMetadata::invisible()`，只作为 Aster compat stream 内部桥接，不新增 provider trait，也不新增对话可见 message content variant。
- OpenAI Responses provider 建立 stream 时提取 `x-codex-safety-buffering-enabled` 与 `x-codex-safety-buffering-faster-model` headers，作为 legacy header fallback 输入传入 parser。
- Aster agent loop 识别 sentinel 后只向外 yield `AgentEvent::Message(response)` 并 `continue`，避免进入 conversation history / `messages_to_add`。
- Lime Aster reply adapter 消费 sentinel，调用既有 `RuntimeReplyProviderStreamEvent::safety_buffering_from_response_event(...)`，输出 `RuntimeReplyStreamEvent::ProviderStreamEvent`，继续由第 72 节 RuntimeEvent envelope / App Server mapper 接手。
...
```bash
TMPDIR="/tmp" CARGO_HOME="/tmp/lime-codex-cargo-home-provider-stream-capture" CARGO_TARGET_DIR="/tmp/lime-codex-provider-stream-capture-target" CARGO_INCREMENTAL=0 CARGO_PROFILE_DEV_DEBUG=0 CARGO_BUILD_JOBS=4 cargo check --manifest-path "lime-rs/vendor/aster-rust/crates/aster/Cargo.toml" -p aster-core
TMPDIR="/tmp" CARGO_HOME="/tmp/lime-codex-cargo-home-provider-stream-capture" CARGO_TARGET_DIR="/tmp/lime-codex-provider-stream-capture-target" CARGO_INCREMENTAL=0 CARGO_PROFILE_DEV_DEBUG=0 CARGO_BUILD_JOBS=4 cargo check --manifest-path "lime-rs/Cargo.toml" -p lime-agent
TMPDIR="/tmp" CARGO_HOME="/tmp/lime-codex-cargo-home-provider-stream-capture" CARGO_TARGET_DIR="/tmp/lime-codex-provider-stream-capture-target" CARGO_INCREMENTAL=0 CARGO_PROFILE_TEST_DEBUG=0 CARGO_BUILD_JOBS=4 cargo test --manifest-path "lime-rs/vendor/aster-rust/crates/aster/Cargo.toml" -p aster-core --lib test_responses_streaming_safety_buffering_emits_provider_stream_notification -- --nocapture
TMPDIR="/tmp" CARGO_HOME="/tmp/lime-codex-cargo-home-provider-stream-capture" CARGO_TARGET_DIR="/tmp/lime-codex-provider-stream-capture-target" CARGO_INCREMENTAL=0 CARGO_PROFILE_TEST_DEBUG=0 CARGO_BUILD_JOBS=4 cargo test --manifest-path "lime-rs/Cargo.toml" -p lime-agent --lib provider_stream_notification_projects_safety_buffering_event -- --nocapture
cargo fmt --manifest-path "lime-rs/vendor/aster-rust/crates/aster/Cargo.toml"
cargo fmt --manifest-path "lime-rs/Cargo.toml" -p lime-agent
git diff --check -- "lime-rs/vendor/aster-rust/crates/aster/src/providers/formats/openai_responses.rs" "lime-rs/vendor/aster-rust/crates/aster/src/providers/openai.rs" "lime-rs/vendor/aster-rust/crates/aster/src/agents/agent.rs" "lime-rs/crates/agent/src/request_tool_policy/aster_reply_adapter.rs" "lime-rs/crates/agent/src/request_tool_policy/aster_reply_adapter/tests.rs"
```
...
```text
aster-core check: passed，1 个既有 unused_mut warning
lime-agent check: passed
aster-core safety buffering SSE unit: 1 passed
lime-agent sentinel projection unit: 1 passed
```

第一次未加 `--lib` 的 vendor test 曾在链接无关 integration test 时因 `/tmp` 空间不足失败；随后清理本轮 `/tmp/lime-codex-provider-stream-capture-target` 临时 target，释放 10.9GiB，并用 `CARGO_INCREMENTAL=0` / 低 debug 重跑通过。Cargo home 与 target 均在 `/tmp` 下；没有使用 `/Users/coso/Library/Caches`。

### 73.4 下一刀

主线剩余缺口按增量排序：

1. App Server read model 把 `provider_safety_buffering` 绑定 session/thread/turn/provider/model，投成 diagnostic / metadata，不污染 assistant text。
2. GUI 只消费 read model / projection 展示 buffering / retry model，不解析 raw `retry_model` 或 legacy header。
3. 如果后续替换 Aster compat provider backend，必须把本刀 sentinel 边界下沉到新的 current provider stream parser，仍输出同一个 `RuntimeReplyProviderStreamEvent::SafetyBuffering`。

### 73.5 分类

- `current`：`RuntimeReplyProviderStreamEvent::SafetyBuffering`、`RuntimeReplyStreamEvent::ProviderStreamEvent`、`AgentEvent::ProviderStreamEvent` 与 App Server `provider_safety_buffering` RuntimeEvent mapper 继续是主链 typed owner。
- `compat`：Aster OpenAI Responses sentinel 是当前 Aster compat provider backend 的 stream bridge，只允许捕获并转发 typed provider event，不承接新业务语义。
- `compat`：旧 safety buffering headers 只作为 provider parser 边界输入。
- `deprecated`：raw provider `retry_model`、legacy `faster_model` / `fasterModel` 不进入 App Server / read model / GUI consumer。
- `dead`：旧 `lime-rs/src/**`、旧 Tauri wrapper、旧 `agent_runtime_*` production surface 继续 forbidden-to-restore / retired guard-only。

## 74. P2 Codex fifth signal safety buffering read model projection

### 74.1 目标

第 73 节已把真实 OpenAI Responses `safety_buffering` SSE 经 Aster compat bridge 投到 App Server `provider_safety_buffering` RuntimeEvent。本节把该 RuntimeEvent 接到 App Server read model，让 GUI 后续只消费 `thread_read.diagnostics` / `runtime_summary` projection，不再解析 raw provider event、legacy header 或 Aster sentinel。

本刀只接 App Server read model 与前端 thin type；不碰 `vendor/aster-rust/**`，不改 `lime-agent` / `model-provider` 已有 owner，不接 AgentChat GUI presentation。
...
lime-rs/crates/app-server/src/runtime/tests/read_model/messages_diagnostics.rs
...
- 新增 `provider_safety_buffering_count`，统计 session event log 中的 `provider_safety_buffering` RuntimeEvent。
- 新增 `latest_provider_safety_buffering` diagnostic sample，绑定 `source_event_id`、`thread_id`、`turn_id`、timestamp、provider、model、use_cases、reasons、show_buffering_ui、retry_model、fallback_header_model、source、backend。
- 同步输出 `runtime_summary.latestProviderSafetyBuffering`，供 GUI 下一刀从 read model 读取。
- diagnostic sample 不透传 raw `runtimeEvent`，也不暴露 raw `retryModel` / legacy `fasterModel` 字段；前端类型只声明 read-model projection。
...
```bash
TMPDIR="/tmp" CARGO_HOME="/tmp/lime-codex-cargo-home-app-server-read-model" CARGO_TARGET_DIR="/tmp/lime-codex-app-server-safety-read-model-target" CARGO_INCREMENTAL=0 CARGO_PROFILE_TEST_DEBUG=0 CARGO_BUILD_JOBS=4 cargo test --manifest-path "lime-rs/Cargo.toml" -p app-server read_session_projects_provider_safety_buffering_into_diagnostics -- --nocapture
cargo fmt --manifest-path "lime-rs/Cargo.toml" -p app-server -- --check
git diff --check -- "lime-rs/crates/app-server/src/runtime/read_model.rs" "lime-rs/crates/app-server/src/runtime/tests/read_model/messages_diagnostics.rs" "src/lib/api/agentRuntime/types.ts" "src/lib/api/agentRuntime/types.d.ts"
```

结果：

```text
app-server read model safety buffering test: 1 passed
app-server fmt check: passed
scoped diff check: passed
```

`npm run test:related -- "src/lib/api/agentRuntime/types.ts" "src/lib/api/agentRuntime/types.d.ts"` 在当前工作树失败 5 个 GUI 测试，落在 `AgentChatPage` bootstrap / preview 与 `StreamingRenderer` 缓存/首帧热区；这些不属于本刀 read-model / 类型写集。下一刀接 GUI presentation 前必须先让该热区收口或明确移交。

### 74.4 下一刀

下一步只剩 presentation：

1. GUI 消费 `thread_read.diagnostics.latest_provider_safety_buffering` 或 `runtime_summary.latestProviderSafetyBuffering` 展示 buffering / retry model。
2. GUI 不解析 raw provider payload、不读取 Aster sentinel、不猜 legacy `fasterModel`。
3. 若后续替换 Aster compat provider backend，仍保持同一个 `RuntimeReplyProviderStreamEvent::SafetyBuffering -> provider_safety_buffering -> read model` 合同。

### 74.5 分类

- `current`：App Server read model diagnostics / runtime summary 是 safety buffering GUI 的唯一 projection owner。
- `current`：前端 `AgentRuntimeDiagnosticProviderSafetyBufferingSample` 只是 read-model thin type。
- `compat`：Aster OpenAI Responses sentinel 仍只是当前 provider compat bridge，不能承接 GUI 或 read-model 语义。
- `deprecated`：raw provider `retry_model`、legacy `faster_model` / `fasterModel` 不进入 GUI consumer。
- `dead`：旧 `lime-rs/src/**`、旧 Tauri wrapper、旧 `agent_runtime_*` production surface 继续 forbidden-to-restore / retired guard-only。

## 75. P2 Codex fifth signal safety buffering GUI projection

### 75.1 目标

第 74 节已把 `provider_safety_buffering` RuntimeEvent 投到 App Server read model。本节关闭 presentation 缺口：可靠性面板和复制诊断包只消费 `thread_read.diagnostics.latest_provider_safety_buffering`，不读取 raw provider event、Aster sentinel、`runtimeEvent` 诊断副本或 legacy `fasterModel`。
...
src/components/agent/chat/components/AgentThreadReliabilityPanel.memory.test.tsx
src/i18n/resources/{zh-CN,zh-TW,en-US,ja-JP,ko-KR}/agentRuntime.json
```

实现事实：

- 新增 `AgentThreadProviderSafetyBufferingCard`，展示 provider/model、retry model、fallback header model、use cases、reasons、source 与 buffering UI 状态。
- `AgentThreadReliabilityPanel` 只把 `threadRead` 传给该卡片；前端不解析 provider wire。
- `formatProviderSafetyBufferingDiagnostic(...)` 为复制诊断包提供纯 formatter，输入类型是 read model thin type `AgentRuntimeDiagnosticProviderSafetyBufferingSample`。
- 五语言 `agentRuntime.json` 覆盖新增用户可见文案。
...
```bash
XDG_CACHE_HOME="/tmp/lime-codex-node-cache-provider-safety-projection" npm test -- "src/components/agent/chat/components/AgentThreadReliabilityPanel.test.tsx" "src/components/agent/chat/components/AgentThreadReliabilityPanel.memory.test.tsx"
CARGO_HOME="/tmp/lime-codex-cargo-home-provider-safety-projection" CARGO_TARGET_DIR="/tmp/lime-codex-provider-safety-projection-target" CARGO_INCREMENTAL=0 CARGO_PROFILE_DEV_DEBUG=0 CARGO_PROFILE_TEST_DEBUG=0 CARGO_BUILD_JOBS=4 cargo test --manifest-path "lime-rs/Cargo.toml" -p app-server provider_safety_buffering -- --nocapture
TMPDIR="/tmp" XDG_CACHE_HOME="/tmp/lime-codex-node-cache-provider-safety-projection" npm_config_cache="/tmp/lime-codex-npm-cache-provider-safety-projection" CARGO_HOME="/tmp/lime-codex-cargo-home-provider-safety-projection" CARGO_TARGET_DIR="/tmp/lime-codex-provider-safety-projection-target" CARGO_INCREMENTAL=0 CARGO_PROFILE_DEV_DEBUG=0 CARGO_PROFILE_TEST_DEBUG=0 CARGO_BUILD_JOBS=4 npm run smoke:agent-runtime-current-fixture
TMPDIR="/tmp" XDG_CACHE_HOME="/tmp/lime-codex-node-cache-inputbar-restore" npm_config_cache="/tmp/lime-codex-npm-cache-inputbar-restore" CARGO_HOME="/tmp/lime-codex-cargo-home-inputbar-restore" CARGO_TARGET_DIR="/tmp/lime-codex-inputbar-restore-target" CARGO_INCREMENTAL=0 CARGO_PROFILE_DEV_DEBUG=0 CARGO_PROFILE_TEST_DEBUG=0 CARGO_BUILD_JOBS=2 npm run smoke:claw-chat-current-fixture -- --scenario inputbar-rich-restore --prefix claw-chat-current-fixture-inputbar-rich-restore-codex --timeout-ms 180000
TMPDIR="/tmp" XDG_CACHE_HOME="/tmp/lime-codex-node-cache-agent-runtime-fixture" npm_config_cache="/tmp/lime-codex-npm-cache-agent-runtime-fixture" CARGO_HOME="/tmp/lime-codex-cargo-home-agent-runtime-fixture" CARGO_TARGET_DIR="/tmp/lime-codex-agent-runtime-fixture-target" CARGO_INCREMENTAL=0 CARGO_PROFILE_DEV_DEBUG=0 CARGO_PROFILE_TEST_DEBUG=0 CARGO_BUILD_JOBS=2 npm run smoke:agent-runtime-current-fixture
TMPDIR="/tmp" XDG_CACHE_HOME="/tmp/lime-codex-node-cache-gui-smoke" npm_config_cache="/tmp/lime-codex-npm-cache-gui-smoke" CARGO_HOME="/tmp/lime-codex-cargo-home-agent-runtime-fixture" CARGO_TARGET_DIR="/tmp/lime-codex-agent-runtime-fixture-target" CARGO_INCREMENTAL=0 CARGO_PROFILE_DEV_DEBUG=0 CARGO_PROFILE_TEST_DEBUG=0 CARGO_BUILD_JOBS=2 npm run verify:gui-smoke
```

结果：

```text
AgentThreadReliabilityPanel tests: 18 passed
app-server provider_safety_buffering filter: 1 matching read model test passed
inputbar-rich-restore single Electron fixture: passed; text/image/path/skill restored after output-free cancel
smoke:agent-runtime-current-fixture: passed; liveProviderUsed=false
verify:gui-smoke: passed; renderer loaded / app-server initialized / Claw shell ready / memory settings ready
```

历史红灯细节：首次 `smoke:agent-runtime-current-fixture` 失败在 `Inputbar rich restore` 场景，取消后富草稿未恢复，textarea 为空，path/image/skill restore 均为 false。第 75.6 节已把该热区转绿；当前事实源以单场景 `claw-chat-current-fixture-inputbar-rich-restore-codex-summary.json` 和聚合 current fixture 通过为准。

### 75.4 下一刀

1. 回到 P2 Context / Token / Compaction 主线，优先补 binary sidecar read / full preview detail，或接 MCP runtime import；Evidence export consumer、session compaction sidecar source、compact item、memory regression、media input reference skeleton、media-reference GUI smoke 与 Plugin skill prompt runtime consumer 已完成，下一刀不要重复做 compaction、plugin prompt skeleton 或 GUI media smoke。
2. 若继续第五信号，只做 `configWarning` thread/session 级 contextual diagnostic 或 evidence polish；不得补不存在的 `.rules` reader。
3. 如继续精修可靠性面板，优先拆 `AgentThreadReliabilityPanel.tsx` / `threadReliabilityDiagnosticText.ts` 这两个接近 `1000` 行的文件，不再向主文件追加新业务逻辑。

### 75.5 分类

- `current`：`provider_safety_buffering` RuntimeEvent -> App Server read model diagnostics -> reliability panel / diagnostic copy 是唯一 presentation 主链。
- `compat`：Aster OpenAI Responses sentinel 仍只是 provider compat bridge，不进入 GUI。
- `deprecated`：raw `retry_model`、legacy `faster_model` / `fasterModel` 不进入 GUI consumer。
- `dead`：旧 `lime-rs/src/**`、旧 `agent_runtime_*` production surface 继续 forbidden-to-restore。

### 75.6 Inputbar rich restore 阻塞审计

本节继续第 75.4 的 current fixture 阻塞定位。2026-07-07 本进程已从只读审计切到窄写集接管，目标是解除 `Claw Inputbar rich draft restore output-free cancel Electron fixture`，不在 `lime-rs/vendor/aster-rust/**` 继续补业务逻辑。
...
lime-rs/crates/agent/**
src/components/agent/chat/components/AgentThreadReliabilityPanel*
src/components/agent/chat/utils/providerSafetyBufferingDiagnostic.ts
```

修复事实：

- `useAgentStream` 在发送开始时保存 `submittedDraftFallbackRef`，`stopSending` 把 fallback 传给 `stopActiveAgentStream`；active stream 尚未写入 `submittedDraft` 时，output-free cancel 仍能恢复最近提交的富草稿。
- `agentStreamFlowControl` 的 restore plan 消费 `activeStream.submittedDraft ?? submittedDraftFallback`，并保留 debug evidence：`AgentStream.inputRestorePlan` / `AgentStream.inputRestoreDispatch`。
- `Inputbar` restore request 进入 `useInputbarController` 后递增 `inputRestoreEpochRef`，再异步应用 `setInput / replacePendingImages / pathReferences / activeCapability`；`useInputbarSend` 在发送开始记录 epoch，发送 Promise resolve 后若 epoch 已变化则跳过旧的 `clearPendingImages / clearPathReferences / clearActiveCapability`。
- `useInputbarSend` 的 `inputRestoreDraft` 改为发送瞬间的图片 / path reference 独立快照，避免后续数组引用被清理状态污染。
- fixture 断言已同步 current skill route 行为：rich restore turnStart 允许 `/capability-report ...` 前缀包含匹配；cancel 断言改为比较同一 rich restore `turnStart` 的 `sessionId / turnId`，不再依赖模块常量精确匹配。

验证：

```bash
npm test -- "src/components/agent/chat/components/Inputbar/hooks/useInputbarSend.test.tsx" "src/components/agent/chat/components/Inputbar/index.test.tsx" "src/components/agent/chat/hooks/agentStreamFlowControl.test.ts" "src/components/agent/chat/hooks/agentStreamInputRestorePolicy.unit.test.ts"
npm test -- "scripts/agent-runtime/claw-chat-current-fixture-smoke.test.mjs"
...
```text
Inputbar / stream restore regression: 4 files, 113 tests passed
claw-chat-current fixture script tests: 27 tests passed
renderer electron build: passed, built in 3m25s
```

Electron fixture evidence：

- `claw-chat-current-fixture-summary.json`（2026-07-07 07:34）已完整跑到 `inputbar-rich-restore`：backend ledger 有 `turnStart` + `turnCancel`，turnStart input 为 `/capability-report 请结合这个截图、文件和 Capability Report 技能，先不要输出正文。`，`inputSummary.imageAttachmentCount=1`、`fileReferenceCount=1`；停止后 GUI `textareaValue` 恢复为原始 prompt，`pathRestored=true`、`imageRestored=true`、`skillRestored=true`，assistant 仅显示 `"(已停止)"`。
- 该轮失败原因是当时尚未修正的 `inputbarRichRestoreBackendCanceled` 断言；产品恢复链已 green。
- 后续单独复跑 `claw-chat-current-fixture-inputbar-rich-restore-solo` 未进入场景，失败在 `Electron renderer / App Server bridge 未就绪`：summary 只停在 native startup shell，`initialize=null`、backend ledger 空。这是 Electron 启动 / bridge ready 阻塞，不能作为富草稿恢复失败证据。
- 当前进程复跑 `claw-chat-current-fixture-inputbar-rich-restore-codex-summary.json`：`ok=true`，backend `inputSummary.imageAttachmentCount=1`、`fileReferenceCount=1`，read model `latestTurnStatus=canceled`，GUI `textareaValue`、`pathRestored`、`imageRestored`、`skillRestored`、`noVisibleAssistantOutput` 均通过。
- 当前进程复跑 `npm run smoke:agent-runtime-current-fixture`：passed，`liveProviderUsed=false`；聚合门禁内的 `claw-chat-current-fixture-inputbar-rich-restore-regression-summary.json` 同样 `ok=true`。
- 本轮最终聚合复核：`npm run smoke:agent-runtime-current-fixture` 进程已自然退出；`.lime/qc/gui-evidence/code-artifact-workbench-electron-fixture/code-artifact-workbench-gui-coding-input-regression-summary.json` 与 `.lime/qc/gui-evidence/claw-chat-current-fixture/*-regression-summary.json` 中脚本覆盖的 12 个 Claw Electron 场景均为 2026-07-07 08:14-08:18 新产物，全部 `ok=true`、common assertions 全 true、scenario assertions 全 true、`liveProviderUsed=false`。其中 `claw-chat-current-fixture-inputbar-rich-restore-regression-summary.json` 时间为 08:15:38，证明富草稿恢复修复已进入聚合门禁。

退出条件：

1. 单场景 `inputbar-rich-restore` 已通过，缓存 / Cargo home / Cargo target 均显式落 `/tmp`。
2. 聚合 `npm run smoke:agent-runtime-current-fixture` 已通过，缓存 / Cargo home / Cargo target 均显式落 `/tmp`。
3. `npm run verify:gui-smoke` 已通过，缓存 / Cargo home / Cargo target 均显式落 `/tmp`。
4. 第 75 节状态从 `fixture-smoke-red-inputbar-hotzone` 收口到 `fixture-smoke-green`；后续不再把 Inputbar rich restore 当作 provider safety buffering GUI projection blocker。
...
### 57.7 Context Evidence export / sidecar summary consumer

本节继续 57.5 的第二个分支，接管 `evidence_provider.rs` / `exports.rs` / evidence export tests 的窄写集。主线收益：`ContextFragmentEnvelope` 与 `context_packet_telemetry` 不再只停在 prompt telemetry，Evidence Pack 已能审计 context fragment、budget decision 与 sidecar reference，而不是让 review / replay 重新解析 prompt 文本。

本轮写集：

```text
lime-rs/crates/app-server/src/runtime/evidence_provider.rs
lime-rs/crates/app-server/src/runtime/evidence_provider/context.rs
lime-rs/crates/app-server/src/runtime/exports.rs
lime-rs/crates/app-server/src/runtime/tests/evidence_exports.rs
lime-rs/crates/app-server/src/runtime/tests/evidence_exports/context_summary.rs
internal/research/refactor/v1/priority-tracking-plan.md
internal/research/refactor/v1/p2-runtime-skeleton.md
internal/research/refactor/v1/quality-fixture-matrix.md
internal/research/refactor/v1/completion-audit.md
...
- `EvidencePackRequest` 新增 `turn_runtime_metadata`，`RuntimeCore::export_evidence(...)` 从 `stored.turn_runtime_options` 收集指定 turn 的 metadata 后交给 evidence provider。
- `evidence_provider::context` 只读取 `context_packet_telemetry` / `contextPacketTelemetry` 与 legacy prompt context 包裹中的 telemetry，不重新拼 prompt，也不读取 Aster vendor。
- `observability_summary.context` 输出 `context-evidence-summary.v1`，包含 `packetCount`、admitted/rejected、`fragmentCount`、`budgetStatusBreakdown`、source breakdown、source turn ids 与 sidecar reference count。
- `sidecarReferences` 只保留 `kind`、`uri` 和 `sha256Present`，不导出 sidecar hash 值；`model_visible_preview` 只计数为 `modelPreviewRedactedCount`，不进入 Evidence Pack 正文。

验证：

```bash
cargo test --manifest-path "lime-rs/Cargo.toml" -p app-server export_evidence_pack_includes_context_fragment_summary_from_turn_metadata -- --nocapture
```

结果：

```text
test result: ok. 1 passed; 0 failed; 0 ignored; 0 measured; 835 filtered out
```

说明：首次尝试复用 `/tmp/lime-codex-context-evidence-target` 复跑时被临时 target 竞争破坏，失败点是 Cargo fingerprint / `aws-lc-sys` 构建产物 `No such file or directory`，不是断言失败；本轮按用户要求不把仓库主 `lime-rs/target` 或系统 cache 目录作为新增证据来源，最新验证口径回退到文件级 `rustfmt` 与 scoped `git diff --check`，重型 Rust 回归留到细节轮在 `/tmp` target 下补跑。

后续缺口校准：

1. 本节记录时 `session_compaction_prompt_context` sidecar source 与 `ContextCompaction item` 仍 pending；该缺口已由 57.8 关闭。
2. memory summary sidecarRef 已进入 `ContextPacket.metadata` 骨架；`media-reference` GUI smoke 缺口已由第 77 节关闭，binary sidecar read / full preview detail 仍需继续。
3. Aster reply loop / root `aster` dependency 无变化；本刀是 App Server Evidence/read model current consumer，不是 Phase 6 Aster dependency 删除。

分类：

- `current`：App Server `evidence/export` 的 context evidence summary，属于 Turn runtime metadata -> Item/Evidence projection。
- `compat`：Aster vendor / reply loop 不参与本刀，只保持既有受控 executor / adapter。
- `deprecated`：无新增。
- `dead`：不恢复旧 `agent_runtime_*` evidence writer，不恢复 Aster context / memory vendor pipeline。

### 57.8 Session compaction sidecar source / compact item

本节继续 57.7 的剩余缺口，接管 `memory_prompt.rs` 与 `sessions.rs` 的窄写集。主线收益：Codex 的 context compaction 不只是 prompt 文本或 Evidence 摘要，Lime 现在把 session compaction 的真实 sidecar source 带回 next-turn `ContextPacket`，并把 compaction lifecycle materialize 成 read model Item。
...
- `session_compaction_packet_from_prompt_context(...)` 透传 `sidecarRef` 到 `ContextPacket` metadata，随后由 `context_packet.rs` 转成 `ContextSidecarReference`。
- `compact_agent_session_injects_next_turn_session_context_packet` 使用真实 `SidecarStore`，断言 `session_compaction_prompt_context.sidecarRef.kind/sha256` 存在，并断言 telemetry `fragmentEnvelope.sidecar_reference` 存在。
- `compact_agent_session_writes_session_context_artifact` 额外断言 read model `items` 中存在 `type=context_compaction`、`status=completed` 的独立 Item。
- 本刀不改 Aster vendor，不在 evidence provider 中重新拼 context 策略；第 77 节已补 GUI media smoke 证据，但仍不声称 binary sidecar read / full preview detail 已完成。

验证：

```bash
cargo test --manifest-path "lime-rs/Cargo.toml" -p app-server compact_agent_session_writes_session_context_artifact -- --nocapture
cargo test --manifest-path "lime-rs/Cargo.toml" -p app-server compact_agent_session_injects_next_turn_session_context_packet -- --nocapture
rustfmt --edition 2021 --check "lime-rs/crates/app-server/src/runtime/memory_prompt.rs" "lime-rs/crates/app-server/src/runtime/tests/sessions.rs"
git diff --check -- "lime-rs/crates/app-server/src/runtime/memory_prompt.rs" "lime-rs/crates/app-server/src/runtime/tests/sessions.rs"
```

结果：

```text
compact_agent_session_writes_session_context_artifact: 1 passed
compact_agent_session_injects_next_turn_session_context_packet: 1 passed
file-level rustfmt: passed
scoped git diff --check: passed
```

说明：首次 `cargo fmt --manifest-path "lime-rs/Cargo.toml" -p app-server -- --check` 被并行脏文件 `runtime/projection_store.rs` 的格式差异阻断；本轮未触碰该并行文件，改用文件级 `rustfmt --check` 验证自身写集。

剩余缺口：

1. memory summary sidecarRef 已进入 `ContextPacket.metadata` 骨架；`media-reference` GUI smoke 缺口已由第 77 节关闭，binary sidecar read / full preview detail 仍需继续。
2. Aster reply loop / root `aster` dependency 无变化；本刀是 App Server Context / Item current consumer，不是 Phase 6 Aster dependency 删除。

分类：

- `current`：App Server session compaction sidecar source、`ContextPacket` sidecar metadata、read model `context_compaction` Item。
- `compat`：Aster vendor / reply loop 不参与本刀，只保持既有受控 executor / adapter。
- `deprecated`：无新增。
- `dead`：不恢复旧 `agent_runtime_*` compaction/evidence writer，不恢复 Aster context / memory vendor pipeline。

### 57.9 Media input reference context skeleton

本节继续 57.8 的剩余缺口，只收“media input reference -> ContextPacket telemetry”骨架，不接 Workbench 预览、不读取二进制、不改 GUI。主线收益：多模态输入先进入统一 Context / Token / Compaction 证据链，避免 media attachment 绕过 budget / evidence，后续再补完整 media preview。

本轮写集：

```text
lime-rs/crates/app-server/src/runtime/context_media.rs
lime-rs/crates/app-server/src/runtime/tests/media_context.rs
lime-rs/crates/app-server/src/runtime/context_packet.rs
lime-rs/crates/app-server/src/runtime/memory_prompt.rs
lime-rs/crates/app-server/src/runtime/turn_execution.rs
...
- `ContextPacketKind` 新增 `media_reference`，`ContextSource` 新增 `media_input`。
- `ContextPacket::media_reference(...)` 只保存 reference metadata，不读取媒体文件、不解析 provider payload。
- `RuntimeCore::prepare_media_prompt_context(...)` 从 `AgentInput.attachments` 读取非 inline `uri`，生成 `media_prompt_context` 与 `context_packet_telemetry`。
- inline `data:` URI fail closed，不进入 prompt、context telemetry 或 evidence。
- `turn_execution` 在 start turn 前调用 media context skeleton；Aster vendor 不参与。

验证：

```bash
TMPDIR="/tmp/lime-codex-rust-tmp-media-context" XDG_CACHE_HOME="/tmp/lime-codex-xdg-cache" npm_config_cache="/tmp/lime-codex-npm-cache" CARGO_HOME="/tmp/lime-codex-cargo-home-media-context" CARGO_TARGET_DIR="/tmp/lime-codex-media-context-target" CARGO_INCREMENTAL=0 CARGO_PROFILE_DEV_DEBUG=0 CARGO_PROFILE_TEST_DEBUG=0 CARGO_PROFILE_TEST_STRIP=none CARGO_BUILD_JOBS=1 cargo test --manifest-path "lime-rs/Cargo.toml" -p app-server media_context -- --nocapture
TMPDIR="/tmp" XDG_CACHE_HOME="/tmp/lime-codex-xdg-cache" CARGO_BUILD_JOBS=4 cargo test --manifest-path "lime-rs/Cargo.toml" -p app-server context_packet -- --nocapture
TMPDIR="/tmp" XDG_CACHE_HOME="/tmp/lime-codex-xdg-cache" CARGO_BUILD_JOBS=4 cargo test --manifest-path "lime-rs/Cargo.toml" -p app-server memory_prompt -- --nocapture
rustfmt --edition 2021 --check "lime-rs/crates/app-server/src/runtime/context_packet.rs" "lime-rs/crates/app-server/src/runtime/context_media.rs" "lime-rs/crates/app-server/src/runtime/memory_prompt.rs" "lime-rs/crates/app-server/src/runtime/turn_execution.rs" "lime-rs/crates/app-server/src/runtime.rs" "lime-rs/crates/app-server/src/runtime/tests.rs" "lime-rs/crates/app-server/src/runtime/tests/media_context.rs"
git diff --check -- "lime-rs/crates/app-server/src/runtime/context_packet.rs" "lime-rs/crates/app-server/src/runtime/context_media.rs" "lime-rs/crates/app-server/src/runtime/memory_prompt.rs" "lime-rs/crates/app-server/src/runtime/turn_execution.rs" "lime-rs/crates/app-server/src/runtime.rs" "lime-rs/crates/app-server/src/runtime/tests.rs" "lime-rs/crates/app-server/src/runtime/tests/media_context.rs"
```

结果：

```text
media_context: 2 passed（本轮严格使用 `/tmp` 下的 `TMPDIR` / `CARGO_HOME` / `CARGO_TARGET_DIR`，冷编译完成用时 20m46s）
context_packet: 4 passed
memory_prompt: 14 passed
file-level rustfmt: passed
scoped git diff --check: passed
```
...
1. `media-reference` GUI smoke 已通过；binary sidecar read / full preview detail 仍 pending。
2. MCP runtime prepare request skeleton 已落到 App Server inventory 主链；Plugin skill prompt runtime consumer 与 MCP guidance consumer 已验证；`mcpApi.executePrepareRequests(...)` API execution helper 已完成；自动触发、执行后刷新 inventory 与可调用闭环仍 pending。
3. Aster Phase 6 未完成：root `aster` dependency、provider/reply loop、Aster `Agent::reply` / `Message` / provider trait、Aster `Tool` trait 注册壳和 `SessionStore` compat blocker 未清零。

分类：

- `current`：App Server `runtime/context_media.rs` media input reference skeleton、`ContextPacket::media_reference(...)`、`context_packet_telemetry`。
- `compat`：Aster vendor / reply loop 仍只作为现有 executor / event source，不参与 media context truth。
- `deprecated`：无新增。
- `dead`：inline `data:` media 进入 prompt / evidence / context 的路径保持 fail closed；不在 Aster vendor 恢复 media/context 逻辑。

## 76. P2 Plugin skill/MCP runtime consumer skeleton

本节补齐第 61 节之后真正 runtime consumer 的状态记录。主线收益：Plugin / Skills / MCP 不再只证明 App Center 展示消费了 `runtimeCapabilities`，App Server turn prompt 主链也已经消费 workflow-scoped skill prompt policy；MCP binding 已推进到 runtime prepare request skeleton，但仍不冒充已完成 MCP server import / start。

本轮写集：

```text
lime-rs/crates/app-server/src/runtime_backend/plugin_runtime_context.rs
...
- 前端 workspace plugin activation metadata 透传 `plugin_activation.runtime_capabilities` 与顶层 `plugin_runtime_capabilities`。
- App Server `runtime_backend/plugin_runtime_context.rs` 解析 `runtimeCapabilities.skills[].promptInjectionPolicy`、`workflowBindings[]` 与 `mcpBindings[]`，并按当前 plugin activation 的 workflow / task / selected skill refs 选择 prompt skill candidates。
- `agent_skills_context.rs` 已把 plugin runtime skill candidates 接入既有 Agent Skills selection；同名 skill 已在 Agent Skills registry / `.agents/skills` 时会进入 `<selected_skill_instructions>`，未注册 plugin package skill 不伪造正文，只进入 capability guidance。
- `session_config.rs` 渲染 `<plugin_runtime_capabilities>` prompt guidance。
- `agent_skills_telemetry.rs` 输出 `plugin_runtime_capabilities` runtime status，包含 promptSkills 与 mcpBindings。
- `plugin_runtime_context.rs` 已把 `runtimeCapabilities.mcpBindings[]` 归一化为 MCP runtime target，生成 `plugin:<plugin_id>` caller 与 `mcp__server__tool` 预期工具名。
- `tool_inventory.rs` 已消费 request metadata 中的 plugin MCP target，并在现有 MCP server status / tool snapshot 上追加 `plugin_mcp_targets` 投影，输出 `available`、`server_stopped`、`server_available_tool_missing`、`server_missing`、`expectedToolName`、`serverRunning`、`toolListRequest`、`prepareStatus` 与 candidate `prepareRequests` skeleton。
- `prepareStatus` 只表达下一步准备状态：`ready` 不产生请求，`start_required` 输出 `mcpServer/start` + `mcpTool/listForContext` candidate，`import_required` 输出 `mcpServer/importFromApp` + `mcpTool/listForContext` candidate，`configure_required` 与 `tool_missing` 不伪造执行成功。
...
- MCP binding 当前已具备 runtime prepare request skeleton、API execution helper 和 Workspace GUI 手动 trigger / refresh skeleton，但不自动触发，也不把 target 可见性或 candidate `prepareRequests` 等同于工具已可调用。

验证：

```bash
rustfmt --edition 2021 <本轮 Rust 写集>
TMPDIR="/tmp" XDG_CACHE_HOME="/tmp/lime-codex-xdg-cache" CARGO_BUILD_JOBS=4 cargo test --manifest-path "lime-rs/Cargo.toml" -p app-server plugin_runtime -- --nocapture
CARGO_TARGET_DIR="/tmp/lime-plugin-mcp-target" cargo test --manifest-path "lime-rs/Cargo.toml" -p app-server tool_inventory --lib -j 2
npx prettier --write "src/components/agent/chat/workspace/workspacePluginActivation.ts" "src/components/agent/chat/workspace/workspacePluginActivation.unit.test.ts"
npx vitest run "src/components/agent/chat/workspace/workspacePluginActivation.unit.test.ts"
npx vitest run "src/lib/api/mcp.test.ts" "src/lib/api/mcp.failClosed.test.ts"
git diff --check -- <本轮写集>
```
...
```text
app-server plugin_runtime: 8 passed
workspacePluginActivation.unit.test.ts: 15 passed
app-server tool_inventory: 8 passed
src/lib/api/mcp*.test.ts: 18 passed
rustfmt --edition 2021 --check scoped Rust 写集: passed
scoped git diff --check: passed
```

### 76.1 下一刀

下一刀只剩两个真实缺口，按写集释放情况选择：

1. MCP prepare trigger / refresh：在已落地的 `plugin_mcp_targets` prepare request skeleton 与 `mcpApi.executePrepareRequests(...)` 之上，接 GUI / runtime 主链触发、执行后 inventory refresh 和 caller-scoped tool listing 可调用验证，不能只写 prompt guidance，也不能把 candidate request 当作已执行动作。
2. Plugin package skill registry install path：把 plugin package skill 文件注册进统一 Agent Skills registry，避免未注册 plugin skill 永远只能出现在 guidance。

这两个缺口都不应回到 Aster vendor，也不应让 App Center UI 卡片反推 runtime capability。

### 76.2 分类

- `current`：App Server `runtime_backend/plugin_runtime_context.rs`、`runtime_backend/tool_inventory.rs`、前端 API `mcpApi.executePrepareRequests(...)`、plugin activation `runtimeCapabilities` 透传、Agent Skills selection consumer、session prompt guidance、runtime status telemetry、inventory `plugin_mcp_targets` prepare request skeleton。
- `compat`：旧 `skillRefs` / `toolRefs` / raw manifest MCP binding 只作为 snapshot / projection 缺失时的 fallback。
- `deprecated`：未注册 plugin package skill 暂只能进入 guidance，不伪造正文；MCP 自动触发、GUI smoke 与 caller-scoped 可调用闭环仍未接通；退出条件是接通 plugin package skill registry install path 与 MCP caller-scoped callable proof。
- `dead`：不从 App Center UI 卡片或 Aster vendor 反推 runtime capability；不把 MCP guidance、target skeleton 或 candidate `prepareRequests` 写成 MCP runtime import 完成。

### 76.3 MCP prepare trigger skeleton
...
src/components/agent/chat/components/HarnessStatusPanel.tools.test.tsx
src/i18n/resources/{zh-CN,zh-TW,en-US,ja-JP,ko-KR}/agent.json
```

实现事实：

- `useWorkspaceHarnessInventoryRuntime` 从 `toolInventory.plugin_mcp_targets[].prepareRequests` 提取 `status="candidate"` 的 MCP prepare request。
- `prepareMcpTargets()` 只调用 `mcpApi.executePrepareRequests(...)`，成功后复用现有 `refreshToolInventory()`；无 candidate 时 fail closed，不触发 legacy bridge 或 mock。
- Harness 工具库存区新增 “准备 MCP” 动作，只有存在 candidate 且未加载时可点击；错误独立显示为 MCP prepare error，不覆盖 inventory read error。
- 该 skeleton 仍不宣称工具已可调用；caller-scoped list / tool call evidence、GUI smoke 与自动触发仍 pending。

验证：

```bash
npx vitest run "src/components/agent/chat/workspace/useWorkspaceHarnessInventoryRuntime.test.tsx" "src/components/agent/chat/components/HarnessStatusPanel.tools.test.tsx" "src/lib/api/mcp.test.ts" "src/lib/api/mcp.failClosed.test.ts"
npx eslint "src/components/agent/chat/workspace/useWorkspaceHarnessInventoryRuntime.ts" "src/components/agent/chat/workspace/useWorkspaceHarnessInventoryRuntime.test.tsx" "src/components/agent/chat/components/HarnessToolInventoryOverview.tsx" "src/components/agent/chat/components/HarnessToolInventorySection.tsx" "src/components/agent/chat/components/HarnessStatusPanel.tsx" "src/components/agent/chat/components/harnessStatusPanelSectionModels.ts" "src/components/agent/chat/components/HarnessStatusPanel.tools.test.tsx" --max-warnings 0
...
ESLint scoped files: passed
typecheck renderer + node: passed
```

## 77. P2 Media reference GUI smoke evidence

本节补充 `media-reference` current fixture 证据，只关闭 GUI media reference / Workbench preview skeleton / read model `contentParts` 链路；不把它写成 binary sidecar read 或 full binary preview consumer 完成。主线收益：Realtime / Media 的骨架已从 App Server Item skeleton 推进到 GUI 可见链路，下一刀可以直接补 binary sidecar read / full preview detail，而不是重复做 GUI smoke。
...
```bash
TMPDIR="/tmp" XDG_CACHE_HOME="/tmp/lime-codex-xdg-cache" npm_config_cache="/tmp/lime-codex-npm-cache" CARGO_HOME="/tmp/lime-codex-cargo-home-media-reference" CARGO_TARGET_DIR="/tmp/lime-codex-media-reference-target" CARGO_INCREMENTAL=0 CARGO_PROFILE_DEV_DEBUG=0 CARGO_PROFILE_TEST_DEBUG=0 CARGO_PROFILE_TEST_STRIP=none CARGO_BUILD_JOBS=1 npm run smoke:claw-chat-current-fixture -- --scenario media-reference --timeout-ms 240000 --evidence-dir "/tmp/lime-codex-media-reference-evidence" --prefix "claw-chat-current-fixture-media-reference"
```

结果：

```text
summary: /tmp/lime-codex-media-reference-evidence/claw-chat-current-fixture-media-reference-summary.json
session: claw-chat-current-1783404484230-13384
ok: true
mediaReferencePromptReachedBackend: true
guiMediaReferenceInputSubmitted: true
guiMediaReferenceCardVisible: true
guiMediaReferenceDoesNotExposeInlinePayload: true
guiMediaReferencePreviewOpened: true
readModelMediaReferenceCompleted: true
readModelMediaReferenceObserved: true
preview: canvasWorkbenchVisible=true / workbenchPreviewVisible=true / layoutMode=chat-canvas
read model: contentPartsKeyObserved=true / mediaPartCount=1 / matchingMediaPartCount=1 / hasReferenceUri=true / noInlinePayload=true
```

剩余缺口：

1. binary sidecar read / full preview detail 仍 pending。
2. MCP prepare trigger / refresh 仍 pending。
3. P3 weekly loop / 脚本化周期证据仍 pending。

分类：

- `current`：App Server current session/read model、GUI `streaming-media-reference-card`、chat-canvas Workbench preview skeleton、media reference `contentParts`。
- `compat`：Aster vendor 仍只作为受控 executor / event source，不承接 media reference truth。
- `deprecated`：binary sidecar read / full preview detail 尚未接通，作为下一刀 implementation gap。
- `dead`：inline `data:image/base64` payload 暴露路径保持 fail closed，不进入 prompt、Evidence 或 GUI inline payload。

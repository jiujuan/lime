...
该入口启动真实 Electron Desktop Host，使用本地 external fixture backend 产生 `artifact.snapshot` 与 current `turn.completed`，再从 GUI 历史会话打开工作台；它不调用正式模型，不使用 App Server mock backend、renderer mock fallback 或 legacy runtime command。
...
3. 如果用户已经明确“不兼容 / 不留历史包袱 / 要最好的选择”，且临时实现已被正式方案取代，本轮校验目标应证明 current 替换完成；不要继续证明旧 textarea、旧 mock、旧硬编码、旧入口或旧协议“还能跑”
4. 如果本轮改动只是清理 dead surface、补 README、局部整理或低杠杆细节，但没有直接推进主链，不能把“校验通过”当作完成目标
5. 汇报时必须同时给出：
   - 本轮改动对应的路线图节点
   - 本轮校验覆盖了哪条主线风险
   - 当前距离该路线图阶段完成还差什么
6. 用户问“完成了么”时，先回答主线目标是否已经达到可交付门槛；额外校验、顺手清理、可选优化必须单独列出，不要反向覆盖主线结论
7. 校验的上限是证明当前主线可交付；一旦已经覆盖本轮真实风险，就不要因为还有更重的检查可跑而无限追加验证
...
除 `npm run smoke:claw-chat-ready-streaming` 这种用户显式发起的 Claw live Provider / WebSearch-WebFetch E2E 入口外，单项 live Provider 脚本统一使用 `--allow-live-provider`；`npm run smoke:managed-objective-continuation`、`npm run smoke:managed-objective-automation` 与 `npm run smoke:code-runtime-fixture` 默认启动 localhost OpenAI-compatible fixture，不读取 `LIME_AGENT_QC_PROVIDER / LIME_E2E_PROVIDER / LIME_DEFAULT_PROVIDER` 作为真实 Provider 兜底，只有显式 `--allow-live-provider` 或 live Provider 环境授权后才可指定真实 provider/model；Vitest 的 `*.live.test.*` 默认从 `npm test` / `npx vitest` 收集中排除，直接点名运行也必须设置 `LIME_ALLOW_LIVE_PROVIDER_SMOKE=1` 或 `LIME_REAL_API_TEST=1`；普通 Vitest 默认还会安装外部网络守卫，覆盖 `fetch`、`XMLHttpRequest`、Node `http` / `https` 与 `net` / `tls` 直连请求，只允许 localhost / data / blob / file 等离线路径，避免未按 `.live.test` 命名的测试或 SDK 误打 Deepseek、OpenAI 或图片 Provider；Rust 真实联网测试必须同时使用 `#[ignore]` 和 `LIME_REAL_API_TEST=1` 二次门禁。设计画布只有 `--image-task live-single-layer` 需要该授权，connector outbox 只有 `--mode live` 需要该授权，Plugin 内容工厂 current-turn / GUI 证据若调用真实 Provider 必须显式授权；replay / fixture / registry-only 路径不应调用真实 Provider。
...
- 检查纯文本 `Claw @配图` 是否已经走 `原始用户消息 -> harness.image_command_intent.image_task -> App Server ImageCommandWorkflow -> 标准 image task artifact + worker` 主链，显式图片动作是否也已经走 `synthetic user message / displayContent -> harness.image_command_intent.image_task -> App Server ImageCommandWorkflow -> 标准 image task artifact + worker`，统一目录声明的图片模型标签（例如设置页从 Provider 模型创建的 `@Nano Banana 2` / `@GPT Images 2`，或 Lime Cloud 下发的同构命令）是否也进入同一 `image_command_intent` 回合并只把 Provider / model / executor_mode 作为路由上下文；未在 catalog 中声明的任意 `@模型名` 不应自动调用图片 API。图片主链不能卡在 `ToolSearch / WebSearch / Read / Glob / Grep`、退回旧的 `Skill(image_generate)` 首发链、`Bash/lime media image generate --json` / `lime task create image --json` 或聊天前端直建任务旁路，或把 `任务 ID：{task_id}` 这类模板占位原样显示出来
...
- 检查纯文本 `Claw @海报` 是否已经走 `原始用户消息 -> harness.image_command_intent.image_task -> App Server ImageCommandWorkflow -> 标准 image task artifact + worker` 主链，而不是回流成普通聊天、另一套海报协议、旧的 Skill/Bash/CLI 图片旁路，卡在 `ToolSearch / WebSearch / Read / Glob / Grep`，或前端本地伪造结果；同时确认默认 `entry_source=at_poster_command`、默认尺寸 `4:5 -> 864x1152` 与“海报设计”语义补齐仍然成立
...
- 修改 `create_image_generation_task_artifact`、`get_media_task_artifact`、`list_media_task_artifacts`、`cancel_media_task_artifact`、`src/lib/api/mediaTasks.ts`、`src/lib/api/skill-execution.ts`、`useWorkspaceSendActions`、`useWorkspaceImageWorkbenchActionRuntime`、`runtime_backend/image_command`，或调整 `Claw @配图` / 图片模型标签 `-> harness.image_command_intent.image_task -> App Server ImageCommandWorkflow -> task artifact/worker` 的异步图片任务主链
...
- 修改 `@海报` parser、`useWorkspaceSendActions`、`runtime_backend/image_command`、`image_command_intent`、`mediaTaskArtifact/image/create`、图片 worker 或相关图片 timeline 展示，尤其是调整 `Claw @海报 -> harness.image_command_intent.image_task -> App Server ImageCommandWorkflow -> task artifact/worker` 主链

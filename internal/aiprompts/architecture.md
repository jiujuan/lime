...
cancel 不改变 goal。per-thread listener 严格在对应 terminal event 后发送
...
producer 的异常终止 flush、tool-finish/abort flush、idle time、
自动 continuation、outbox crash-drain 与 MCP terminal notification 仍为 OPEN；禁止在 Renderer、Electron、RuntimeCore read
...
Responsible developer confirmation: root, 2026-07-20.
...
idle accounting、outbox crash-drain、automatic continuation 和 Codex

...
- `current`：Rust reply policy 的 Aster 自动压缩事件投影事实源是 `lime-rs/crates/agent/src/request_tool_policy/auto_compaction_projection.rs`；中心执行器只能消费 `AutoCompactionProjectionState::project_event(...)`，不得重新散落自动压缩 system notification 过滤、disabled context limit 错误投影或 compaction failure 文案提取。
- `current`：live `message.delta/text_delta` 必须保留 `itemId/phase/sequence/turnId`；显式 `phase=final/final_answer` 且带 `itemId` 的 delta 必须像 commentary 一样进入 `Message.contentParts` 的结构化 text part，不能只停留在 streaming overlay。只有“整轮没有 process boundary”的旧无 phase 文本可进入 assistant final overlay / `Message.content`。一旦后续出现 tool / reasoning / plan / action 这类 process boundary，legacy 无 phase 文本必须 fail closed：清除 overlay，不进入 final answer，不再因缺少 `sequence` 绕过边界。
...
10. streaming overlay 不是全局“当前助手文本”缓冲；overlay 只允许承载 final answer 的可见增量，不能承载 commentary、reasoning、search 阶段说明或工具状态文案。
11. `legacy_unphased` / `item_scoped_legacy` 文本只能在未遇到 process boundary 时作为旧流 final fallback；如果 process boundary 证明它只是工具前后阶段说明，必须清除 overlay、保留 process 顺序，并 fail closed。只有显式 `phase=commentary` 的 agent message 才能作为 commentary thread item 展示。
...
- `agentStreamRuntimeHandler.unit.test.ts`：工具先到、早期 text 后到、后续 text 再到的乱序事件；工具完成后没有最终正文不能把工具前开场白当完成；工具边界前 legacy text 不得提交到 final content；显式 `phase=final_answer` 在 `turn_completed` 前已经进入结构化 `contentParts`，且不依赖 overlay。

...
- [x] Rust `style_profile.rs` 不再维护四套静态 profile；通过 `include_str!` 读取同一组 Style Pack manifest，并只负责 registry profile id、intensity 覆盖与 prompt context projection。
...
- [ ] media / contentParts 继续接通用 binary sidecar read、sidecar media store、digest 校验和非 sourcePath owner 的完整读取链。
...
下一刀优先级：骨架阶段继续优先补直接影响“用户能感受到 Soul”的输出面。Style Pack 安装链路骨架、MessageList media reference skeleton、source owner-backed media preview skeleton 和 media-reference current Electron fixture skeleton 已可收口；后续按杠杆排序是：先补通用 binary sidecar read / sidecar media store / digest 校验，其次盘点业务协作入口是否还未消费 `collaborationFacts`，再扩展业务域 risk taxonomy 并清理工具 summary 特殊工具名 fallback。Cloud catalog / download、签名 / digest 实测校验、安装审计，以及安装真实风格包后在 Claw 中选择 profile 的 Playwright / Claw 体验验收仍是后续项。

...
- `deprecated`：Aster `CredentialBridge` 中 provider/model 字符串映射。后续只能委托 `ResolvedModelRoute`，不能继续长 provider-specific 规则。
...
- `runtime_backend.rs` 现在只负责 turn 生命周期编排和事件顺序，消费 `ChatModelRouteResolution` 后继续交给现有 Aster provider wire；未新增 App Server JSON-RPC method，未修改 Electron bridge，未改变 provider 执行协议。

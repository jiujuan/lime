...
- W3 仍不标 complete：本轮已补 App Server normalizer 注入 owner 负向矩阵、迁掉两条 current smoke 的 `turn.final_done` 依赖、补了代码产物 Electron GUI 侧 `turn.completed` 证据，收掉 `lime-agent` workflow 正向 `FinalDone` emit，删除 Rust 协议层 `FinalDone` 输入面，并把 packages/frontend legacy detector 收敛到 shared fail-closed owner。下一刀继续清历史执行计划 / 旧 roadmap 中仍把 `final_done` 写成 current fixture evidence 或实现入口的 residual，并复核 live runner timeout grace 是否仍有真实实现入口；若只剩历史 evidence，则登记为 history-only。

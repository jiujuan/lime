...
- `npm run test:contracts` 在本轮复跑时被非 MCP 脏改动阻塞：`src/components/agent/chat/hooks/useAsterAgentChat.ts` 当前把 `getThreadItems` 从 `() => session.threadItems` 改成了 `session.getThreadItems`，触发 `scripts/check-app-server-client-contract.mjs` 的 “Renderer Agent chat passes active stream event into runtime sync” 守卫。本轮未回退该 Agent chat 改动，MCP 定向测试与 GUI smoke 已通过。
- 本轮再次复跑 `npm run test:contracts`，仍被同一处非 MCP `useAsterAgentChat.ts` contract 阻塞；MCP 本轮写集未触碰该文件。

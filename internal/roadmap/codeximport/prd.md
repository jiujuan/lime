...
- 导入成功后直接打开新会话。
- canonical 历史消息、命令、工具、patch、搜索、reasoning 按真实时序写入 read model；消息主线只
  投影最终正文、附件、文件产物/变更与处理时长。
- terminal 历史不挂载 command/tool/approval/search/reasoning operational DOM，也没有点击展开；
  当前 active turn 才展示完整运行过程，普通对话、历史恢复与导入共享同一规则。
- 来源只作为会话 provenance，不改变 Item schema、状态或执行语义。
- 宽窗口可以并排查看聊天与工作台；窄窗口默认保留完整聊天，通过“聊天 / 工作台”模式
  控件切换，不把聊天藏进抽屉，也不因缩放重挂载消息树或丢失滚动位置。
...
2. canonical read model 保留历史 command 的 command/cwd/output/exit code；terminal GUI 不挂载
   command row，但仍展示相关文件产物/变更和最终正文，且没有重新执行痕迹。
...
7. 真实样本 GUI 审计按有界预算执行并记录 source 规模；超过预算的历史必须进入可观测的
   分段/后台导入流程，不能在同步请求中无提示超时。
8. desktop、compact、narrow 下消息列表和输入框均可用；聊天/工作台模式控件不得与其他
   按钮重叠，断点切换不得重挂载消息树或丢失滚动位置；大历史 terminal GUI 不挂载 operational
   item，滚动性能不因 canonical item 数量线性恶化。

后台导入只允许使用 `conversationImport/thread/commit` 启动、`conversationImport/job/read`
读取状态；GUI 必须展示当前阶段、当前/总会话数与稳定百分比。job terminal result 才能触发
进入会话；queued/running 不得伪造成成功，也不得建立 imported-only session read model。

...
当前 `execution_profile` / `executor_adapter` 已从治理 registry 进入前端 launch metadata、Rust runtime contract snapshot、Evidence Pack、Replay 与 `list_media_task_artifacts` 统一媒体任务索引。任务列表可以直接查询 `entry_key`、`thread_id`、`turn_id`、`content_id`、`modality`、`skill_id`、`model_id`、`cost_state`、`limit_state`、`estimated_cost_class`、`limit_event_kind`、`quota_low`、`profile_key`、`adapter_key`、`executor_kind`、`executor_binding_key`、`limecore_policy_refs` 与最小 `limecore_policy_snapshot(...)`；Evidence Pack 的 `snapshotIndex.taskIndex`、Replay / grader、前端任务中心查询模型、`HarnessTaskIndexSection` 与内嵌任务中心过滤列表消费同一查询口径，`task index presentation guard` 会阻止该过滤面回流成面板内联平行实现。图片 / 视频 worker 已消费 `ResolvedModelRoute` 与本地 broker execution binding；`voice_generation` 当前是 `metadata_only` compat，只保留 GUI / Skill 元数据和 `audio_task` 草稿索引，不声明 `executor_invoked`，不写 `resolved_route` / `model_route_execution`，直到 audio worker 或 RuntimeCore provider protocol mapper 接入。当前默认 `policy_value_hits=[]`、`policy_value_hit_count=0` 只表示真实 LimeCore 控制面命中值尚未接入；如果已有 `status=resolved` 的命中值，resolver seam 会把该 ref 转入 `evaluated_refs` 并从 `missing_inputs / pending_hit_refs` 移除。图片任务执行前的本地 model registry assessment 已成为最小 `model_catalog` hit producer；图片任务进入真实执行器前也会从已解析的 runner config/API key 与 task payload provider/model 生成最小 `provider_offer` hit，且不序列化 API key。Browser Assist 与 Web Research 类 launch 现在也会从请求侧 `harness.oem_routing` 生成最小 `gateway_policy` hit，Workspace send metadata 还会从 OEM Cloud bootstrap snapshot 的 `features` 生成 `tenant_feature_flags` hit。权限确认与显式用户模型锁定仍按本地最小恢复入口处理，不代表完整 GUI 自动重试、云端 LimeCore 授权或上层 `@` 命令事实源。

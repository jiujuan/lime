...
- `tool-runtime::native_dispatch` 已建立 Codex-style current dispatcher 骨架，统一持有已迁 native tool 的 definition / executor dispatch：`apply_patch`、`skill_search`、`sleep`、`view_image`、`update_plan`、`WebFetch`、`WebSearch`，并通过 gateway-aware builder 接入 `memory_store` / `image_task` / `tool_search`。`lime-agent` 中对应临时 Aster `Tool` adapter 已改为委托 `runtime_native_dispatch_handle()` 或 per-gateway `NativeDispatch::builder()`，adapter 不再自行选择 memory/image/tool_search executor handle；gateway-backed memory/image/tool_search 的 Aster 适配已集中到 `native_tools/gateway_bridge.rs`，旧 `native_tools/{memory_store,image_tasks,tool_search}.rs` 已删除，模型可见 definitions 只从同一个 dispatch 读取，不再直接调用 `memory_store_tool_definitions()` / `image_task_tool_definition()` 或 Aster vendor `ToolSearchTool`；`native_tools/runtime_tool_bridge.rs` 集中 Aster `ToolContext` / `ToolResult` / `ToolError` 与 current `RuntimeTool*` 的临时转换，单个 wrapper 不再复制这套桥接代码。
...
### 2026-07-09：Agent init command boundary Aster-free
...
- `classification`：`current` 是 `initAgentRuntime` / `AgentRuntimeInitStatus` 与 `AgentRuntimeAdapter.init()`；当时底层 Electron IPC 命令名仍是 `aster_agent_init`，已在后续 `Agent init command boundary Aster-free` 批次迁出；`dead / guarded` 是前端公开 API、fixture 或 adapter 继续暴露 `initAsterAgent` / `AsterAgentStatus`。
...
- `completed`：`agent-runtime::ask` 新增 `RequestUserInputRunRequest`、`RequestUserInputAction`、`RequestUserInputGateway`、`RequestUserInputError` 与 `run_request_user_input(...)`，current owner 统一负责 prompt 解析、requested schema 构造、gateway 调用和 response 归一化。
- `completed`：`lime-agent/src/ask_bridge.rs` 收缩为 Aster `AskCallback` adapter：只保留 Aster `AskRequest` 到 current `AskRequest` 的 DTO 映射、Aster action scope 到 `agent_protocol::action_required::ActionRequiredScope` 的转换，以及临时 `ActionRequiredManager` gateway。adapter 不再直接调用 `resolve_request_prompt`、`build_requested_schema` 或 current response extractor。
- `guarded`：`asterMigrationBoundary.test.ts` 要求 `agent-runtime/src/ask.rs` 持有 request_user_input current runner 且不含 `aster::`；同时禁止 `ask_bridge.rs` / `lime-agent` crate 根恢复 Ask prompt/schema/response 纯逻辑或公开 Aster callback/request 类型。
- `Thread / Turn / Item`：归属 Turn action_required / elicitation lifecycle。Lime 当前真实消费链是 Aster reply loop 的 `AskCallback` 触发 `ActionRequiredManager`，再由 App Server / GUI action response 主链接收用户输入；本刀把其中可复用 runner 迁到 `agent-runtime`，为后续删除 Aster reply loop callback 壳做准备。
- `classification`：`current` 是 `agent-runtime::ask::{run_request_user_input, RequestUserInputGateway, RequestUserInputRunRequest}` 与 `agent_protocol::action_required::ActionRequiredScope`；`compat` 是 `lime-agent/src/ask_bridge.rs` 的 Aster `AskCallback` / `ActionRequiredManager` gateway；`dead / guarded` 是 adapter 内恢复 prompt/schema/response runner、`extract_current_ask_response` 直连和 crate 根公开 Ask bridge。
- `validated`：`cargo fmt --manifest-path "lime-rs/Cargo.toml" --package agent-runtime --package lime-agent -- --check` 通过。
- `validated`：`cargo test --manifest-path "lime-rs/Cargo.toml" -p agent-runtime ask --lib -j 2` 通过，`4 passed`；`cargo test --manifest-path "lime-rs/Cargo.toml" -p lime-agent ask_bridge --lib -j 2` 通过，`3 passed`。
...
- `completed`：`lime-agent/src/request_tool_policy/aster_reply_backend_adapter.rs` 的 `AsterReplySource` 只负责把 `RuntimeReplyMessage` / `AgentSessionConfig` lowering 成 Aster `Message` / `SessionConfig`，然后调用 `call.run_with(CompatReplySourceExecutor::new(...))`。`CompatReplySourceExecutor` 只实现 `run_default(...)` 调用旧 `Agent::reply(...)` 与 `run_provider(...)` 调用临时 `ConfiguredReplyProvider::stream_reply_with_agent(...)`。
- `guarded`：`asterMigrationBoundary.test.ts` 要求 `agent-runtime` 暴露 `RuntimeReplySourceExecutor` / `run_with`，并要求 Aster adapter 的 `AsterReplySource` 不含 `RuntimeReplySourceCall::Default` / `Provider`、`.reply(...)` 或 `.stream_reply_with_agent(...)`。旧路回流只能发生在显式 `CompatReplySourceExecutor`，且该 executor 仍是 reply loop 未迁出前的删除边界。
...
- `completed`：新增 `tool-runtime::skill_execute` current owner，承接 Skill execution envelope、session access gate、workspace skill source lookup、runtime contract preflight、调用参数规范化、backend result metadata 合并和 Aster-free execution backend trait。
...
- `completed`：新增 `lime-skills::run`，承接 Aster-free `SkillRunner`、prompt Skill 执行、workflow step 顺序执行、变量插值、step result DTO 与 `SkillRunResult` metadata。该 runner 不依赖 Aster，也不返回 Aster `ToolResult`。

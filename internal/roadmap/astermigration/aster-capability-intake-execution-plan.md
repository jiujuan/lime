...
- `tool-runtime::native_overlay` 已承接 Lime-owned native tool overlay 清单（`view_image`、`apply_patch`、`skill_search`、`Skill`、`sleep`、`update_plan`、`WebFetch`、`WebSearch`）；`native_tools/runtime_overlay.rs` 只按该 current 清单注册 Aster `Tool` 对象，`tool_inventory_runtime_adapter.rs` 也只把该清单命中的工具标成 `current_surface`，不再把整个 Aster registry 误标为 current。Aster `WriteTool` / `EditTool` 与 `TaskCreate` / `TaskList` / `TaskGet` / `TaskUpdate` / `TaskOutput` / `TaskStop` 已从 Lime production tool 面移除；历史 `Write` / `Edit` / `Task*` 只允许作为 frontend / hook matcher / transcript 兼容读面，不再是生产 native tool。
...
- `agent-runtime::reply_message` 已建立 Aster-free reply message current contract，承接 text / image / action-required response message content、role、agent-only 与 text concat 规则；`aster_reply_adapter.rs` 只把该 current message lowering 成 Aster `Message`，Aster `Message` 不再是 reply input/message 的隐含事实源。
...
- `completed`：`aster_reply_adapter.rs::start_aster_reply_stream(...)` 在进入 Aster backend 前先把 `RuntimeReplyAttemptInput` 转成 `RuntimeReplyMessage`，`message_chars` 也从 current message 计算；Aster `Message` 构造收缩为 `lower_aster_reply_message(...)` 单一 lowering 边界。
- `guarded`：`asterMigrationBoundary.test.ts` 要求 `agent-runtime` 导出 `reply_message`，要求 `reply_message.rs` 不含 Aster source type，并要求 `aster_reply_adapter.rs` 通过 `RuntimeReplyMessage::from_attempt_input(...)` / `lower_aster_reply_message(...)` 做 DTO lowering，禁止恢复 `build_aster_user_message(...)`。
...
- `completed`：`agent_skills_context.rs` 把 plugin prompt skill candidate 接入既有 Agent Skills selection；同名 skill 已在统一 Agent Skills registry / `.agents/skills` 时注入 `<selected_skill_instructions>`，未注册 plugin package skill 不伪造正文，只进入 capability guidance。
...
- `completed`：前端 API `mcpApi.executePrepareRequests(...)` 已能顺序执行 candidate `prepareRequests`，只复用 App Server current `mcpServer/importFromApp`、`mcpServer/start` 与 `mcpTool/listForContext`；非 candidate、未知 method、malformed params 均 fail closed，且不回退 legacy `safeInvoke`。
- `Thread / Turn / Item`：归属 Turn prompt/context selection 与 Turn tool inventory prepare skeleton；MCP binding 只有在后续接入 MCP server import / start / caller-scoped listing execution owner 并被工具调用消费后，才进入完整 Turn tool lifecycle 与 Item tool projection。
- `validated`：`rustfmt --edition 2021` 覆盖本轮 Rust 写集通过；`cargo test --manifest-path "lime-rs/Cargo.toml" -p app-server plugin_runtime -- --nocapture` 通过，`8 passed`；`CARGO_TARGET_DIR="/tmp/lime-plugin-mcp-target" cargo test --manifest-path "lime-rs/Cargo.toml" -p app-server tool_inventory --lib -j 2` 通过，`8 passed`；`npx vitest run "src/components/agent/chat/workspace/workspacePluginActivation.unit.test.ts"` 通过，`15 passed`；`npx vitest run "src/lib/api/mcp.test.ts" "src/lib/api/mcp.failClosed.test.ts"` 通过，`18 passed`；scoped `git diff --check` 通过。共享 target 曾因并行 target 竞争出现链接缺失，不作为代码失败结论。
- `classification`：`current` 是 App Server `runtime_backend/plugin_runtime_context.rs`、`runtime_backend/tool_inventory.rs`、前端 API `mcpApi.executePrepareRequests(...)`、Agent Skills selection consumer、session prompt guidance、runtime status telemetry 与 inventory `plugin_mcp_targets` prepare request skeleton；`compat` 是旧 `skillRefs` / `toolRefs` / raw manifest MCP binding 的缺 snapshot fallback；`deprecated` 是未注册 plugin package skill 只能作为 guidance，MCP prepare trigger / refresh 仍未接通；`dead` 是从 App Center UI 卡片或 Aster vendor 反推 runtime capability，以及把 MCP guidance / target skeleton / candidate `prepareRequests` 误报成 MCP import 完成。
- `remaining`：MCP prepare trigger / refresh 与 plugin package skill registry install path 仍 pending；full media preview / binary sidecar read / Workbench consumer 仍 pending。整体 Phase 6 仍约 `96%`，root `aster` dependency、provider/reply loop、Aster `SessionStore` / `Tool` trait 注册壳未清零。

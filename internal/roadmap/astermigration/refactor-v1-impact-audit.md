...
- `compat blocker`：`LimeSkillTool` 仍是 Aster `Tool` trait 外壳，等待 R4 reply loop tool execution 迁出。
- `dead / deleted / forbidden-to-restore`：Aster skills registry / loader / executor / workflow helper 和 `loadSkill` platform extension。
...
- R4 完成后删除 `LimeSkillTool` Aster `Tool` trait 外壳；后续 Skill 能力只通过 current owner 进入 runtime / App Server / GUI。

...
Aster 框架里的 agent、provider、tool、session、permission、MCP、skills 等能力有参考价值，但不能因此继续把 Aster 作为 Lime current runtime 事实源。正确策略不是“因为有用所以保留整套 Aster”，也不是“把 vendor 当垃圾粗暴清空”。2026-07-07 起，本策略按 **Codex 有则迁，Codex 没有则删** 收紧：

1. Codex 有且需要进入 Lime 产品主链的能力，迁到 Lime-owned current crate。
2. 已迁能力从 vendored Aster 删除，并用守卫阻止回流。
3. Codex 没有的 Aster-only 能力标为 `dead / deleted / forbidden-to-restore`，同步删除 vendor current surface、catalog alias、前端专用展示和正向测试。
4. 暂时还没 owner 但 Codex 有对应 current 能力的项目只允许作为 `compat` / `deprecated` adapter 存活，并写清退出条件。
5. 只具参考价值、尚未产品化的能力进入历史 evidence 或 backlog，不允许用来延续 root `aster` dependency。
...
命名声明：迁移后的 current API 必须短、领域化、可读。Codex 工具名和 Aster 的简洁命名可以作为品味参考，但不得把 `lime_*`、`aster_*`、`agent_runtime_*` 或冗长历史词带入 current API。
...
| `valuable-reference`           | Codex 有相近 current 能力，但 Lime 暂未产品化 | 记录设计参考、测试场景和未来 owner                   | 保留 root Aster dependency 只为了“以后可能会用”        |
| `aster-only-dead`              | Codex 无对应 current 能力                     | 删除 vendor / catalog / frontend 正向入口并补守卫    | 因为 Aster 实现完整就继续维护 compat 壳                |
...
| `tools` / registry executor | 有价值，包含 registry、tool context、执行错误、host tool bridge | `tool-runtime`                                             | `tool_orchestrator` registry adapter 已删除；Aster reply loop native tool registry 仍 `compat-blocker` | 已迁 shell/path/process/WebFetch/WebSearch/apply_patch/skill_search/memory_store/image_task/sleep/view_image 能力继续从 vendor 删除；下一步迁 reply loop 内工具注册壳 | Aster reply loop 不再需要这些临时 `Tool` trait adapter，vendor tool runtime 重复实现清空             |
...
2. Aster reply loop 内 native tool registry / WebFetch / WebSearch / apply_patch / skill_search / memory_store / image_task / sleep / view_image `Tool` trait adapter：迁到 `tool-runtime` current executor，让已迁工具能力真正脱离 Aster `Tool` trait 注册壳。

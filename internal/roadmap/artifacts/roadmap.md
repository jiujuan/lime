...
9. 只有可抽取出可读文本的文档（例如 DOCX 文本抽取结果）才进入 `renderMode=document_text`；PDF、Excel、PPT 等二进制文档若没有文本抽取结果，仍保留同一 preview artifact，但必须退化为 `renderMode=system_open`，由统一 artifact 工具栏调起系统默认应用，而不是渲染空白正文。
10. 二进制不可内嵌时仍进入同一 preview artifact，renderMode 退化到 `system_open` 或 `unsupported`，而不是消失。
...
- PDF、Excel、PPT 等暂无文本抽取的二进制文档不再进入空白 `document_text`，而是进入 `contentKind=document / renderMode=system_open`；仍使用同一 preview artifact、同一 ArtifactToolbar、同一 `open_with_default_app` 桌面 current 命令。

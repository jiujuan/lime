...
| 路径 | 责任 |
| --- | --- |
| `lime-rs/src/commands/plugin_runtime_cmd.rs` | Plugin runtime Tauri command 外壳，只做参数反序列化和 state 注入。 |
| `lime-rs/src/services/plugin_runtime_service.rs` | App task facade：校验 appId / entryKey / manifest capability / provenance，组装 surface request。 |
| `lime-rs/src/services/agent_runtime_surface_service.rs` | 多 surface 共享入口：把 Chat、Claw、Plugin、Automation 输入转换成 runtime turn。 |
| `lime-rs/src/services/plugin_runtime_capability_catalog_service.rs` | Plugin -> Claw capability catalog 第一刀：声明 capabilityId / alias / launch metadata contract，并由 runtime facade 复用。 |

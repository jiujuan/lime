...
当前已落地的业务 surface 之一是 `articleWorkspace`，对应 Writing / 内容工厂的右侧 Article Editor。它的 dock、tab、compact / expanded 行为遵循本文件；写作路线图只描述文章内容、workflow 和编辑动作，不再重复右侧布局规则。
...
| Surface Kind | 一个可注册的 tab / pane 类型，如 `productProfile`、`articleWorkspace`、`file`、`evidence`、`terminal`、`browser`、`sideChat` |

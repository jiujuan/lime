...
| `current`    | `electron/updateHost.ts`、`forge.config.mjs`、`electron/forge/*`、`.github/workflows/release.yml` | updater、签名、公证和发布包事实源                                                            |
...
| `dead`       | `electron-builder.yml`、`electron-builder` CLI、builder yml 事实源                                | 不得作为 release、updater、签名、公证、CI、i18n app metadata 或版本同步输入                  |
...
5. `electron-builder.yml` / `electron-builder` CLI 已进入 `dead`，不能作为 Electron current 打包、发布或 metadata 事实源。

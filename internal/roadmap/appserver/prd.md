...
2. 让 Lime Desktop 继续可用，同时逐步把现有 Tauri command glue 收敛为 app-server adapter。
...
2. 抽象 `RuntimeCore`，供 App Server 和 Tauri command 同时委托。
...
1. Tauri command 迁移后仍能走原有 GUI 主路径。
2. 旧命令只作为 thin adapter 委托服务层。
...
这样不需要理解 Lime Desktop 内部目录、Tauri command 和 runtime_turn 细节。
...
1. Tauri command 与 App Server 都委托 `RuntimeCore`。
...
8. Tauri command 迁移计划明确哪些保留为 compat adapter。
...
2. Tauri command 层 Agent 业务逻辑 LoC 逐步下降。

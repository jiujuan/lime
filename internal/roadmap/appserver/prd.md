...
2. 让 Lime Desktop 继续可用，同时逐步把 legacy Tauri command glue 收敛为 App Server compat adapter。
...
2. 抽象 `RuntimeCore`，供 App Server 和 legacy Tauri adapter 同时委托。
...
2. legacy Tauri 命令只作为 thin adapter 委托服务层。
...
这样不需要理解 Lime Desktop 内部目录、legacy Tauri command 和 runtime_turn 细节。
...
我希望所有执行仍进入同一个 RuntimeCore，并按 backend adapter 分发，
这样 tool runtime、workspace、memory、artifact、evidence 和 policy 不会在多个 App 中复制。

验收：

1. legacy Tauri adapter 与 App Server 都委托 `RuntimeCore`。
...
3. 不把所有 legacy Tauri commands 一次性迁走。
...
8. legacy Tauri command 迁移计划明确哪些保留为 compat adapter。
...
2. legacy Tauri command 层 Agent 业务逻辑 LoC 逐步下降。

...
5. 旧 Aster / agent message / timeline 多条路径长期并存，事实源不清。
...
| Aster session store 直接写 `agent_messages` 的新逻辑 | deprecated | event log writer 接管新会话 |
...
2. 旧 `request_logs/`、`sessions/`、`aster/` 目录进入 P0 inventory，分别标记为 `current / compat / deprecated / dead`，不能因为目录还存在就继续承接新 runtime fact。
...
1. P0 inventory 必须输出所有 durable 写入根目录：Product DB、request logs、sessions、Aster、workspace `.lime`、Electron `userData`、cache/temp。
...
1. 禁止新增生产代码把新 Agent transcript 直接写入 `agent_messages`。
2. 禁止新增生产代码把大输出写入 `agent_thread_items.payload_json`。
3. 禁止新增 current 文档引用 `lime-rs/src/**` 作为实现路径。
4. 禁止 App Server export 绕过 shared load context。
5. 禁止 GUI 直接读取 event log 或 sidecar 文件。
...
3. `agent_thread_items.payload_json` 大正文写入被守卫禁止。
4. 裸 `outputSnapshotFile` / `checkpointSnapshotFile` 新写入被守卫禁止，除非同一 payload 已有 `sidecarRef.sha256`。
5. current runtime event 缺少 `sidecarRef.sha256` 的大正文 ref 被守卫禁止。
6. hard-coded platform path 写入被守卫禁止。
...
| S2 | 当前唯一主刀：guard before fine detail | scans、contract guard、legacy writer whitelist、path guard | compat 常驻且继续回流 |
| S3 | legacy message exit | migration marker / optional user export / `agent_messages` 产品路径删除 | 旧数据未迁移导致用户历史不可读 |
| S4 | fine-grained cleanup | P4-b、current_timeline 收尾、旧 DAO/store/schema 引用清零、历史表 drop migration；usage/model 已退出 `agent_messages` fallback | 细节先行导致骨架未封口 |
...
P0 inventory、P1 data root / JSONL event log 底座、P2 最小 Projection DB / session-scoped repair、P3 shared load context、`message.created` durable event、P4 Telemetry DB 最小骨架和 P5/S1 Sidecar contract 已完成。下一步不再把 `current_timeline` 旧桥、P4-b shared logger 查找或 evidence 展示 polish 作为主线，执行顺序固定为：
...
2. S2 当前唯一主刀：补骨架守卫，扫描 `agent_messages` 新写入、`agent_thread_items.payload_json` 大正文、裸 `outputSnapshotFile` / `checkpointSnapshotFile`、缺少 `sidecarRef.sha256` 的 current runtime event 和业务层硬编码平台路径，并禁止产品层通过 `AgentDao` / `ChatDao` 旧消息 API 回读。
3. S3 只在 S2 守卫可用后推进：旧历史迁移或导出完成后删除 `agent_messages` 产品读写路径。
4. S4 细节收尾：P4-b 真实 request telemetry 接线、`current_timeline` bridge 收口、旧 DAO/store/schema 引用清零和历史表 drop migration；usage/model 统计已退出 `agent_messages` fallback，Telemetry DB 归属作为后续细化项。

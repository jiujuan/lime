...
| `agent_messages` legacy backfill     | `agent/src/session_store.rs`、`services/src/aster_session_store/legacy_conversation.rs`、历史 smoke fixture | JSONL Event Log + Projection DB                                 | `compat`，只允许 migration/backfill/export/test fixture 输入                          | S3 实现幂等状态机并收窄白名单；S4 清旧 DAO/schema 引用            | `npm run smoke:agent-session-history-electron-fixture`                                                       |

...
flowchart LR
  ...
  Worker --> Artifact[articleDraft Artifact]
  Artifact --> Card[聊天小产物卡]
  Card --> RightSurface[右侧 Product Profile]
...
| 层 | 责任 | 不允许 |
| --- | --- | --- |
| 内容工厂插件 | 按 Lime Plugin Package v1 声明入口、workflow、subagents、skills、CLI、connectors、hooks、renderer 契约。 | 直接控制 Lime 右侧栏布局。 |
...
| 聊天消息区 | 展示运行状态、小产物卡和动作入口。 | 展开整篇正文。 |
| Right Surface | 渲染文章 Product Profile、编辑动作、历史恢复。 | 直接调用 provider 或插件私有文件系统。 |
...
| `/Users/coso/Documents/dev/ai/aiclientproxy/lime` | 插件安装态读取、manifest normalize、plugin contract、输入栏建议、activation metadata、Product Profile、GUI / Playwright 验证。 |
...
| 右侧栏被插件重建 | 右侧 dock 由 Host 管理，插件只声明 renderer / surface contract。 |

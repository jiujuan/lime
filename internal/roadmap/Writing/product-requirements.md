...
内容工厂已经作为插件存在，因此写作入口不应再通过宿主硬编码。正确路径是让内容工厂按 Lime Plugin Package v1 声明自己能写文章，声明需要哪些子智能体、skills、CLI、connectors、hooks 和 workflow；宿主只负责安装态发现、显式激活、运行 metadata 透传、产物卡和右侧 Product Profile。

## 2. 目的

1. 用户输入 `@写文章`、`@写作` 或选择内容工厂写作入口后，明确进入内容工厂插件 workflow。
2. 写作过程不是一次普通聊天回答，而是有搜索、策划、写作、校对和配图规划的多步编排。
3. 文章正文最终作为结构化产物出现，聊天里只显示小产物卡。
4. 用户点击小产物卡后，右侧 Product Profile 展开文章草稿、结构、引用和后续动作。
...
| 内容创作者 | 能看到文章结构、引用、配图建议和审稿状态，后续可继续改写。 |
| 插件开发者 | 能通过 Lime 插件包自描述入口、子 Agent、skills、CLI、hooks 和 renderer，不需要宿主写死能力。 |
...
| WS-03 | 作为用户，我不希望聊天窗口被整篇文章刷屏。 | 聊天只出现文章产物卡和摘要；完整正文进入 Product Profile。 |
| WS-04 | 作为用户，我想点击文章卡后在右侧继续编辑或生成配图。 | 小产物卡点击后打开右侧 `articleDraft` Product Profile。 |
...
5. 聊天中出现文章小产物卡。
6. 用户点击卡片，右侧 Product Profile 打开文章草稿。
...
3. 如果有 selected `articleDraft`，右侧直接恢复该文章。
...
2. 聊天正文不承载完整文章。
3. 小产物卡需要稳定尺寸，避免流式内容把布局撑开。
4. 右侧 Product Profile 可展开 / 收起，并保留宿主 tab 行为。
...
- 写作产物物化为 `articleDraft` / `content_factory.workspace_patch`。
- UI 显示小产物卡，点击后右侧 Product Profile 打开。
- Playwright 覆盖插件中心可见、输入栏 `@`、发送、产物卡、右栏展开。

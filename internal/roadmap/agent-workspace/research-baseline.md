...
| Lime Run Control 事实源 | `WorkspaceMainArea.tsx`、`WorkspaceGeneralWorkbenchSidebar.tsx`、`GeneralWorkbenchSidebar.tsx`、`ThreadGoalPanel.tsx`、`useAgentSessionThreadGoal.ts`、`threadGoalClient.ts`、`AgentRuntimeStrip.tsx`、`agentUiSubagentsViewModel.ts`、`permissionProjection.ts`、`projectGit.ts`、`InputbarModelExtra.tsx`、`harnessEvidenceViewModel.ts` | Lime 的目标控制只读取 thread-owned canonical `ThreadGoal`，并与 Thread / Turn / Item、runtime strip、Git API、subagent projection、permission/model/evidence 共同构成局部 run-control facts；仍缺 Codex 式统一 run rail / bottom control surface |

...
| `multi-agent-team.md` | Multi-Agent / SubAgent | 3.5 | 六个 AgentControl 工具、AgentGraph/identity/mailbox、canonical child Thread 与真实 cold-restart GUI；旧 synthetic Team scenario 已退役 |
...
| Multi-Agent / SubAgent | 3.0 | 六工具 managed Gate B 与 durable graph/mailbox 已闭环；canonical GUI 有 focused 证据，仍缺同一 batch 的完整 visible-DOM 与 cold restart 证据 |
...
| `agent-control-tools` | `.lime/qc/s4ae-agent-control-tools-gate-b-final.json`、`.lime/qc/s2o3-s4ae-agent-control-tools-gate-b-rerun.json` | `pass` | 两次 fresh managed Gate B 均为 15/15 assertions，`spawn_agent`、`list_agents`、`send_message`、`followup_task`、`interrupt_agent`、`wait_agent` 全部 completed；覆盖真实 Electron current runtime、durable child/tree/mailbox/terminal Result 与 Evidence Pack，但尚不冒充完整 visible-DOM Gate B |

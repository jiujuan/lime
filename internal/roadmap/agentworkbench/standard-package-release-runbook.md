...
截至 `2026-06-12`，scoped registry 发布、Content Studio runtime-client 接入和 GUI smoke / 产品回归均已完成。已误发的无 scope `app-server-client@1.66.0` 与依赖旧名的 `@limecloud/agent-runtime-client@0.1.0` 只作为 compat-misrelease 记录，不进入 Content Studio 接入。后续缺口已转入 v2.11 协议深水区和需要用户单独确认的 Agent App 本地 process fallback 物理删除。

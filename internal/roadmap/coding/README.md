...
| ExecutionBackend | P2 已接入 RuntimeBackend tool event mirror、真实 `patch-apply` workdir apply service、command/test/action facts、output refs、checkpoint refs、artifact/evidence refs、policy/sandbox/action metadata、tool confirmation resume、默认 policy rule catalog、组织 / 用户 / runtime / request policy layer、network rules、macOS/Linux sandbox runner 与 Windows restricted token runner；Windows runner 已落地 CreateRestrictedToken、workspace ACL 授权、denied path write deny、CreateProcessAsUserW、Job Object timeout/process-tree termination、ACL rollback 和 stdout/stderr 并发 pipe reader。P2-B first-core-slice 已新增 `agent_tools::execution::process` owner，具备 process snapshot、stdout/stderr delta、有界 retained output、stdin write、interrupt、terminate、status 与本地 process runner；App Server current 已新增 `executionProcess/start`、`executionProcess/writeStdin`、`executionProcess/interrupt`、`executionProcess/terminate`、`executionProcess/status`、`executionProcess/drainOutput` JSON-RPC 控制面，client / schema / processor 契约已接通。P2-B second slice 已让 no-sandbox shell path 使用 live process 输出 `tool.output.delta`，并在启动前复用 Lime `ToolExecutionDecision` 与 Aster `ToolRegistry::check_tool_permissions`；live process 启动成功后会立即发 metadata-only delta，把 `processId / running / live_process` hydrate 到 `thread_read.commands`，因此无 stdout 的长命令也能立刻显示控制面；`executionProcess/start` 也已收敛为受控入口：只接受 shell canonical tool，必须携带 `workingDirectory`，先过 Lime decision 与 Aster safety / permission preflight，workspace sandbox backend required / enforced 时 fail-closed，不再用 `cwd` 覆盖实际工作目录。需要 workspace sandbox backend 的命令仍留在 Aster sandbox executor，不走裸进程。主对话 shell 终态已把 embedded / sandbox / live process metadata 投影为 `processId / executionProcessStatus / executionSurface / outputBytes / outputOmittedBytes / outputTruncated / stdoutBytes / stderrBytes`，并 hydrate 到 `thread_read.commands` 与 `CodingWorkbenchView.commands`。Workbench 输出面板已通过 `src/lib/api/executionProcess.ts` 复用 App Server current client，live process 可触发 interrupt / terminate / status / drain 控制；组件只消费 injected controls，不直接散落 JSON-RPC 调用。 | 下一步把 command/test 默认执行切到 sandbox-aware process control owner，并补 stdin 交互与状态刷新 hydrate；所有 UI 控制继续复用 `executionProcess/*`，且不得绕过 policy / sandbox。Windows-only smoke 门禁已入库，随后在 Windows 机器运行并保存证据。 |
...
主任务：`P2-B live process lifecycle second slice`。目标是把 command / test 的默认执行生命周期从当前 batch outcome bridge，迁到 sandbox-aware `agent_tools::execution::process` owner；Workbench 已接入的 interrupt / terminate / status / drain 控制继续复用 `executionProcess/*` current API，不新增旧命令、不绕过 `ToolExecutionDecision`。
...
- stdin / status hydrate 进入 RuntimeCore lifecycle，Workbench 控制状态可从 `CodingWorkbenchView.commands/tests` 恢复。
- 需要 workspace sandbox backend 的命令仍走 Aster sandbox executor；no-sandbox live process 与 App Server `executionProcess/start` 都必须继续消费 `ToolExecutionDecision.requires_sandboxed_execution()`。
...
P2-B live process lifecycle second slice
  ...
  -> reuse executionProcess current API for command/test default execution, stdin and status hydrate
...
1. 继续 P2-B：Workbench live process interrupt / terminate / status / drain 控制已接入 current API；下一刀把 command/test 默认执行切到 sandbox-aware `agent_tools::execution::process` owner，并把 stdin / status hydrate 接进 RuntimeCore lifecycle；no-sandbox shell live process 已落地，不再重复做裸进程接管。

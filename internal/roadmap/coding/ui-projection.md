...
| 变更 | `changes`、`patches` | selected diff、collapse | 从 assistant summary 推断 diff。 |
...
| thread item / read model adapter | `workspaceConversationWorkbenchViewModel.ts` | migration adapter 会把旧 thread item 转 RuntimeEvent，同时把 current `threadRead` 传入 selector；旧 thread item 不再承接新 command/test/action truth。 |
...
1. `file_artifact`、`command_execution`、`approval_request`、`turn_summary` 等旧 thread item 只能作为 migration adapter 输入。
2. adapter 必须输出标准 RuntimeEvent：`file.changed`、`command.started/exited`、`patch.started/applied/failed`、`test.started/completed`、`action.required/resolved`。
...
5. 当 App Server 已能直接输出 coding RuntimeEvent 后，thread item adapter 降级为 compat 或删除。

迁移顺序：

1. `changeView` 从 `CodingWorkbenchView.changes/patches/files` 派生（已接入）。
...
7. 删除或限制旧 thread item direct reader，只保留 adapter 测试。
...
```ts
interface ContinueFixContext {
  sessionId: string;
  threadId: string;
  failedTurnId: string;
  failureKind: "patch" | "command" | "test" | "sandbox" | "provider";
  sourceIds: {
    toolCallId?: string;
    commandId?: string;
    testRunId?: string;

...
export type PluginTaskWorkerRunRequest = {
  appId: string;
  taskId: string;
  taskKind: string;
  sessionId: string;
...
export type PluginTaskWorkerRunResult = {
  status: "completed";
  runtimeEvents: WorkerRuntimeEvent[];
  artifactKind: string;
};

export type PluginTaskWorkerFailureResult = {
  status: "failed";
  runtimeEvents: WorkerRuntimeEvent[];
  errorCode: string;
  errorMessage: string;
...
export async function runPluginTaskWorker(
  request: PluginTaskWorkerRunRequest,
): Promise<PluginTaskWorkerRunResult> {
  ...
  const artifactKind =
    normalizeString(output.artifactKind) ?? "plugin.worker_output";
  ...
  if (!workspacePatch || typeof workspacePatch !== "object") {
    throw new Error("Plugin worker did not return a workspace patch object");
  }
  return {
    status: "completed",
    artifactKind,
    runtimeEvents: [
      {
        ...
        payload: {
          artifactId: `plugin-worker-${request.appId}-${request.taskId}`,
          kind: artifactKind,
...
export function buildPluginTaskWorkerFailureResult(
  request: PluginTaskWorkerRunRequest,
  error: unknown,
): PluginTaskWorkerFailureResult {
  const errorMessage = normalizeErrorMessage(error);
  const errorCode = classifyWorkerError(error, errorMessage);
  const message = `Plugin task worker failed: ${errorMessage}`;
  return {
    status: "failed",
    errorCode,
    errorMessage,
...
async function resolveWorkerPath(
  ...
  if (
    ...
    throw new Error("Plugin worker entrypoint is not a safe relative path");
  }
  const workerPath = path.resolve(root, entrypoint.replace(/^\.\//, ""));
  if (workerPath !== root && !workerPath.startsWith(`${root}${path.sep}`)) {
    throw new Error("Plugin worker entrypoint escapes package root");
  }
  const workerStat = await stat(workerPath);
  if (!workerStat.isFile()) {
    throw new Error("Plugin worker entrypoint is not a file");
  }
  return workerPath;
}
...
async function runNodeWorker(
  workerPath: string,
  request: PluginTaskWorkerRunRequest,
): Promise<PluginTaskWorkerOutput> {
  ...
  return await new Promise((resolve, reject) => {
    const child = spawn(process.execPath, [workerPath], {
      ...
      env: {
        ...
        LIME_PLUGIN_RUNTIME: "task-worker",
      },
    ...
    const timer = setTimeout(() => {
      ...
      settled = true;
      child.kill("SIGTERM");
      reject(new Error(`Plugin worker timed out after ${timeoutMs}ms`));
    }, timeoutMs);
    ...
    child.stdout.on("data", (chunk: Buffer) => {
      ...
      if (stdoutBytes > MAX_WORKER_OUTPUT_BYTES && !settled) {
        ...
        clearTimeout(timer);
        child.kill("SIGTERM");
        reject(new Error("Plugin worker output exceeded 1 MiB"));
        return;
      }
      stdout.push(chunk);
    });
    ...
    child.on("close", (code) => {
      ...
      if (code !== 0) {
        ...
        reject(new Error(message || `Plugin worker exited with ${code}`));
        return;
      }
      try {
...
function summarizeWorkerInput(request: PluginTaskWorkerRunRequest): string {
  const parts = [
...
function normalizeErrorMessage(error: unknown): string {
  ...
  return "Unknown Plugin worker error";
}

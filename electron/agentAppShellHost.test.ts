...
function createHost(request: AppServerRequestMock): AgentAppShellHost {
  return new AgentAppShellHost(
    request as ConstructorParameters<typeof AgentAppShellHost>[0],
  );
}
...
describe("AgentAppShellHost", () => {
  ...
  it("UI runtime lifecycle 命令透传 App Server current methods", async () => {
    ...
    await expect(
      ...
      method: "agentAppUiRuntime/status",
      params: {
        appId: "content-factory-app",
      },
    });

...
import {
  ...
  ELECTRON_APP_SERVER_TRUTH_ADAPTER_COMMANDS,
...
describe("electron/ipcChannels", () => {
  it("Electron host command 白名单承接本地宿主与 App Server 桥接命令", () => {
    ...
    expect(isElectronHostCommand("agent_runtime_get_tool_inventory")).toBe(true);
    expect(isElectronHostCommand("agent_runtime_list_sessions")).toBe(true);
    expect(isElectronHostCommand("agent_runtime_list_workspace_skill_bindings")).toBe(true);
  ...
  it("App Server 原始桥接命令只包含 JSONL sidecar 通道", () => {
    ...
    expect(isElectronAppServerCommand("app_server_handle_json_lines")).toBe(true);
    expect(isElectronAppServerCommand("agent_runtime_list_sessions")).toBe(false);
  });

  it("App Server truth adapter 命令由 Host adapter 投影，不走原始 JSONL 通道", () => {
    expect([...ELECTRON_APP_SERVER_TRUTH_ADAPTER_COMMANDS].sort()).toEqual([
    ...
    expect(
      ELECTRON_APP_SERVER_TRUTH_ADAPTER_COMMANDS.every((command) =>
        isElectronHostCommand(command),
      ),
    ).toBe(true);
    expect(
      ELECTRON_APP_SERVER_TRUTH_ADAPTER_COMMANDS.some((command) =>

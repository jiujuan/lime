...
test('exports app-server method catalog with request and notification kinds', () => {
  ...
  assert.equal(isAppServerNotificationMethod(METHOD_AGENT_SESSION_START), false);
});
...
test('connection wraps capability list response', async () => {
  ...
  assert.equal(result.result.capabilities[0].methods[0], METHOD_AGENT_SESSION_START);
...
test('connection wraps artifact read response', async () => {
  ...
  assert.equal(result.result.artifacts[0].artifactRef, 'artifact-document:req-1');
...
test('routes agent session event notifications for renderer projection', async () => {
  ...
  assert.equal(agentSessionEventNotification(notification)?.params.event.payload.text, 'delta');
...
test('uses codex style stdio sidecar launch args', () => {
  ...
  assert.deepEqual(sidecarArgs(config), ['--stdio', '--backend', 'unavailable']);
  assert.equal(config.expectedSha256, undefined);
  const policyConfig = stdioSidecar('/tmp/app-server', '/tmp/content-studio.policy.json');
  ...
  assert.deepEqual(sidecarArgs({ binaryPath: 'app-server', listenUrl: 'stdio://' }), [
    '--stdio',
    '--backend',
    'unavailable',
  ]);
  assert.deepEqual(sidecarArgs({ binaryPath: 'app-server', listenUrl: 'local://lime' }), [
    '--listen',
    'local://lime',
    '--backend',
    'unavailable',
...
test('resolves sidecar binary path for env resources and dev fallback', () => {
  ...
  assert.deepEqual(
    ...
    {
      binaryPath: join('/app/resources', 'app-server', 'darwin-arm64', 'app-server'),
      source: 'resources',
    },
  );
  assert.deepEqual(
...
test('selects release manifest artifact by platform and protocol', () => {
  ...
  assert.throws(
    () => assertCompatibleManifest({ ...manifest, protocolVersion: 'appserver.v9' }),
  ...
  assert.throws(
    () =>
      resolveSidecarFromReleaseManifest(
        ...
        { env: {}, resourcesPath: '/app/resources', platform: 'darwin', arch: 'arm64' },
      ),
    /unsupported app-server protocol/,
  );
});
...
test('starts packaged sidecar lifecycle from resources manifest', async () => {
  ...
  try {
    ...
    assert.equal(defaultReleaseManifestPath(resourcesPath), join(resourcesPath, DEFAULT_RELEASE_MANIFEST_NAME));
    ...
    assert.equal(started.resolved.config.backendMode, DEFAULT_STANDALONE_BACKEND_MODE);
    assert.equal(started.connected.initializeResponse.serverInfo.protocolVersion, PROTOCOL_VERSION);
    await waitFor(() => started.connected.sidecar.stderrLines.includes('initialized-packaged'));
  } finally {
...
test('spawns stdio sidecar and exchanges JSON-RPC lines', async () => {
  ...
  try {
    ...
    const client = new AppServerClient();
    const sidecar = await spawnAppServerSidecar(stdioSidecar(process.execPath), {
      args: [fakeSidecar],
    });
...
test('connects sidecar with initialize and initialized handshake', async () => {
  ...
  try {
    ...
    try {
      assert.equal(connected.initializeResponse.serverInfo.protocolVersion, PROTOCOL_VERSION);
      await waitFor(() => connected.sidecar.stderrLines.includes('initialized-received'));
...
test('calculates sidecar restart backoff policy', () => {
  assert.equal(sidecarRestartDelayMs(1, { initialDelayMs: 100, factor: 2 }), 100);
  assert.equal(sidecarRestartDelayMs(3, { initialDelayMs: 100, factor: 2 }), 400);
  assert.equal(
...
test('sidecar lifecycle restarts once after crash', async () => {
  ...
  try {
    ...
    await waitFor(() => scheduled.length === 1 && restarted.length === 1, 2_000);

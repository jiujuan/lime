...
use crate::commands::unified_memory_cmd::{build_memory_embedding_text, embedding_to_blob};
...
async fn backfill_search_embeddings(
    db: &DbConnection,
    embedding_config: &MemoryEmbeddingConfig,
    category: Option<&MemoryCategory>,
    dimension: usize,
) -> Result<usize, String> {
    if embedding_config.provider == MemoryEmbeddingProvider::Disabled || dimension == 0 {
        return Ok(0);
    }

    let candidates = {
        let conn = lock_db(db)?;
        load_embedding_backfill_candidates(&conn, category, dimension)?
    };
    ...
    tracing::info!(
        "[Memory Embedding Backfill] candidates={}, dimension={}",
        candidates.len(),
        dimension
    );
    ...
    for mut memory in candidates {
        ...
        match embed_text_with_config(db, &text, embedding_config).await {
            Ok(embedding) if embedding.len() == dimension => {
                memory.metadata.embedding = Some(embedding);
                let embedding_blob = embedding_to_blob(memory.metadata.embedding.as_ref());
                let conn = lock_db(db)?;
                conn.execute(
                    "UPDATE unified_memory SET embedding = ?1 WHERE id = ?2",
                    params![embedding_blob, &memory.id],
                )
                .map_err(|e| format!("Failed to backfill memory embedding: {e}"))?;
                updated += 1;
            }
            Ok(embedding) => {
                tracing::warn!(
                    "[Memory Embedding Backfill] skip memory id={} because embedding dim {} != query dim {}",
                    memory.id,
                    embedding.len(),
                    dimension
                );
            ...
            Err(error) => {
                tracing::warn!(
                    "[Memory Embedding Backfill] skip memory id={} because embedding failed: {}",
                    memory.id,
                    error
                );
            }
    ...
    tracing::info!(
        "[Memory Embedding Backfill] updated={}, dimension={}",
        updated,
        dimension
    );
...
pub async fn unified_memory_semantic_search(
    ...
    let query_embedding = embed_text_with_config(&db, &options.query, &memory_config.embedding)
        .await
    ...
    .await
    .map_err(|e| format!("Failed to backfill memory embeddings: {e}"))?;

...
fn build_video_skill_launch_system_prompt(
    ...
    let mut lines = vec![
        ...
        "- 不要搜索 “video_generate” 或 “lime media video generate --json” 之类目录信息；当前 video_task 已经提供了足够上下文。".to_string(),
        "- 如果某个通用搜索/读文件工具因为 session policy 被拒绝，不要重复同类调用；应立即改为直调 Skill(video_generate)。".to_string(),
        "- Skill 执行后，只能沿 video_generate skill 的 task file / App Server media task artifact 主链提交异步任务；如果 Skill 或 binding 明确不可用，应报告视频生成绑定不可用并停止，不得回退旧视频 Tauri facade 或其它命令。".to_string(),

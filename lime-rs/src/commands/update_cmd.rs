//! 更新检查命令模块
//!
//! 提供自动更新检查相关的 legacy 命令。
//! Electron Desktop Host 是当前应用内升级事实源；这里仅保留检查和手动下载兜底。
...
pub async fn start_update_install_session(
    ...
    tauri::async_runtime::spawn(async move {
        ...
        update_install_session(&app_handle_for_task, &install_state, |session| {
            ...
            session.session_id = session_id.clone();
            session.percent = 0.0;
            session.message = "当前升级安装已由 Electron updater 接管".to_string();
            session.error = Some(
                "旧升级器已下线，请使用 Electron Desktop Host 或前往发布页下载最新版".to_string(),
            );
            session.is_active = false;
            session.can_close_window = true;
            session.completed_at = Some(current_unix_timestamp());
...
pub async fn download_update(
    ...
    Ok(DownloadResult {
        success: false,
        message: "旧升级器已下线，当前升级安装由 Electron updater 接管；请使用 Electron Desktop Host 或前往发布页手动下载最新版".to_string(),
        file_path: update_info.download_url,
    })
}

use super::{args_or_default, get_string_arg, parse_request, DynError};
use serde_json::Value as JsonValue;
use tauri::{AppHandle, Manager};

pub(super) async fn handle_get_objective(
    ...
    Ok(serde_json::to_value(
        crate::commands::aster_agent_cmd::agent_runtime_get_objective(db, session_id).await?,
    )?)
}

pub(super) async fn handle_set_objective(
    app_handle: &AppHandle,
    args: Option<&JsonValue>,
) -> Result<JsonValue, DynError> {
    let request =
        parse_request::<crate::commands::aster_agent_cmd::AgentRuntimeSetObjectiveRequest>(args)?;
    let db = app_handle.state::<crate::database::DbConnection>();
    Ok(serde_json::to_value(
...
pub(super) async fn handle_update_objective_status(
    app_handle: &AppHandle,
    args: Option<&JsonValue>,
) -> Result<JsonValue, DynError> {
    ...
    let db = app_handle.state::<crate::database::DbConnection>();
    Ok(serde_json::to_value(
        crate::commands::aster_agent_cmd::agent_runtime_update_objective_status(db, request)
            .await?,
    )?)
}

pub(super) async fn handle_clear_objective(
    app_handle: &AppHandle,
    args: Option<&JsonValue>,
) -> Result<JsonValue, DynError> {
    let request = parse_request::<
    ...
    let db = app_handle.state::<crate::database::DbConnection>();
    Ok(serde_json::to_value(
        crate::commands::aster_agent_cmd::agent_runtime_clear_objective(db, request).await?,
    )?)
}

pub(super) async fn handle_continue_objective(
    app_handle: &AppHandle,
    args: Option<&JsonValue>,
) -> Result<JsonValue, DynError> {
    let request = parse_request::<
...
pub(super) async fn handle_audit_objective(
    app_handle: &AppHandle,
    args: Option<&JsonValue>,
) -> Result<JsonValue, DynError> {
    let request = parse_request::<

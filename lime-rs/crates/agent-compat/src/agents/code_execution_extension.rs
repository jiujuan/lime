            ...
            tools: vec![
                McpTool::new(
                    ...
                    indoc! {r#"
                        ...
                        - WRONG: execute_code to list → execute_code to read (2 calls)
                        - RIGHT: execute_code that lists AND reads in one script (1 call)

                        EXAMPLE - List files and read a document (ONE call):
                        ```javascript
                        import { shell, read } from "developer";
                        ...
                        [
                          ...
                          {"tool": "developer/shell", "description": "inspect git status", "depends_on": [0, 1]}
                        ]
...
mod tests {
    ...
    fn test_search_plain_text() {
        let tools = vec![
            ...
            ToolInfo {
                ...
                full_name: "developer__read".to_string(),
                description: "Read text files".to_string(),
        ...
        // Search for "read" - should match description
        let result =
            CodeExecutionClient::handle_search(&tools, &["read".to_string()], false).unwrap();
        let text = match &result[0].raw {
            RawContent::Text(t) => &t.text,
            _ => panic!("Expected text"),
        };
        assert!(text.contains("developer/read"));
    ...
    fn test_search_regex() {
        ...
        // Regex search for "^read" - should match read
        let result =
            CodeExecutionClient::handle_search(&tools, &["^read".to_string()], true).unwrap();
        let text = match &result[0].raw {
            RawContent::Text(t) => &t.text,
            _ => panic!("Expected text"),
        };
    ...
    fn test_validate_allowed_caller() {
        ...
        let denied = CodeExecutionClient::validate_allowed_caller("developer__search", &allowed);

# config

<!-- 一旦我所属的文件夹有所变化，请更新我 -->

## 架构说明

迁移期配置兼容模块，只保留 `lime-agent` 仍直接消费的最小 Aster config surface。
旧 `/config` CLI、多源配置 manager、AGENTS.md watcher、experiments 和 provider signup flow 已按
Codex 无对应 current 面删除；后续 provider/settings 能力必须进入 Lime current owner。

### 配置优先级（从低到高）

1. `default` - 内置默认值
2. `userSettings` - 用户全局配置 (~/.aster/settings.yaml)
3. `projectSettings` - 项目配置 (.aster/settings.yaml)
4. `localSettings` - 本地配置 (.aster/settings.local.yaml)
5. `envSettings` - 环境变量 (ASTER\_\*)
6. `flagSettings` - 命令行标志
7. `policySettings` - 企业策略（最高优先级）

## 文件索引

| 文件                       | 说明                             |
| -------------------------- | -------------------------------- |
| `mod.rs`                   | 模块入口和导出                   |
| `aster_mode.rs`            | Aster 运行模式定义               |
| `base.rs`                  | 基础配置结构和 YAML/Keyring 存储 |
| `declarative_providers.rs` | 声明式 Provider 配置             |
| `extensions.rs`            | 扩展配置管理                     |
| `paths.rs`                 | 路径配置                         |
| `permission.rs`            | 权限管理                         |
| `search_path.rs`           | 搜索路径配置                     |

## 更新提醒

任何文件变更后，请更新此文档和相关的上级文档。

...
impl CollabAgentExecutionBackend for AgentControlExecutionBackendAdapter {
    ...
    async fn normalize_send_target(
        ...
        normalize_send_target(session_id, target).await
    }
    ...
    async fn resolve_send_targets(
        ...
        resolve_send_targets(session_id, canonical_target).await
    }
    ...
    async fn resolve_local_peer_target(
        ...
        resolve_local_peer_target(current_session_id, address).await
    }
    ...
    async fn resolve_sender_name(
        ...
        Ok(resolve_sender_name(session_id).await)
    }
    ...
    async fn current_session_is_team_lead(
        ...
        Ok(resolve_team_context(session_id)
            .await
            .map_err(|error| {
...
async fn normalize_send_target(
    ...
    let Some(team_context) = resolve_team_context(session_id).await.map_err(|error| {
        CollabAgentSurfaceError::execution_failed(format!("读取 team 状态失败: {error}"))
    })?
    else {
...
async fn resolve_send_targets(
    ...
    if let Some(team_context) = resolve_team_context(session_id).await.map_err(|error| {
        CollabAgentSurfaceError::execution_failed(format!("读取 team 状态失败: {error}"))
    })? {
    ...
    if let Some(child_session) = resolve_named_subagent_child_session(session_id, target)
        .await
        .map_err(|error| {
            CollabAgentSurfaceError::execution_failed(format!("读取命名子 agent 路由失败: {error}"))
        })?
    {
        return Ok(vec![ResolvedCollabSendTarget {

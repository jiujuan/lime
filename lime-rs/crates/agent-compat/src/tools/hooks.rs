...
mod tests {
    ...
    async fn test_file_operation_hook() {
        let hook = FileOperationHook::new("file_hook".to_string(), vec!["Read".to_string()]);

        let context = create_test_context();
        assert!(!hook.should_execute(&context)); // TestTool 不包含 "Read"，应该不匹配

        let read_context = HookContext::new(
            "ReadTool".to_string(),
            serde_json::json!({"path": "/test/file.txt"}),
            ToolContext::new(PathBuf::from("/tmp")),
        );
        assert!(hook.should_execute(&read_context)); // ReadTool 包含 "Read"，应该匹配
    }
}

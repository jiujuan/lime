use crate::session::{
    query_session, require_shared_session_runtime_queue_service, resolve_team_context,
    save_team_membership, save_team_state, ExtensionState, Session, SessionManager,
    SessionRuntimeQueueService, SessionType, TeamMember as AsterTeamMember,
    TeamMembershipState as AsterTeamMembershipState, TeamSessionState as AsterTeamSessionState,
};
...
pub(crate) async fn execute_team_runtime_tool(
    ...
    let backend = TeamExecutionBackendAdapter;
    match tool_name {
...
impl CollabAgentTeamExecutionBackend for TeamExecutionBackendAdapter {
    async fn current_session_has_team(
        ...
        let session = query_session(session_id, false).await.map_err(|error| {
    ...
    async fn existing_team_names_except(
        ...
        existing_team_names_except(session_id).await
    }
    ...
    async fn resolve_reachable_team_members(
        ...
        resolve_reachable_team_members(team_state).await
    }

    async fn clear_team_membership(&self, agent_id: &str) -> Result<(), CollabAgentSurfaceError> {
        save_team_membership(agent_id, None).await.map_err(|error| {
    ...
    async fn resolve_local_session_peers(
        ...
        resolve_local_session_peers(current_session_id).await
    }

...
use app_server_protocol::{
    ...
    MemoryStoreListResponse, MemoryStoreReadParams, MemoryStoreReadResponse, MemoryStoreRootParams,
...
pub(crate) fn create_memory_tools(gateway: Arc<dyn MemoryStoreGateway>) -> Vec<Box<dyn Tool>> {
    vec![
        ...
        Box::new(MemorySearchTool::new(gateway.clone())),
        Box::new(MemoryAddNoteTool::new(gateway)),
    ]
}
...
impl MemoryToolBase {
    fn new(gateway: Arc<dyn MemoryStoreGateway>) -> Self {
        Self { gateway }
    }
...
impl MemoryListTool {
    fn new(gateway: Arc<dyn MemoryStoreGateway>) -> Self {
        Self {
            base: MemoryToolBase::new(gateway),
        }
    }
}
...
fn runtime_error(error: String) -> ToolError {
...
mod tests {
    ...
    #[derive(Default)]
    struct UnavailableMemoryStoreGateway;

    #[async_trait]
    impl MemoryStoreGateway for UnavailableMemoryStoreGateway {
        async fn list_memory_store(
            &self,
            _params: MemoryStoreListParams,
        ) -> Result<MemoryStoreListResponse, String> {
            Err("memoryStore/list unavailable".to_string())
        }
        ...
        async fn read_memory_store(
            ...
            Err("memoryStore/read unavailable".to_string())
        }
        ...
        async fn search_memory_store(
            ...
            Err("memoryStore/search unavailable".to_string())
        }
        ...
        async fn add_memory_store_note(
            ...
            Err("memoryStore/addNote unavailable".to_string())
        }
    }
    ...
    async fn list_tool_uses_workspace_scope_by_default() {
        let tool = MemoryListTool::new(Arc::new(UnavailableMemoryStoreGateway));
        let error = tool
            .execute(json!({}), &context())
            .await
            .expect_err("gateway should fail after params are accepted");

...
struct SeededAgentAppsDescriptor {
    apps: Vec<SeededAgentAppDescriptorEntry>,
}

struct SeededAgentAppDescriptorEntry {
    app_id: String,
    version: String,
...
pub(crate) fn materialize_seeded_agent_app_runtime_package(state: &Value) -> Result<(), String> {
    let data_root = agent_app_data_dir()?;
    materialize_seeded_agent_app_runtime_package_from_data_root(state, &data_root)
}

pub(crate) fn materialize_seeded_agent_app_runtime_package_from_data_root(
    state: &Value,
    data_root: &Path,
) -> Result<(), String> {
    let Some(fixture) = resolve_seeded_agent_app_fixture(state)? else {
        return Ok(());
    };
    ...
    let package_hash = read_json_string(state, &["identity", "packageHash"])
        .ok_or_else(|| "Seeded Agent App installed state 缺少 packageHash。".to_string())?;
...
pub(crate) fn migrate_seeded_agent_app_installed_state(mut state: Value) -> Value {
    let Some(fixture) = resolve_seeded_agent_app_fixture(&state).ok().flatten() else {
        return state;
    };
    if state
        .get("setup")
...
fn resolve_seeded_agent_app_fixture(
    state: &Value,
) -> Result<Option<SeededAgentAppFixture>, String> {
    ...
    for fixture in load_seeded_agent_app_fixtures()? {
...
fn load_seeded_agent_app_fixtures() -> Result<Vec<SeededAgentAppFixture>, String> {
    let descriptor = read_seeded_agent_apps_descriptor()?;
    descriptor
        .apps
        .into_iter()
        .map(load_seeded_agent_app_fixture)
        .collect()
}
...
fn read_seeded_agent_apps_descriptor() -> Result<SeededAgentAppsDescriptor, String> {
    let descriptor_path = seeded_agent_app_fixture_root().join("seeded-agent-apps.json");
    let value = read_json_file(&descriptor_path)?;
    if read_json_string(&value, &["schemaVersion"]).as_deref() != Some("seeded-agent-apps/v1") {
        return Err(format!(
            "Seeded Agent App descriptor schemaVersion 不支持: {}",
            descriptor_path.display()
        ));
    }
    let apps_value = value.get("apps").and_then(Value::as_array).ok_or_else(|| {
        format!(
            "Seeded Agent App descriptor 缺少 apps: {}",
            descriptor_path.display()
        )
    })?;
    let mut apps = Vec::new();
    for app in apps_value {
        apps.push(parse_seeded_agent_app_descriptor_entry(
            app,
            &descriptor_path,
        )?);
    }
    Ok(SeededAgentAppsDescriptor { apps })
}

fn parse_seeded_agent_app_descriptor_entry(
    value: &Value,
    descriptor_path: &Path,
) -> Result<SeededAgentAppDescriptorEntry, String> {
    ...
    Ok(SeededAgentAppDescriptorEntry {
        app_id,
        version,
...
fn required_descriptor_string(
    ...
    read_json_string(value, &[key]).ok_or_else(|| {
        format!(
            "Seeded Agent App descriptor 缺少 {key}: {}",
            descriptor_path.display()
        )
    })
}

fn load_seeded_agent_app_fixture(
    descriptor: SeededAgentAppDescriptorEntry,
) -> Result<SeededAgentAppFixture, String> {
    let fixture_root = seeded_agent_app_fixture_root();
    ...
    if !package_json_path.is_file() || !plugin_manifest_path.is_file() {
        return Err(format!(
            "Seeded Agent App package fixture 不完整: {}",
            fixture_dir.display()
        ));
    }
    ...
    if let Some(entrypoint) = worker_entrypoint.as_deref() {
        ...
        if !worker_path.is_file() {
            return Err(format!(
                "Seeded Agent App package fixture worker 不存在: {}",
                worker_path.display()
            ));
        }
    }

    Ok(SeededAgentAppFixture {
        fixture_dir,
...
fn replace_directory_from_fixture(source: &Path, target: &Path) -> Result<(), String> {
    if target.exists() {
        fs::remove_dir_all(target)
            .map_err(|error| format!("清理 Agent App seeded package cache 失败: {error}"))?;
    }
...
fn copy_directory_from_fixture(source: &Path, target: &Path) -> Result<(), String> {
    fs::create_dir_all(target)
        .map_err(|error| format!("创建 Agent App seeded package cache 失败: {error}"))?;
    for entry in fs::read_dir(source)
        .map_err(|error| format!("读取 Agent App seeded fixture 失败: {error}"))?
    {
        let entry =
            entry.map_err(|error| format!("读取 Agent App seeded fixture 条目失败: {error}"))?;
        ...
        let file_type = entry
            .file_type()
            .map_err(|error| format!("读取 Agent App seeded fixture 类型失败: {error}"))?;
        if file_type.is_dir() {
            copy_directory_from_fixture(&source_path, &target_path)?;
        } else if file_type.is_file() {
            if let Some(parent) = target_path.parent() {
                fs::create_dir_all(parent).map_err(|error| {
                    format!("创建 Agent App seeded package cache 父目录失败: {error}")
                })?;
            }
            fs::copy(&source_path, &target_path).map_err(|error| {
                format!(
                    "复制 Agent App seeded package 文件失败 {} -> {}: {error}",
                    source_path.display(),
                    target_path.display()
                )
            })?;

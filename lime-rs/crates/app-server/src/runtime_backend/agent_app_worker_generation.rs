...
const HOST_MANAGED_GENERATION_SCHEMA: &str = "lime.agent_app.host_managed_generation.v1";
...
pub(super) async fn prepare_agent_app_worker_request(
    ...
    for generation_request in plan.requests.iter().take(MAX_GENERATION_REQUESTS) {
        ...
        .thread_id(format!(
            ...
            string_field(worker_request, &["sessionId", "session_id"])
                .unwrap_or_else(|| "agent-app-worker".to_string())
        ))
        .turn_id(format!(
            "{}:host-managed-generation:{}",
...
fn generation_system_prompt(plan: &GenerationPlan, request: &GenerationRequest) -> String {
    let mut prompt = String::from(
        "你是 Lime App Server 托管的 Agent App 文本生成器。你只为插件 worker 生成受控文本内容，不解释流程，不输出审计说明，不调用工具。",
    );
...
mod tests {
    ...
    async fn prepare_worker_request_injects_host_managed_generation_from_fixture_provider() {
        ...
        prepare_agent_app_worker_request(&backend, &request, &mut worker_request)
            .await

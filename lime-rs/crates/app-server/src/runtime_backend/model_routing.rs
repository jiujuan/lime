...
pub(super) fn resolve_model_routing(
    request: &ExecutionRequest,
...
pub(super) fn routing_not_possible_payload(
    ...
    let mut payload = routing_decision_payload(selection, routing, readiness);
...
mod tests {
    ...
    fn routing_payload_keeps_review_fast_local_as_diagnostics_only() {
        ...
        let routing = resolve_model_routing(&request, &selection);
        ...
        let payload = routing_decision_payload(&selection, &routing, &readiness);

use crate::runtime::content_factory_final_article_streaming_events;
use crate::runtime::ensure_content_factory_workspace_patch_artifact_paths;
...
#[derive(Debug, Clone)]
pub(super) struct ContentFactorySearchPlan {
    pub(super) requests: Vec<ContentFactorySearchRequest>,
}

#[derive(Debug, Clone)]
pub(super) struct ContentFactorySearchRequest {
    pub(super) id: String,
...
pub(super) fn content_factory_search_turn_context(
    request_text: &str,
    mut context: crate::runtime::TurnContextOverride,
) -> crate::runtime::TurnContextOverride {
    context
        .metadata
    ...
    context.user_visible_input_text = Some(request_text.to_string());
    context
}

pub(super) fn enrich_content_factory_search_tool_event(event: &mut RuntimeEvent) {
    if !matches!(
...
pub(super) fn build_content_factory_search_evidence(
    requests: &[ContentFactorySearchRequest],
    outcomes: &[crate::runtime::ToolExecutionOutcome],
) -> Vec<Value> {
...
pub(super) fn update_content_factory_artifact_events(
...
pub(super) fn content_factory_final_streaming_events(
    events: &[RuntimeEvent],
) -> Vec<RuntimeEvent> {
    content_factory_final_article_streaming_events(events)
}

...
use app_server_protocol::PluginUninstallRehearsalTarget;
...
const INSTALLED_PLUGIN_STATE_SCHEMA_VERSION: u64 = 1;
...
fn list_plugin_installed_state_from_data_root(
    ...
    let installed_dir = installed_plugin_dir_from_data_root(data_root);
    ...
    let entries = fs::read_dir(&installed_dir)
        .map_err(|error| format!("读取 Plugin installed 目录失败: {error}"))?;
    for entry in entries {
        let entry = match entry {
            Ok(entry) => entry,
            Err(error) => {
                issues.push(plugin_persistence_issue(
                    "READ_FAILED",
    ...
    sort_plugin_states_by_id(&mut states);

    Ok(PluginInstalledListResponse { states, issues })
}

fn sort_plugin_states_by_id(states: &mut [Value]) {
...
fn read_plugin_installed_state_path(path: &Path) -> Result<Option<Value>, String> {
    if !path.exists() {
        return Ok(None);
    }
    let content =
    ...
    let schema_version = envelope
        ...
        .ok_or_else(|| "Installed Plugin state 缺少 schemaVersion。".to_string())?;
    if schema_version != INSTALLED_PLUGIN_STATE_SCHEMA_VERSION {
        return Err(format!(
            "不支持的 Plugin installed state schemaVersion: {schema_version}",
        ));
    }
    Ok(envelope.get("state").cloned())
}

fn plugin_persistence_issue(
...
fn uninstall_plugin_from_data_root(
    ...
    let rehearsal =
        build_plugin_uninstall_rehearsal_from_data_root(params.app_id, params.mode, data_root)?;
    let mut blocker_codes = if rehearsal.mode == "delete-data" {
        let expected = build_plugin_delete_data_confirmation_phrase(
            &rehearsal.app_id,
            rehearsal
                .package_hash
                .as_deref()
    ...
    let (removed_target_count, missing_target_count) =
        if rehearsal.mode == "keep-data" && blocker_codes.is_empty() {
            remove_plugin_install_references_from_data_root(&rehearsal.app_id, data_root)?
        } else {
            (0, 0)
        };
    ...
    Ok(PluginUninstallResponse {
        ...
        rehearsal,
        list: list_plugin_installed_state_from_data_root(data_root)?,
...
fn build_plugin_uninstall_rehearsal_from_data_root(
    ...
    let mode = match mode.as_str() {
        ...
        other => return Err(format!("不支持的 Plugin 卸载演练模式: {other}")),
    };
    let path = installed_plugin_state_path_from_data_root(data_root, &app_id)?;
    let Some(state) = read_plugin_installed_state_path(&path)? else {
        return Err(format!("Plugin 未安装: {}", app_id));
    };
    ...
    let mut targets = vec![
        ...
        plugin_uninstall_target(
            ...
            "Cached runtime package for this Plugin.",
        ),
        plugin_uninstall_target(
            "path",
    ...
    targets.push(plugin_uninstall_target(
        "namespace",
    ...
    targets.push(plugin_uninstall_target(
        ...
        true,
        data_action,
        "Optional user exports for this Plugin.",
    ));
    ...
    Ok(PluginUninstallRehearsalResponse {
        app_id,
...
fn build_plugin_delete_data_confirmation_phrase(app_id: &str, package_hash: &str) -> String {
    format!("DELETE_PLUGIN_DATA {app_id} {package_hash}")
}

fn remove_plugin_install_references_from_data_root(
    app_id: &str,
    data_root: &Path,
    ...
    remove_plugin_install_reference_paths(&paths)
}

fn remove_plugin_install_reference_paths(paths: &[PathBuf]) -> Result<(usize, usize), String> {
    let mut removed = 0;
    let mut missing = 0;
    for path in paths {
        if !path.exists() {
        ...
        fs::remove_file(path).map_err(|error| {
            format!("移除 Plugin 安装引用文件 {} 失败: {error}", path.display())
        })?;
        removed += 1;
    }
    Ok((removed, missing))
...
fn installed_plugin_state_path_from_data_root(
    ...
    validate_plugin_id_for_storage(app_id)?;
    Ok(installed_plugin_dir_from_data_root(data_root).join(format!("{app_id}.json")))
}

fn setup_plugin_state_path_from_data_root(
    data_root: &Path,
    app_id: &str,
) -> Result<PathBuf, String> {
    validate_plugin_id_for_storage(app_id)?;
    Ok(setup_plugin_dir_from_data_root(data_root).join(format!("{app_id}.json")))
}
...
fn write_installed_plugin_state(app_id: &str, state: &Value, saved_at: &str) -> Result<(), String> {
    ...
    let envelope = json!({
        "schemaVersion": INSTALLED_PLUGIN_STATE_SCHEMA_VERSION,
        "savedAt": saved_at,
        "state": state,
    });
    fs::write(
...
fn set_object_field(value: &mut Value, key: &str, next: Value) -> Result<(), String> {
    let Some(object) = value.as_object_mut() else {
        return Err("Installed Plugin state 必须是对象。".to_string());
    };
...
mod tests {
    ...
    const SEEDED_CONTENT_FACTORY_SOURCE_URI: &str =
        "https://seeded.local/plugins/content-factory-app/2.2.2.lapp";
    ...
    #[test]
    fn remove_plugin_install_reference_paths_removes_only_reference_files() {
        ...
        let (removed, missing) =
            remove_plugin_install_reference_paths(&[installed.clone(), setup.clone()])
    ...
    #[test]
    fn remove_plugin_install_reference_paths_counts_missing_reference_files() {
        let temp = TempDir::new().expect("temp dir");
        let installed = temp
            .path()
            .join("installed")
    ...
    #[test]
    fn uninstall_plugin_keep_data_removes_install_state_from_current_list() {
        let temp = TempDir::new().expect("temp dir");
        let data_root = temp.path();
        let app_id = "custom-plugin";
        let installed =
            installed_plugin_state_path_from_data_root(data_root, app_id).expect("installed path");
        let setup = setup_plugin_state_path_from_data_root(data_root, app_id).expect("setup path");
        ...
        let before = list_plugin_installed_state_from_data_root(data_root).expect("list before");
        assert_eq!(before.states.len(), 1);

        let response = uninstall_plugin_from_data_root(
    ...
    #[test]
    fn list_plugin_installed_state_does_not_inject_content_factory() {
        let temp = TempDir::new().expect("temp dir");
        let data_root = temp.path();

        let response =
            list_plugin_installed_state_from_data_root(data_root).expect("list installed");
    ...
    #[test]
    fn list_plugin_installed_state_migrates_seeded_content_factory_release_evidence() {
        let temp = TempDir::new().expect("temp dir");
        let data_root = temp.path();
        let installed =
            installed_plugin_state_path_from_data_root(data_root, "content-factory-app")
    ...
    fn saving_seeded_content_factory_materializes_runtime_package_cache() {
        ...
        let cache_dir = data_root
            .join("packages")
            .join(plugin_packages::safe_hash_path_segment(
    ...
    fn seeded_local_app_not_registered_does_not_migrate_or_materialize() {
        ...
        let state = json!({
            ...
            "identity": {
                "sourceKind": "cloud_release",
                "sourceUri": "https://seeded.local/plugins/unknown-seeded-app/1.0.0.lapp",
                "appId": "unknown-seeded-app",
                "appVersion": "1.0.0",

...
#[derive(Debug, Clone)]
pub(in crate::runtime::conversation_import) enum ImportedRuntimeEvent {
    Runtime {
        event_type: &'static str,
        payload: Value,
    },
    Tool(ImportedToolDraft),
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(in crate::runtime::conversation_import) enum ImportedToolPhase {
    Started,
    Completed,
    Failed,
}
...
#[derive(Debug, Clone)]
pub(in crate::runtime::conversation_import) struct ImportedToolDraft {
    pub(in crate::runtime::conversation_import) phase: ImportedToolPhase,
    ...
    pub(in crate::runtime::conversation_import) source: ImportedToolSource,
}

#[derive(Debug, Clone, Default)]
pub(in crate::runtime::conversation_import) struct ImportedToolSource {
...
impl ImportedRuntimeEvent {
    ...
    fn tool(draft: ImportedToolDraft) -> Self {
        Self::Tool(draft)
    }
    ...
    pub(in crate::runtime::conversation_import) fn tool_draft(&self) -> Option<&ImportedToolDraft> {
        match self {
            Self::Runtime { .. } => None,
    ...
    pub(in crate::runtime::conversation_import) fn sidecar_payload(&self) -> Value {
        match self {
            Self::Runtime { payload, .. } => payload.clone(),
            Self::Tool(draft) => json!({
                "callId": draft.call_id,
                "name": draft.name,
                "arguments": draft.arguments,
                "output": draft.output,
                "source": draft.source.to_value(),
            }),
        }
    }
...
impl ImportedToolDraft {
    fn started(
        ...
        Self {
            ...
            arguments,
            output: None,
            source: ImportedToolSource::from_value(metadata),
        }
    }
    ...
    pub(in crate::runtime::conversation_import) fn event_type(&self) -> &'static str {
        match self.phase {
            ImportedToolPhase::Started => "import.tool.started",
            ImportedToolPhase::Completed => "import.tool.completed",
            ImportedToolPhase::Failed => "import.tool.failed",
        }
    }
}

impl ImportedToolSource {
...
pub(super) fn imported_tool_started(
    call_id: Option<String>,
    name: Option<String>,
    arguments: Option<Value>,
    metadata: Value,
) -> ImportedRuntimeEvent {
    ImportedRuntimeEvent::tool(ImportedToolDraft::started(
        call_id, name, arguments, metadata,
    ))
}

pub(super) fn imported_tool_terminal(
    call_id: Option<String>,
    name: Option<String>,
    arguments: Option<Value>,
    output: Option<Value>,
    ...
    ImportedRuntimeEvent::tool(ImportedToolDraft::terminal(
        call_id, name, arguments, output, failed, metadata,
    ))
}

pub(super) fn tool_start_events_from_response_item(payload: &Value) -> Vec<ImportedRuntimeEvent> {
...
pub(super) fn tool_started_from_response_item(payload: &Value) -> ImportedRuntimeEvent {
...
pub(super) fn tool_finished_from_response_item(
    payload: &Value,
    failed: bool,
) -> ImportedRuntimeEvent {
...
pub(super) fn response_item_web_search_event(payload: &Value) -> Option<ImportedRuntimeEvent> {
    let call_id = call_id(payload)?;
    let action = payload.get("action").cloned();
    Some(imported_tool_started(
        Some(call_id),
...
pub(super) fn completed_turn_item_tool_event(payload: &Value) -> Option<ImportedRuntimeEvent> {
...
mod tests {
    ...
    fn response_item_tool_output_preserves_content_items() {
        ...
        let draft = events[0].tool_draft().expect("typed tool draft");

        assert_eq!(draft.phase, ImportedToolPhase::Completed);
        assert_eq!(draft.output.as_ref(), Some(&output));
        assert_eq!(draft.call_id.as_deref(), Some("call-structured"));
    }

    #[test]
    fn completed_dynamic_turn_item_becomes_tool_draft() {
        ...
        let draft = event.tool_draft().expect("typed tool draft");

...
pub(super) fn build_plugin_task_runtime_contract(
    state: &Value,
    app_dir: Option<&Path>,
...
pub(super) fn resolve_plugin_runtime_dir(
    ...
    if source_kind == "local_folder" {
        return resolve_local_plugin_runtime_dir(&source_uri, app_id.as_deref());
    }
...
pub(super) fn canonicalize_existing_plugin_dir(value: &str) -> Result<PathBuf, RuntimeCoreError> {
    let path = PathBuf::from(value);
    let canonical = fs::canonicalize(&path).map_err(|error| {
        RuntimeCoreError::Backend(format!(
            "无法解析 Plugin runtime 目录 {}: {error}",
            path.display()
        ))
    })?;
    if !canonical.is_dir() {
        return Err(RuntimeCoreError::Backend(format!(
            "Plugin runtime 路径不是目录: {}",
            canonical.display()
        )));
    }
    Ok(canonical)
...
pub(super) fn ensure_plugin_runtime_folder(app_dir: &Path) -> Result<(), RuntimeCoreError> {
    if !app_dir.join("package.json").is_file() {
        return Err(RuntimeCoreError::Backend(format!(
            "Plugin runtime 目录缺少 package.json: {}",
            app_dir.display()
        )));
    }
    Ok(())
...
fn resolve_local_plugin_runtime_dir(
    source_uri: &str,
    app_id: Option<&str>,
) -> Result<PathBuf, RuntimeCoreError> {
    match canonicalize_existing_plugin_dir(source_uri) {
...
fn is_retired_content_factory_fixture_path(app_id: Option<&str>, source_uri: &str) -> bool {
    ...
    let retired_suffix = format!("/{RETIRED_PLUGIN_RUNTIME_FIXTURE_DIR}/{CONTENT_FACTORY_APP_ID}");
...
mod tests {
    ...
    fn blocks_declared_worker_when_entrypoint_file_is_missing() {
        ...
        let contract = build_plugin_task_runtime_contract(&state, Some(temp.path()));
    ...
    fn detects_retired_content_factory_fixture_path() {
        assert!(is_retired_content_factory_fixture_path(
            ...
            "/workspace/aiclientproxy/lime/.lime/qc/plugins-runtime-fixtures/content-factory-app"
        ));
        assert!(is_retired_content_factory_fixture_path(
            Some(CONTENT_FACTORY_APP_ID),
            "C:\\workspace\\lime\\.lime\\qc\\plugins-runtime-fixtures\\content-factory-app"
        ));
        assert!(!is_retired_content_factory_fixture_path(
            Some("other-app"),
            "/workspace/aiclientproxy/lime/.lime/qc/plugins-runtime-fixtures/content-factory-app"

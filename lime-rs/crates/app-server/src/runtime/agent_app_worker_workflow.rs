...
#[derive(Debug, Clone)]
pub(super) struct AgentAppWorkerWorkflowContext {
    ...
    pub(super) steps: Vec<AgentAppWorkerWorkflowStep>,
...
#[derive(Debug, Clone)]
pub(super) struct AgentAppWorkerWorkflowContextInput<'a> {
...
pub(super) struct AgentAppWorkerWorkflowStep {
    id: String,
    title: String,
...
pub(super) fn build_agent_app_worker_workflow_context(
    input: AgentAppWorkerWorkflowContextInput<'_>,
    ...
    Some(AgentAppWorkerWorkflowContext {
...
pub(super) fn workflow_started_events(
...
impl AgentAppWorkerWorkflowContext {
    ...
    fn run_payload(&self, status: &str) -> Value {
        json!({
            ...
            "metadata": {
                "agentAppWorkflow": {
                    "source": "agent_app_worker_workflow",
                    "appId": self.app_id,
                    "workflowRunId": self.workflow_run_id(),
                    "workflowKey": self.workflow_key,
                    "workflowTitle": self.workflow_title,
    ...
    fn step_payload(
        &self,
        step: &AgentAppWorkerWorkflowStep,
        index: usize,
        status: &str,
        detail: Option<Value>,
    ) -> Value {
...
fn workflow_steps_from_value(value: Option<&Value>) -> Vec<AgentAppWorkerWorkflowStep> {
...
fn workflow_step_from_value(value: &Value) -> Option<AgentAppWorkerWorkflowStep> {
    let id = string_field(value, &["id", "key"])?;
    Some(AgentAppWorkerWorkflowStep {
...
fn bind_common_workflow_payload(
    ...
    ensure_object_field(payload, "source", json!("agent_app_task_worker"))?;
    ensure_object_field(payload, "backend", json!("agent_app_worker"))?;
    ...
    ensure_agent_app_workflow_metadata(payload, context, None, None, event_source)
}
...
fn bind_step_workflow_payload(
    ...
    ensure_agent_app_workflow_metadata(payload, context, Some(step), Some(step_index), event_source)
}

fn ensure_agent_app_workflow_metadata(
    payload: &mut Value,
    context: &AgentAppWorkerWorkflowContext,
    step: Option<&AgentAppWorkerWorkflowStep>,
    ...
    metadata_object.insert("agentAppWorkflow".to_string(), workflow_metadata);
    Ok(())
}
...
mod tests {
    ...
    fn builds_workflow_lifecycle_from_manifest_steps() {
        ...
        let context = build_agent_app_worker_workflow_context(AgentAppWorkerWorkflowContextInput {
    ...
    fn binds_worker_tool_progress_to_declared_workflow_step() {
        ...
        assert_eq!(
            event.payload["metadata"]["agentAppWorkflow"]["eventSource"],
            "worker_progress"
        );
        assert_eq!(
            event.payload["metadata"]["agentAppWorkflow"]["stepId"],
            "research"
        );
    }

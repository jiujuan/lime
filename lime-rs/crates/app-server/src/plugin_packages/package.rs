use app_server_protocol::PluginCloudReleaseDescriptor;
...
use app_server_protocol::PluginPackageIdentity;
use chrono::Utc;
...
pub(crate) fn inspect_plugin_local_package(
    params: PluginLocalPackageInspectParams,
) -> Result<PluginLocalPackageInspectResponse, String> {
    let app_dir_path = canonicalize_existing_plugin_dir_path(&params.app_dir)?;
    let plugin_projection = resolve_plugin_package_manifest(&app_dir_path)?;
    let inspected_at = now_iso();
    let manifest_hash = sha256_json_value(&plugin_projection.plugin_manifest)?;
    let package_hash = sha256_package(&app_dir_path, &plugin_projection.plugin_manifest)?;

    Ok(PluginLocalPackageInspectResponse {
...
pub(crate) async fn fetch_plugin_cloud_package(
    ...
    let bytes = download_plugin_package(&descriptor.package_url).await?;
    let actual_package_hash = sha256_prefixed(&bytes);
    if actual_package_hash != descriptor.package_hash {
        return Err(format!(
            "Plugin package hash mismatch for {}@{}: expected {}, got {}",
    ...
    if staging_dir.exists() {
        fs::remove_dir_all(&staging_dir).map_err(|error| {
            format!(
                "清理 Plugin package staging 目录失败 {}: {error}",
                staging_dir.display()
            )
        })?;
    }
    fs::create_dir_all(&staging_dir)
        .map_err(|error| format!("创建 Plugin package staging 目录失败: {error}"))?;
    ...
    extract_plugin_package_archive(&bytes, &staging_dir)?;
    let extracted_root = find_plugin_package_root(&staging_dir)?;
    let plugin_projection = resolve_plugin_package_manifest(&extracted_root)?;
    let actual_manifest_hash = sha256_json_value(&plugin_projection.plugin_manifest)?;
    if actual_manifest_hash != descriptor.manifest_hash {
        return Err(format!(
            "Plugin manifest hash mismatch for {}@{}: expected {}, got {}",
            descriptor.app_id, descriptor.version, descriptor.manifest_hash, actual_manifest_hash
        ));
    }
    let manifest = plugin_projection.plugin_manifest;
    ensure_manifest_matches_cloud_release(&manifest, &descriptor)?;

    if cache_dir.exists() {
        fs::remove_dir_all(&cache_dir).map_err(|error| {
            format!(
                "清理旧 Plugin package cache 目录失败 {}: {error}",
                cache_dir.display()
            )
        })?;
    }
    if let Some(parent) = cache_dir.parent() {
        fs::create_dir_all(parent).map_err(|error| {
            format!(
                "创建 Plugin package cache 目录失败 {}: {error}",
                parent.display()
            )
        })?;
    }
    fs::rename(&extracted_root, &cache_dir).map_err(|error| {
        format!(
            "写入 Plugin package cache 失败 {} -> {}: {error}",
...
fn validate_cloud_release_descriptor(
    ...
    validate_plugin_id_for_storage(&descriptor.app_id)?;
    let url = Url::parse(&descriptor.package_url)
        .map_err(|error| format!("Plugin packageUrl 非法: {error}"))?;
    if url.scheme() != "https" {
        return Err("Plugin packageUrl 必须使用 https。".to_string());
    }
    if descriptor.source_uri != descriptor.package_url {
        return Err("Plugin release descriptor sourceUri 必须等于 packageUrl。".to_string());
    }
...
fn validate_sha256_hash(field: &str, value: &str) -> Result<(), String> {
    let Some(hex) = value.strip_prefix("sha256:") else {
        return Err(format!("Plugin {field} 必须使用 sha256:<64 hex> 格式。"));
    };
    if hex.len() == 64 && hex.chars().all(|ch| ch.is_ascii_hexdigit()) {
        return Ok(());
    }
    Err(format!("Plugin {field} 必须使用 sha256:<64 hex> 格式。"))
}

async fn download_plugin_package(package_url: &str) -> Result<Vec<u8>, String> {
    let response = reqwest::get(package_url)
        .await
        .map_err(|error| format!("下载 Plugin package 失败: {error}"))?;
    if !response.status().is_success() {
        return Err(format!(
            "下载 Plugin package 失败，HTTP 状态: {}",
            response.status()
        ));
    }
    let bytes = response
        .bytes()
        .await
        .map_err(|error| format!("读取 Plugin package 响应失败: {error}"))?;
    Ok(bytes.to_vec())
}

fn extract_plugin_package_archive(bytes: &[u8], staging_dir: &Path) -> Result<(), String> {
    let reader = Cursor::new(bytes);
    let mut archive = ZipArchive::new(reader)
        .map_err(|error| format!("Plugin package 必须是 zip/lapp 格式: {error}"))?;
    for index in 0..archive.len() {
        let mut file = archive
            .by_index(index)
            .map_err(|error| format!("读取 Plugin package 条目失败: {error}"))?;
        let enclosed = file
            .enclosed_name()
            .map(PathBuf::from)
            .ok_or_else(|| format!("Plugin package 包含不安全路径: {}", file.name()))?;
        let out_path = staging_dir.join(enclosed);
        if file.name().ends_with('/') {
            fs::create_dir_all(&out_path).map_err(|error| {
                format!(
                    "创建 Plugin package 目录失败 {}: {error}",
                    out_path.display()
                )
            })?;
            continue;
        ...
        let mut output = fs::File::create(&out_path).map_err(|error| {
            format!(
                "写入 Plugin package 文件失败 {}: {error}",
                out_path.display()
            )
        })?;
        io::copy(&mut file, &mut output).map_err(|error| {
            format!(
                "解压 Plugin package 文件失败 {}: {error}",
                out_path.display()
            )
        })?;
    }
...
fn find_plugin_package_root(staging_dir: &Path) -> Result<PathBuf, String> {
    if staging_dir.join("plugin.json").is_file() {
        return Ok(staging_dir.to_path_buf());
    }
    let mut matches = Vec::new();
    collect_plugin_roots(staging_dir, &mut matches)?;
    matches.sort();
    matches.dedup();
    match matches.len() {
        0 => Err("Plugin package 缺少 plugin.json。".to_string()),
        1 => Ok(matches.remove(0)),
        _ => Err("Plugin package 包含多个 plugin.json，无法确定 package root。".to_string()),
    }
}

fn collect_plugin_roots(dir: &Path, matches: &mut Vec<PathBuf>) -> Result<(), String> {
    for entry in fs::read_dir(dir)
        .map_err(|error| format!("读取 Plugin package 目录失败 {}: {error}", dir.display()))?
    {
        ...
        if path.is_dir() {
            ...
            collect_plugin_roots(&path, matches)?;
        }
    }
    Ok(())
}
...
fn ensure_manifest_matches_cloud_release(
    ...
    if manifest_app_id.as_deref() != Some(descriptor.app_id.as_str()) {
        return Err(format!(
            "Plugin package manifest appId 与 release descriptor 不一致: expected {}",
            descriptor.app_id
        ));
    }
    ...
    if manifest_version.as_deref() != Some(descriptor.version.as_str()) {
        return Err(format!(
            "Plugin package manifest version 与 release descriptor 不一致: expected {}",
            descriptor.version
        ));
    }
    Ok(())
...
fn sha256_package(app_dir: &Path, manifest: &Value) -> Result<String, String> {
    ...
    for file in list_plugin_package_files(app_dir)? {
        let relative = file.strip_prefix(app_dir).map_err(|error| {
            format!(
                "计算 Plugin package hash 时无法生成相对路径 {}: {error}",
                file.display()
            )
        })?;
        ...
        hasher.update(fs::read(&file).map_err(|error| {
            format!("读取 Plugin package 文件失败 {}: {error}", file.display())
        })?);
        hasher.update([0]);
    }
...
fn list_plugin_package_files(app_dir: &Path) -> Result<Vec<PathBuf>, String> {
    let mut result = Vec::new();
    collect_plugin_package_files(app_dir, &mut result)?;
    result.sort();
    Ok(result)
}

fn collect_plugin_package_files(path: &Path, result: &mut Vec<PathBuf>) -> Result<(), String> {
    let entries =
    ...
    for entry in entries {
        ...
        let metadata = entry
            .metadata()
            .map_err(|error| format!("读取 Plugin package 元数据失败: {error}"))?;
        if metadata.is_dir() {
            collect_plugin_package_files(&entry_path, result)?;

...
- Use ToolSearch only for deferred extension/MCP tools, and use exact deferred tool names such as `select:mcp__context7__query-docs` or `select:mcp__playwright__browser_click` when you need to load or confirm a deferred tool. Do not use ToolSearch for already-visible native tools like Read, Write, Edit, Glob, Grep, Bash, WebFetch, or WebSearch.
...
- If ToolSearch returns `retry_allowed=false`, treat the search as terminal for that capability and stop calling ToolSearch for the same target.

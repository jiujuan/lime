    ...
    fn daemon_settings_use_camel_case_json() {
        let settings = DaemonSettings {
            ...
            backend_command: Some(
                "/app/resources/app-server/backend/content-backend.mjs".to_string(),
            ),
        ...
        assert_eq!(
            ...
            "/app/resources/app-server/backend/content-backend.mjs"
        );
        assert_eq!(value["backendArgs"][0], "--workspace");
        assert_eq!(value["backendArgs"][1], "/app/workspace");
        assert_eq!(value["backendTimeoutMs"], 45_000);
        assert_eq!(value["appPolicyPath"], "content-studio.policy.json");
    }

    #[test]
    fn daemon_settings_round_trips_external_backend_launch_options() {
        let path = temp_settings_path("external/settings.json");
        let settings = DaemonSettings {
        ...
        assert_eq!(
            DaemonSettings::load(&path).expect("load settings"),
            settings
        );

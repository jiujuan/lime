...
- Added current extension surface, runtime store, sampling, gateway dispatch, and request-user-input execution layers across MCP, thread-store, model-provider, and tool-runtime to reduce old Agent surface in production paths.
...
- Removed large old Agent-shaped areas from `agent-compat` and `agent-runtime`, including ask, subagent, session manager, legacy tool, file/search/shell/powershell implementations, while keeping the required compat owner.
...
- Updated Desktop Host, DevBridge, command runtime, Playwright, quality workflow, and Agent migration boundary guidance to keep old paths classified as retired guards or dead surfaces.
...
- Updated Agent migration boundary, production command current boundary, DevBridge command policy, model reasoning policy, and modality execution profile guards.
...
- Updated Agent migration Phase 6, refactor v1 impact audit, approval HITL decision model, provider reply backend, clawstream guardrail, benchmark progress, and version test plan.
...
- Bumped version facts to `1.100.0` across the root app, CLI npm package, Rust workspace, `lime-rs/Cargo.lock`, and release notes.

**Full changes**: `v1.99.0` -> `v1.100.0`

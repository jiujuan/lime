...
function createFixture() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "lime-i18next-cli-parity-"));
...
function runDetectTranslations(resourcesRoot) {
  return runCommand({
    args: [
      "tsx",
      path.join(REPO_ROOT, "scripts", "detect-missing-translations.ts"),
      "--format",
      "json",
      "--resources-dir",
      resourcesRoot,
...
function runLimeUnused(resourcesRoot, sourceRoot) {
  return runCommand({
    args: [
      "tsx",
      path.join(REPO_ROOT, "scripts", "i18n-unused-key-check.ts"),
      "--format",
      "json",
      "--resources-dir",
      resourcesRoot,
...
function summarizeReport(commands, detectTranslations) {
  ...
  return {
    cliStatus: {
      ...
      namespacesFound: Number(statusText.match(/Namespaces Found:\s+(\d+)/)?.[1] ?? 0),
...
function main() {
  ...
  const report = {
    ...
    decision: {
      ...
      next:
      "只有在官方 CLI 对动态 key 保护、unused family 分桶和 Patch retirement gate 具备等价覆盖后，再评估替换最薄的 i18n:check 层。",
  },
  };
...
export { runCommand, runDetectTranslations, runI18nextCli, runLimeUnused, summarizeReport };

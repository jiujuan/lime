...
function readManifest(manifestPath: string): ChromeExtensionWorkflowReport["manifest"] {
  ...
  const defaultLocale = typeof manifest.default_locale === "string" ? manifest.default_locale : null;
...
function buildPageReports(
  ...
  return htmlFiles.map((filePath) => {
    ...
    return {
      ...
      dataI18nLikeAttributeCount: countMatches(content, /\bdata-i18n(?:-[a-z]+)?\s*=/g),
...
export function analyzeChromeExtensionWorkflowReport(
  ...
  const installI18nPath = path.join(extensionRoot, "pages", "scripts", "install-i18n.js");
  const optionsScriptPath = path.join(extensionRoot, "pages", "scripts", "options.js");
...
function printHelp(): void {
  console.log(`Usage: tsx scripts/i18n-chrome-extension-workflow-report.ts [options]

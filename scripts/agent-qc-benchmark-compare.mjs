...
function readReward(trialDir) {
  ...
  const rewardJson = readJsonIfExists(path.join(trialDir, "verifier/reward.json"));
  if (!rewardJson) {
    return null;
  }
  ...
  const values = Object.values(rewardJson).filter((value) => typeof value === "number");
  if (values.length === 0) {
    return null;
  }
...
function listTrialDirs(jobDir) {
  ...
  return fs.readdirSync(resolved, { withFileTypes: true })
    ...
    .filter((trialDir) => fs.existsSync(path.join(trialDir, "result.json")) || fs.existsSync(path.join(trialDir, "verifier")));
}
...
function summarizeJob(jobDir) {
  ...
  const trials = trialDirs.map((trialDir) => {
    ...
    const rewardDetailsRef = path.join(trialDir, "verifier/reward-details.json");
    ...
    const evidenceComplete = reward != null && fs.existsSync(trajectoryRef) && fs.existsSync(rewardDetailsRef);
    return {
  ...
  const rewards = trials.map((trial) => trial.reward).filter((value) => typeof value === "number");
  const meanReward = rewards.length > 0
    ? rewards.reduce((sum, value) => sum + value, 0) / rewards.length
    : 0;
  const evidenceCompletenessRate = trials.length > 0

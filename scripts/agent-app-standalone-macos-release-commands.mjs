...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--installer-identity" && argv[index + 1]) {
      options.installerSigningIdentity = String(argv[index + 1]).trim();
      index += 1;
      continue;
    }
...
Options:
  ...
  --installer-identity <ref>      Developer ID Installer identity ref for pkg
  --notarization-profile <ref>    notarytool keychain profile ref
  --evidence <path>               Write command plan or run result JSON
  --execute                       Actually run codesign/productsign/notarytool/stapler

...
import {
  ...
  writeJsonFile,
} from "./lib/agent-app-standalone-release-secret-preflight-core.mjs";
...
function printHelp() {
  console.log(`Usage:
  node scripts/agent-app-standalone-release-secret-preflight.mjs [options]
...
Options:
  ...
  --no-updater                        Skip updater signing secret checks
...
async function main() {
  ...
  const result = buildStandaloneReleaseSecretPreflight({
    ...
    updaterEnabled: options.updaterEnabled,
  });
  ...
  console.log(
    `[agent-app-release-secret-preflight] status=${result.status} checked=${result.checkedSecretCount} missing=${result.missingSecrets.length}`,
  );
  if (result.missingSecrets.length > 0) {
    console.log(
      `[agent-app-release-secret-preflight] missing=${result.missingSecrets
        .map((item) => item.key)
        .join(",")}`,
    );
  }
  if (options.output) {
    console.log(`[agent-app-release-secret-preflight] output=${options.output}`);
  }
...
main().catch((error) => {
  console.error(
    `[agent-app-release-secret-preflight] failed: ${

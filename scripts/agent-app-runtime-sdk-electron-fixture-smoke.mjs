...
用途:
  ...
  @limecloud/agent-app-runtime browser SDK 发起 lime.agent.startTask /
...
<html lang="zh-CN">
  ...
  <body>
    <main>
      <h1>内容工厂 SDK</h1>
      <p class="muted">Agent App iframe · Host Bridge · lime.agent.startTask</p>
      <section class="panel">
    ...
    <script type="module">
      ...
      window.__limeSdkSmoke = {
        appId,
        entryKey,
        bridgeProtocol: LIME_AGENT_APP_BRIDGE_PROTOCOL,
        bridgeVersion: LIME_AGENT_APP_BRIDGE_VERSION,
  ...
  while (Date.now() - startedAt < options.timeoutMs) {
    ...
    if (frame) {
      ...
      if (
        ready?.hasSmoke &&
        ready.ready &&
        ready.protocol === "lime.agentApp.bridge" &&
        ready.version === 1
      ) {
        return { frame, ready };
      }
...
function assertSdkSmokeResult(result, backendSummary) {
  assert(result?.protocol === "lime.agentApp.bridge", "Host Bridge protocol 不正确");

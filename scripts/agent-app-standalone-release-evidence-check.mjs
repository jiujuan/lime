...
import {
  ...
  readJsonFile,
  writeJsonFile,
} from "./lib/agent-app-standalone-release-evidence-core.mjs";
...
function printHelp() {
  console.log(`Usage:
  node scripts/agent-app-standalone-release-evidence-check.mjs --evidence <release-evidence.json> [options]
...
async function main() {
  ...
  console.log(
    `[agent-app-release-evidence] status=${result.status} blockers=${result.blockers.length}`,
  );
  if (options.output) {
    console.log(`[agent-app-release-evidence] output=${options.output}`);
  }
...
main().catch((error) => {
  console.error(
    `[agent-app-release-evidence] failed: ${

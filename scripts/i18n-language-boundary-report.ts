...
function buildFileSummaries(
  ...
  return Array.from(
    ...
    .sort((left, right) => sortCountSummaries(left, right, (item) => item.file));
}
...
function buildMarkerSummaries(
  ...
  return Array.from(
    ...
    .sort((left, right) => sortCountSummaries(left, right, (item) => item.marker));
}
...
export function analyzeI18nLanguageBoundaryReport(
  ...
  const sourceDirs = options.sourceDirs.length > 0 ? options.sourceDirs : DEFAULT_SOURCE_DIRS;
  ...
  const categorySummaries = Array.from(
    ...
    .sort((left, right) => sortCountSummaries(left, right, (item) => item.category));
  ...
  return {
    ...
    summary: {
      ...
      unknownCount:
        categorySummaries.find((item) => item.category === "unknownLanguageLike")
          ?.count ?? 0,
    },
  };
}
...
export function formatI18nLanguageBoundaryReport(
  ...
  if (report.summary.unknownCount > 0) {
    ...
    for (const entry of report.entries
      ...
      .slice(0, 20)) {
      lines.push(`  - ${entry.file}:${entry.line} ${entry.marker} :: ${entry.text}`);
    }
  }
...
function parseCliArgs(argv: string[]): CliOptions {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--category" && next) {
      if (LANGUAGE_BOUNDARY_CATEGORIES.includes(next as I18nLanguageBoundaryCategory)) {

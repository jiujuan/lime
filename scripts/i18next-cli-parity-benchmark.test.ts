...
describe("i18next-cli parity benchmark", () => {
  it("应把 Lime sidecar 的 unused 报告纳入 summary", () => {
    const report = summarizeReport(
      {
        extractDryRun: { exitCode: 0, stderr: "", stdout: "  /tmp/1 [update]\n" },
        lint: { exitCode: 1, stderr: "", stdout: "Found hardcoded string: Hardcoded UI text\n" },
        status: { exitCode: 1, stderr: "", stdout: "Keys Found: 3\nNamespaces Found: 2\n" },
        types: {
          exitCode: 0,
          stderr: "",
          stdout: "Resources interface written\nTypeScript definitions written\n",
        },
        unused: {
          exitCode: 0,
          stderr: "",

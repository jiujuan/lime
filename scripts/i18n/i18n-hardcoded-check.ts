...
const SCAN_ROOT_PREFIXES = ["src/components/", "src/features/", "src/pages/"];
...
const SKIP_FILE_PATTERNS = [/\.(test|spec)\.[^.]+$/, /\.testFixtures\.[^.]+$/];
...
function visitJsxTree(
  ...
  ts.forEachChild(node, (child) =>
    visitJsxTree(file, sourceFile, child, findings),
  );
}

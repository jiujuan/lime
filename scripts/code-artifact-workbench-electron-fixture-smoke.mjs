...
用法:
  node scripts/code-artifact-workbench-electron-fixture-smoke.mjs
  ...
  console.log(JSON.stringify({
    events: [
      ...
      },
      {
        ...
        payload: {
          status: "completed",
          text: "CODE_ARTIFACT_WORKBENCH_DONE"
        }
      },
      {
...
async function createCodeArtifactSession(page, options, workspaceId) {
  ...
  const turn = await call(APP_SERVER_METHOD_SESSION_TURN_START, {
    ...
    turnId: TURN_ID,
    input: {
      text: "生成一个 TypeScript greeting 代码产物，并打开工作台验证。",
    },
    runtimeOptions: {
      stream: true,
      ...
      hostOptions: {
        asterChatRequest: {
          message: "生成一个 TypeScript greeting 代码产物，并打开工作台验证。",
  ...
  while (Date.now() - startedAt < 60_000) {
    ...
    if (hasCodeArtifactProjection(read.result)) {
      return {
  ...
  throw new Error(
    `代码产物会话未完成或未持久化 artifact.snapshot: ${JSON.stringify(
...
async function waitForSessionHydrated(page, options) {
  ...
  while (Date.now() - startedAt < options.timeoutMs) {
    const snapshot = await evaluatePageSnapshot(
      page,
      ({ artifactPath, doneText }) => {
        const text = document.body?.innerText || "";
        return {
          url: window.location.href,
          hasDoneText: text.includes(doneText),
          hasGeneratedText: text.includes("已生成代码产物"),
      ...
      },
      {
        ...
        doneText: "CODE_ARTIFACT_WORKBENCH_DONE",
      },
    );
    if (!snapshot) {
      await sleep(options.intervalMs);

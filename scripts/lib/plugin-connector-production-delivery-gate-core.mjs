...
export function buildConnectorProductionDeliveryGate(input = {}) {
  ...
  if (!guiEvidence) {
    missingRequirements.push({
      ...
      detail:
        "A GUI evidence artifact is required to prove the production delivery surfaced back to Plugin task events or UI.",
    });
  } else {
    if (!gui.statusReady) {
      missingRequirements.push({
    ...
    if (!gui.productionDeliveryVisible) {
      missingRequirements.push({
        ...
        detail:
          "GUI evidence must prove the production delivery was projected to Plugin UI or task events.",
      });
    }

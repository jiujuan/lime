...
} from "./plugin-connector-production-webhook-delivery-core.mjs";

describe("plugin connector production webhook delivery", () => {
  ...
  it("send 前要求 matching connector 的 ready preflight", () => {
    ...
    expect(
      productionPreflightReadyForConnector(
        {
          connector: "webhook",
          missingSecrets: [{ key: "LIME_PLUGIN_CONNECTOR_WEBHOOK_URL" }],
  ...
  it("CLI --help 说明 env/file target 与 send preflight gate", () => {
    const result = spawnSync(
      process.execPath,
      [
        path.resolve("scripts/plugin/connector-production-webhook-delivery.mjs"),
        "--help",
      ],

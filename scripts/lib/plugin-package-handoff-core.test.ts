...
import {
  ...
  countMarkerHits,
  parseGitStatusShort,
} from "./plugin-package-handoff-core.mjs";

describe("plugin-package-handoff-core", () => {
  ...
  it("把仍保留私有 bridge transport 的 package 判为 blocked", () => {
    const report = createPluginPackageHandoffReport({
  ...
  it("把直接调用模型或 Gateway 的 Plugin package 判为 blocked", () => {
    const report = createPluginPackageHandoffReport({
      files: {
        hostBridge: {
          exists: true,
    ...
    expect(report.verdict.blockers).toContain(
      "plugin package contains direct model/provider runtime bypass markers: 3 hit(s) in 2 file(s)",
    );

...
const EVIDENCE_PATHS = {
  ...
  translationPrPack:
    "internal/roadmap/i18n/evidence/translation-pr-pack.json",
} as const;
...
function getNamespaceNames(report: Record<string, unknown> | null): Set<string> {
  const namespaces = Array.isArray(report?.namespaces)
    ? report.namespaces
    : [];
  return new Set(
    namespaces
...
function readTranslationCoverageLocales(
  ...
  return Array.isArray(report?.locales)
    ? report.locales.filter((locale): locale is string => typeof locale === "string")
    : [];
}
...
function uniquePositiveCounts(
  counts: Record<string, number>,
): { distinct: number[]; values: Record<string, number> } {
...
export function analyzeI18nRoadmapReadinessReport(
  ...
  const evidence = {
    ...
    sourceLocaleExport: readEvidence(repoRoot, EVIDENCE_PATHS.sourceLocaleExport),
  ...
  const packageJson = readJsonObject(path.join(repoRoot, SOURCE_PATHS.packageJson));
  const localesText = readTextIfExists(path.join(repoRoot, SOURCE_PATHS.locales));
  ...
  const patchGateLegacy = readNestedRecord(evidence.patchRetirementGate.report, [
    "legacy",
  ]);
  const supportedCoverageLocales = readTranslationCoverageLocales(
    evidence.translationCoverage.report,
  );
  const sourceLocale = readString(evidence.sourceLocaleExport.report?.sourceLocale);
  ...
  const sourceKeyCounts = uniquePositiveCounts({
    ...
    translationCoverage: readNumber(translationCoverageSummary, "sourceKeyCount"),
  ...
  const p4KnownGaps = Array.isArray(evidence.p4Readiness.report?.knownGaps)
    ? evidence.p4Readiness.report.knownGaps
        .filter(isRecord)
        .map((gap): I18nRoadmapKnownGap => ({
          evidencePath: readString(gap.evidencePath) || EVIDENCE_PATHS.p4Readiness,
  ...
  const p0 = buildPhase(
    ...
    [
      ...
      buildCheck(
        ...
        readTextIfExists(path.join(repoRoot, SOURCE_PATHS.localesTest)).includes(
          "normalizeLocale",
        ) &&
          readTextIfExists(
  ...
  const p1 = buildPhase(
    ...
    [
      buildCheck(
        ...
        {
          hasChangeLimeLocale: appearanceSettingsText.includes("changeLimeLocale"),
          hasUiLocaleOptions: appearanceSettingsText.includes("UI_LOCALE_OPTIONS"),
      ...
      buildCheck(
          ...
          appSidebarTestText.includes("saveConfig"),
        {
          appSidebarTestHasLocaleChange:
            appSidebarTestText.includes("changeLimeLocale"),
          appSidebarTestHasPersistence: appSidebarTestText.includes("saveConfig"),
          appSidebarTestHasRootLang:
            appSidebarTestText.includes("document.documentElement.lang"),
        },
      ),
    ],
  ...
  const p2 = buildPhase(
    ...
    [
      ...
      buildCheck(
        ...
        {
          contentTargetEntryCount: readNumber(contentBoundarySummary, "entryCount"),
      ...
      buildCheck(
        ...
        EVIDENCE_PATHS.sourceLocaleExport,
        sourceExportContains(evidence.sourceLocaleExport.report, "Accept-Language") &&
          (sourceExportContains(
            evidence.sourceLocaleExport.report,
            "不控制 Lime 界面语言",
          ) ||
      ...
      buildCheck(
        ...
        {
          hasErrorsNamespace: hasNamespaces(evidence.sourceLocaleExport.report, [
            "errors",
          ]),
    ...
    [
      ...
      buildCheck(
        ...
        readLanguageCategoryCount(
          ...
          readLanguageCategoryCount(evidence.languageBoundary.report, "uiLocale") >
            0,
        {
  ...
  const p3 = buildPhase(
    ...
    [
      ...
      buildCheck(
        ...
        {
          glossaryExists: fileExists(path.join(repoRoot, SOURCE_PATHS.glossary)),
        },
      ),
      buildCheck(
        ...
        {
          localesWithGaps: readNumber(translationPrPackSummary, "localesWithGaps"),
      ...
      buildCheck(
        ...
        {
          prTemplateExists: fileExists(path.join(repoRoot, SOURCE_PATHS.prTemplate)),
        },
      ),
      buildCheck(
          ...
          readNumber(bundleSummary, "inlineGroupCount") > 0,
        {
          ...
          sourceLocaleKeyCount: readNumber(bundleSummary, "sourceLocaleKeyCount"),
    ...
    [
      buildCheck(
        ...
        translationCoverageReady,
        {
          ...
          missingKeyCount: readNumber(detailedCoverageSummary, "missingKeyCount"),
        },
      ),
      buildCheck(
  ...
  const p4 = buildPhase(
    ...
    [
      buildCheck(
        ...
        evidence.p4Readiness.ref.exists && readBoolean(p4Summary, "deliverablesReady"),
        {
          deliverableFailedCount: readNumber(p4Summary, "deliverableFailedCount"),
          deliverablePassedCount: readNumber(p4Summary, "deliverablePassedCount"),
    ...
    [
      buildCheck(
        ...
        EVIDENCE_PATHS.p4Readiness,
        evidence.p4Readiness.ref.exists && readBoolean(p4Summary, "acceptanceReady"),
        {
  ...
  const phaseReadyCount = phases.filter((phase) => phase.status === "ready").length;
...
export function formatI18nRoadmapReadinessReport(
  ...
  const lines = [
    ...
    "",
    "phases:",
    ...report.phases.map((phase) => `- ${phase.id} ${phase.title}: ${phase.status}`),
  ];
  ...
  if (failedChecks.length > 0) {
    lines.push("", "failed checks:", ...failedChecks.map((check) => `- ${check}`));
  }
...
function printHelp(): void {
  console.log(`Usage: tsx scripts/i18n-roadmap-readiness-report.ts [options]

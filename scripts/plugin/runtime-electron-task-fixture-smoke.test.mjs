...
describe("plugin runtime Electron task fixture smoke guard", () => {
  ...
  it("drives Plugin task commands through Electron Host facade", () => {
    const content = readSmokeScript();
    ...
    expect(content).toContain('api.invoke("plugin_runtime_start_task"');
    expect(content).toContain('api.invoke("plugin_runtime_get_task"');
    expect(content).toMatch(
      /api\.invoke\(\s*"plugin_runtime_submit_host_response"/,
    );
    expect(content).toContain('api.invoke("plugin_runtime_cancel_task"');
    expect(content).toContain(

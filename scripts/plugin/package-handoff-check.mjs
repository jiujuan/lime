...
import { createPluginPackageHandoffReport } from "../lib/plugin-package-handoff-core.mjs";
...
function printHelp() {
  console.log(`
Plugin Package Handoff Check

用法:
  node scripts/plugin/package-handoff-check.mjs --package-dir <plugin-package>
  node scripts/plugin/package-handoff-check.mjs --package-dir <plugin-package> --format json
  node scripts/plugin/package-handoff-check.mjs --package-dir <plugin-package> --check

选项:
  --package-dir PATH  要只读检查的 Plugin package 根目录。
...
function createReport(packageDir) {
  ...
  return createPluginPackageHandoffReport({

...
import {
  ...
  writeJsonFile,
} from "../lib/plugin-standalone-installer-verify-core.mjs";
...
function printHelp() {
  console.log(`Usage:
  node scripts/plugin/standalone-installer-verify.mjs --artifacts <artifacts.json> --output-root <dir> [options]
...
async function main() {
  ...
  console.log(
    `[plugin-installer-verify] status=${result.status} commands=${
      result.commands?.length ?? result.commandsRun?.length ?? 0
    }`,
  );
  if (options.evidence) {
    console.log(`[plugin-installer-verify] evidence=${options.evidence}`);
  }
...
main().catch((error) => {
  console.error(
    `[plugin-installer-verify] failed: ${

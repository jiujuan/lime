...
} from "../lib/plugin-connector-production-webhook-delivery-core.mjs";
...
function printHelp() {
  console.log(`Usage:
  node scripts/plugin/connector-production-webhook-delivery.mjs [options]
...
async function main() {
  ...
  if (!options.send) {
    ...
    console.log(
      `[plugin-connector-production-webhook-delivery] status=${dryRun.status} sendRequested=false output=${options.output || ""}`,
    );
    return;
  }
  ...
  console.log(
    `[plugin-connector-production-webhook-delivery] status=${evidence.status} httpStatus=${httpStatus} output=${options.output || ""}`,
  );
...
main().catch((error) => {
  console.error(
    `[plugin-connector-production-webhook-delivery] failed: ${

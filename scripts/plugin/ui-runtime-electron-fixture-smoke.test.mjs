...
function readSmokeScript() {
  return fs.readFileSync(
    "scripts/plugin/ui-runtime-electron-fixture-smoke.mjs",
    "utf8",
  );
}

describe("plugin ui runtime Electron fixture smoke guard", () => {
  ...
  it("uses isolated fixture app data and the formal runtime iframe surface", () => {
    ...
    expect(content).toContain("stableStringifyPluginValue(");
    expect(content).toContain("buildPluginPackageHash(");
    expect(content).toContain("buildPluginManifestHash(");
    ...
    expect(content).toContain("waitForPluginSidebarEntry(");
    expect(content).toContain("clickPluginSidebarEntry(");
    ...
    expect(content).not.toContain("sha256:fixture-plugin-ui-runtime");
  });

  it("proves current App Server JSON-RPC methods and rejects legacy Plugin commands", () => {
    const content = readSmokeScript();
    ...
    expect(content).toContain("legacyPluginCommandsSeen.length === 0");
    expect(content).toContain("missingRequiredAppServerMethods");
    expect(content).toContain("LEGACY_PLUGIN_COMMANDS");
    expect(content).not.toContain('"plugin_save_installed_state"');
    expect(content).toContain('"plugin_start_ui_runtime"');
    expect(content).toContain('"plugin_get_ui_runtime_status"');
    expect(content).toContain('"plugin_stop_ui_runtime"');
    expect(content).not.toContain("saveInstalledPluginState");
    expect(content).not.toContain(
      'window.electronAPI.invoke("plugin_start_ui_runtime"',
    );
    expect(content).not.toContain(
      'window.electronAPI.invoke("plugin_get_ui_runtime_status"',
    );
    expect(content).not.toContain(
      'window.electronAPI.invoke("plugin_stop_ui_runtime"',
    );
  });
});

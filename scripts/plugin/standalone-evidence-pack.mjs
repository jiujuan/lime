...
function printHelp() {
  console.log(`Usage:
  node scripts/plugin/standalone-evidence-pack.mjs [options]

Options:
  --evidence-dir <dir>       Directory that contains Plugin GUI evidence summaries
...
function buildEvidencePack({ flowSummaryPath, shellSummaryPath, hostSummaryPath }) {
  ...
  const checklist = [
    ...
    check(
      "AA-V2-P4-02",
      "Content Factory is launched as an independent standalone Plugin identity",
    ...
    check(
      ...
      assertions.standaloneBusinessHostReady === true &&
        ...
        standalone.businessHost?.kind === "plugin-runtime-page",
      {
  ...
  return {
    schemaVersion: "lime.plugin.standalone-evidence-pack/v1",
    generatedAt: new Date().toISOString(),
    subject: {
      roadmap: "internal/roadmap/plugin/v2",
...
function main() {
  ...
  console.log(`[plugins-standalone-evidence-pack] output=${options.output}`);
  console.log(
    `[plugins-standalone-evidence-pack] verdict=${pack.verdict.status} passed=${pack.verdict.passed}/${pack.verdict.total}`,
  );

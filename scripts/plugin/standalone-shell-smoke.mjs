...
function resolveDefaultPluginDir() {
...
const DEFAULTS = {
  ...
  timeoutMs: 180_000,
  intervalMs: 1_000,
  contentFactoryDir: resolveDefaultPluginDir(),
  evidenceDir: path.join(process.cwd(), ".lime", "qc", "gui-evidence", "plugins"),
...
function printHelp() {
  console.log(`Usage:
  node scripts/plugin/standalone-shell-smoke.mjs [options]
...
Options:
  ...
  --content-factory-dir <dir>     Content Factory Plugin directory, default ../../limecloud/content-factory-app
  ...
  return page.evaluate(
    async ({ appDir }) => {
            ...
            apps: [
              {
                ...
                enabled: true,
                packageUrl:
                  "https://lime.local/plugins/content-factory-app/releases/0.8.0/package.zip",
                packageHash:
      ...
      const runtimeProfileModule = await import(
        "/src/features/plugin/runtime-profile/index.ts"
      );
      const shellModule = await import("/src/features/plugin/shell/index.ts");
      ...
      const review = await api.reviewLocalPluginPackage({
        appDir,
        profile: hostProfile,
      });
      ...
      await api.saveInstalledPluginState({ state });
      ...
      const result = await api.launchPluginShell({
  ...
  try {
    ...
    assert(
      launch.result.shellWindow?.label ===
        "plugin-shell-content-factory-app-standalone",

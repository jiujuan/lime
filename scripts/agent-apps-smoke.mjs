    ...
    apps: [
      {
        ...
        packageHash: "package-fnv1a-smoke-formal",
        manifestHash: "manifest-fnv1a-smoke-formal",
...
async function main() {
  ...
  try {
    ...
    assert(
      (await page.locator('button[title="Agent Apps"]').count()) > 0,
      "Agent Apps nav should be visible",
    );
    assert(
      (await page.locator('button[title="Agent App Lab"]').count()) === 0,
      "Agent App Lab nav should stay hidden in formal smoke",
    );

    logStage("open-agent-apps");
    await page.click('button[title="Agent Apps"]');
    ...
    await page.waitForFunction(
      () =>
        !document
          .querySelector('[data-testid="agent-apps-install-cloud-content-factory-app"]')
    ...
    const assertions = {
      ...
      cleanupEvidenceRehearsalOnly: cleanupEvidence.completionEffect === "rehearsal-only",

...
function isSourceValue(value: unknown): value is string | number | boolean | null {
  return (
    value === null ||
    typeof value === "string" ||
    typeof value === "number" ||
...
export function buildTranslationPrPackReport(
  ...
  const sourceExport = analyzeI18nSourceLocaleExport({ resourcesDir, sourceLocale });
  ...
  const localeProposals = translationResult.issues.map((issue) => {
    ...
    return {
      ...
      proposedEntryCount,
      sourceKeyCount: localeCoverage?.sourceKeyCount ?? translationResult.sourceKeyCount,
...
export function runCli(argv = process.argv.slice(2)): number {
  ...
  const report = buildTranslationPrPackReport(options.resourcesDir, options.sourceLocale);

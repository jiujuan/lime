...
P0 只做“只读 App Host 骨架”：客户端能读取一个 Agent App v0.2 本地 package / fixture，得到稳定的 manifest、projection、readiness、cleanup dry-run，并在 Lab 页面展示出来；不执行 App UI，不执行 App worker，不写入主路径 registry。
...
flowchart TD
  ...
  Fixture --> Parse[parseManifest]
  Parse --> Normalize[normalizeManifest]
  Normalize --> Validate[validateManifest]
  Validate --> Project[projectApp]
...
`AppManifest` 对齐 Agent App v0.2 声明，P0 只解析核心字段：
...
const p0HostCapabilityProfile: HostCapabilityProfile = {
  appRuntimeVersion: '0.2.0',
  runtimeTargets: ['local'],
  capabilities: {
    'lime.ui': { version: '0.1.0', enabled: false, implementation: 'none' },
    'lime.storage': { version: '0.1.0', enabled: false, implementation: 'none' },
    'lime.agent': { version: '0.1.0', enabled: false, implementation: 'none' },
    'lime.knowledge': { version: '0.1.0', enabled: false, implementation: 'none' },
    'lime.artifacts': { version: '0.1.0', enabled: false, implementation: 'none' },
    'lime.evidence': { version: '0.1.0', enabled: false, implementation: 'none' }
  },
...
Agent App Lab
├── Package Source
│   └── content-engineering-app fixture
├── App Card
...
`content-engineering-app.json` 应该覆盖足够多字段，避免 P0 只验证玩具样例：
...
```json
{
  ...
  "status": "draft",
  "appType": "domain-app",
  "description": "AI 内容工程化 App fixture",
  "runtimeTargets": ["local"],
  "requires": {
    "lime": {
  ...
  "entries": [
    ...
    { "key": "scene_exhaustion", "kind": "workflow", "title": "场景穷尽", "workflow": "./workflows/exhaust-scenes.workflow.md" }
  ]
}
```
...
| 测试 | 输入 | 期望 |
|---|---|---|
| parse valid fixture | content engineering fixture | 返回 `AppManifest`。 |

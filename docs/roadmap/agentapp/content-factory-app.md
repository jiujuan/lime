...
内容工厂只实现内容业务层，不实现 Lime 底层 AI 平台层。所有 Lime 功能先看 `docs/roadmap/agentapp/capability-sdk.md` 的全量能力地图；内容工厂 MVP 消费其中一部分，后续扩展也必须继续走 `lime.*` capability，而不是新增 `content_factory_*` 专用后端命令或裸模型 API。

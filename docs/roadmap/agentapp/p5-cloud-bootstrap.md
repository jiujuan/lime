...
```bash
rg -n "scene_exhaustion|content_engineering|shenlan-content-engineering|scene\b|home\b" docs/roadmap/agentapp
```
...
| 类型与命令边界 | `npm run typecheck`、`npm run test:contracts`、`git diff --check -- docs/roadmap/agentapp src/features/agent-app src/vite-env.d.ts` 均通过。 |

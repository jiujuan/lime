...
| `packaging`       | `PackageDescriptor`、`MacOsStandaloneIdentity`、`StandaloneReleasePlan`、`TauriConfigWritePlan`、`TauriConfigWriter`、`TauriBuildRunner`、`ReleasePipelineGate`、`MacOsReleaseCommandPlan`、`UpdaterPublisher`、`ProductionArtifactBuilderPort`、`UpdaterManifestPlan` | descriptor hash / target validation / macOS identity gate / release blocker projection / deterministic config/env write plan / controlled filesystem writer executor / Node writer CLI adapter / Tauri build runner command gate / artifact-level release pipeline gate / macOS release command plan / updater publisher local manifest writer / artifact builder blocked adapter / updater manifest blocked seam | deterministic hash tests / macOS identity warning tests / release plan blocker tests / config write plan tests / writer executor tests / Node writer CLI tests / Tauri build runner tests / release pipeline gate tests / macOS release command tests / updater publisher tests / artifact builder adapter tests / updater manifest blocker tests | 新 target validator / remote upload adapter / Windows signing adapter | 假装生产签名 ready，生成伪 artifact、提前发布 updater manifest，或复用 Lime Desktop Bundle ID。 |

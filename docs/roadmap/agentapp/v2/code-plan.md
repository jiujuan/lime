...
| `shell`           | `ShellDescriptor`、`ShellChromeDescriptor`、`ShellLaunchPort`、`resolveShellLaunchDescriptorForInstalledEntry`                                                                                                                                                         | descriptor factory / launch resolver / 单 App chrome policy / native close policy seam / standalone runtime env gate / per-app native action router / label-key menu spec / native tray spec / native chrome spec / Tauri app menu 与单 App TrayIcon lifecycle adapter                                                                                                                                            | shell launch tests / chrome policy tests / close event policy tests / standalone env tests / deep link / menu / tray routing tests                                                                                                                                                                                                                 | 新 shell adapter / 产品壳 adapter                                     | 复制 Desktop runtime service、暴露多 App 管理面，或让 UI 直接拼 descriptor。                    |
| `packaging`       | `PackageDescriptor`、`MacOsStandaloneIdentity`、`StandaloneReleasePlan`、`TauriConfigWritePlan`、`TauriConfigWriter`、`TauriBuildRunner`、`ReleasePipelineGate`、`MacOsReleaseCommandPlan`、`UpdaterPublisher`、`ReleaseSecretPreflight`、`ReleaseGateWorkflow`、`InstallerVerificationPlan`、`FinalReleaseEvidenceCheck`、`ProductionArtifactBuilderPort`、`UpdaterManifestPlan` | descriptor hash / target validation / macOS identity gate / release blocker projection / deterministic config/env write plan / controlled filesystem writer executor / Node writer CLI adapter / Tauri build runner command gate / artifact-level release pipeline gate / macOS release command plan / updater publisher local manifest writer / CI secret presence preflight / manual GitHub release gate workflow / installer verification command gate / final release evidence + artifact file/hash checker / artifact builder blocked adapter / updater manifest blocked seam | deterministic hash tests / macOS identity warning tests / release plan blocker tests / config write plan tests / writer executor tests / Node writer CLI tests / Tauri build runner tests / release pipeline gate tests / macOS release command tests / updater publisher tests / release secret preflight tests / workflow YAML syntax check / installer verification tests / final release evidence and artifact file/hash tests / artifact builder adapter tests / updater manifest blocker tests | 新 target validator / remote upload adapter / Windows signing adapter | 假装生产签名 ready，生成伪 artifact、提前发布 updater manifest、跳过签名后安装器验证、打印 secret 值，或复用 Lime Desktop Bundle ID。 |
| `install`         | `InstalledAgentAppState`、cleanup plan、delete-data confirmation gate、`executeAgentAppDeleteData`                                                                                                                                                                     | state mapper / migration / rehearsal evidence / residual audit / destructive gate / 注入式 filesystem executor seam / post-delete residual audit / Rust confirmation adapter / UI exact phrase gate                                                                                                                                                                                                              | state / cleanup / lifecycle gate / delete-data executor tests / Rust delete-data adapter tests / post-delete residual audit tests / Agent Apps UI uninstall gate tests / Agent Apps smoke                                                                                                                                                          | 新 migration plan / installer-level uninstall audit                   | 未经 rehearsal、越界检查、执行后残留审计和精确确认短语就覆盖或删除用户数据，或让 ref 目标被当成真实路径删除。 |
...
| `AgentAppCleanupService`         | installed state + install mode        | cleanup plan / execution evidence | 支持 disable / keep data / delete data；delete-data 必须经 gate、port 与 post-delete residual audit 执行。 |
...
- `packaging` 模块只生成 identity descriptor、validation result、release command plan 或 installer verification plan；默认不直接调用 `codesign`、`notarytool`、`spctl`、`signtool` 或修改系统 keychain；真实签名、公证与安装器验证必须通过显式 release adapter / CI 执行并留下 evidence。

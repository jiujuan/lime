...
| [`release-operator-runbook.md`](./release-operator-runbook.md) | 真实 standalone 发布执行手册，列出 release secrets、workflow 输入、发布顺序和 final evidence 格式。 |
| [`mainline-submission-plan.md`](./mainline-submission-plan.md) | 主干状态与发布预案，记录 release gate 进入 main 后的阻断、候选写集和 selective staging 边界。 |
| [`evidence/agentapp-v0.8-release.json`](./evidence/agentapp-v0.8-release.json) | AgentAPP 标准仓库 v0.8.0 的 main / tag / GitHub Release / npm package 机器可读发布证据。 |
| [`evidence/release-gate-run-26069161772.json`](./evidence/release-gate-run-26069161772.json) | 远程 `Agent App Standalone Release Gate` 首跑的非敏感 JSON 证据，证明缺失 release 配置会 fail-closed。 |
...
7. 进入真实发布环境时，按 [`release-operator-runbook.md`](./release-operator-runbook.md) 配置 secrets、执行 workflow 和生成 final release evidence。
8. 准备主干提交时，按 [`mainline-submission-plan.md`](./mainline-submission-plan.md) 做 selective staging，避免误带并行写集。

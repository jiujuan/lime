...
- 不把开发者工具的发布认证改成宿主代理业务；`lime.cloudSession` 只作为通用会话能力。
...
| Updater 签名 key | 每个 standalone App / channel 默认独立 Tauri updater signing key；它不是 Apple 证书。 | 降低更新链路 blast radius；一个 App 的 updater key 轮换或泄露不应影响其他 App。 |
...
- `lime.cloudSession` 只承担通用登录、会话和 token mint seam；开发者工具可 just-in-time 取 token，但宿主不能代业务发布，也不能把 token 当成长期状态保存。

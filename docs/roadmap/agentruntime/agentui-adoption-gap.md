...
> 更新时间：2026-05-17
> 对比来源：`/Users/coso/Documents/dev/ai/limecloud/agentui` v0.6.1、Lime AgentRuntime / Agent App current 路线图与 Host Run UI first-cut
> 目标：明确 AgentUI 标准中哪些能力可以进入 Lime Agent App，哪些必须适配后进入，哪些暂缓，避免业务 App 为了使用 AI Agent 重写一套运行 UI。
...
| 业务 App 直连模型 API 或自建运行 UI | dead | 与 Lime Agent 工具定位冲突，不进入新 Agent App。 |
...
对 Lime 来说，AgentUI 的价值是把“Claw 那种过程可见的 AI 运行现场”抽象成可复用规则，而不是要求内容工厂或每个 Agent App 各自实现一遍。

## 3. 可直接用于 Agent App 的部分

这些能力与 Lime current 主链一致，可以作为 Agent App 的立即验收标准。
...
这些能力不能直接进入 Agent App，原因不是它们不重要，而是直接采用会破坏 Lime 的事实源边界。

| AgentUI 标准内容 | 处理方式 | 原因 |
| --- | --- | --- |
| AgentUI standalone demo layout | 暂缓 | Demo 是 reference，不是 Lime 设计系统；不能让 Agent App 复制另一套视觉语言。 |
...
| AgentUI Surface | Lime current 支持 | 缺口 | Agent App 结论 |
| --- | --- | --- | --- |
...
2026-05-17 继续只读核对当前工作树时，发现 Lime 并不是完全没有 AgentUI 基础，而是 **Claw / Chat 主链已有较完整的 AgentUI projection 资产，Agent App Host 尚未接入这条事实源**。
...
```text
不是“先新建 AgentUI adapter”，而是“把 Agent App Host 接到 Lime 已有 AgentUI projection adapter / store 上”。
```
...
这说明 Lime 并不是“缺 AgentUI 标准类型”，真正缺的是 **Agent App Host Run UI 的 surface 接入深度**。因此可用子集要按两层判断：
...
| Claw / Chat projection 层 | `buildAgentUiProjectionEvents` 已覆盖 runtime status、stream text、thinking、tool lifecycle、HITL、queue、context、artifact、evidence、model / routing / cost metric、subagent / team / worker notification | 这证明 Lime 主 App 可以承载 AgentUI projection，但不等于 Agent App Host 已全部渲染。 |
...
优先级最高的下一刀不是继续美化内容工厂页面，也不是重新造一个 AgentUI adapter，而是把 Agent App Host 接到 Lime 已有 AgentUI projection seam：
...
| 后端也要共享 Lime 能力 | 当前脏工作树已有 `runtime_turn.rs` Agent App task projection first-cut 和 `agent_app_runtime_cmd/*` 分模块化 | partial | 不夹写；由后端写集持有人完成 ToolRuntime / Connector 深水位 smoke。 |

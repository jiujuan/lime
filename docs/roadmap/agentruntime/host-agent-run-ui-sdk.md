...
- `npm test -- "src/features/agent-app/ui/AgentAppRuntimePage.test.tsx"`：通过，覆盖 `lime.ui.openAgentRun / updateAgentRun / closeAgentRun`、Host dock/drawer、模型/Token/费用/Skill、Claw 工具名、Claw 工具过程摘要、Claw Tool / Skill 步骤 renderer、Claw thinking renderer、Claw execution / output Markdown renderer、流式分组合并、facts 和终态折叠。
...
- `cargo test -p lime agent_app_required_skill --lib`：通过，覆盖 Agent App required Skill 参数保留 `agentTaskContract` 快路径输入、`LimeSkillTool` fast path 真实执行、ToolEnd metadata 可被 Host task projection 消费。
...
- `npm run typecheck`：通过。
- `git diff --check -- "src-tauri/src/commands/aster_agent_cmd/runtime_turn.rs" "docs/roadmap/agentruntime/host-agent-run-ui-sdk.md"`：通过。
...
- `cargo test -p lime agent_app_output_contract --lib`：通过，覆盖三条边界：模型已返回 fenced JSON patch 时保留模型结构；`strategy_report` 只有自然语言时物化为可复核 workspace patch；`content_batch` 没有 patch 时不伪造内容条目。
- `cargo test -p lime agent_app_required_skill --lib`：通过，确认 required Skill pre-execution 与 ToolEnd projection 仍可用。
...
- `cargo test --manifest-path "src-tauri/crates/agent/Cargo.toml" agent_app_content_factory_skill_should_use_fast_path_for_text_args`：通过；修正模型二次调用 `Skill(content-reviewer)` 且 `args` 是自然语言时可能进入 nested Skill 长跑的问题。
- `cargo test --manifest-path "src-tauri/Cargo.toml" -p lime agent_app_output_contract --lib`：通过，4 tests；新增 `script_batch` review draft materialization，仍保持 `content_batch` 无结构化 patch 时不伪造内容。
...
| 不和并行进程打架 | 本轮先只读审计并行 `scripts/agent-apps-smoke.mjs`，确认 Host task record 判定是主线阻塞后只接管 `taskAccepted / expectedSkillsInvoked` 两个最小补丁点；继续避让 `docs/roadmap/agentapp/*` 和 0.7 写集 | covered | 若继续扩展 chained seed / 多动作 flow，需要继续按脚本写集合并窗口处理 |

...
当前已经具备 README 与 Release Notes 的最小 `zh-CN / en-US` companion 覆盖，并有可机器读取的 release docs translation scope；但仍**没有**独立的官网文档 / 帮助文档 locale 构建工作流。
...
4. 文档站仍是单语言 Nuxt Content 站，没有 locale 路由，也没有 translation pipeline；inventory 会额外统计 `docs/content` 内英文 companion 文件数量，当前应保持为 `0`。
5. 商业、OEM、帮助文档等内容虽然分目录，但还没有 companion 文件、术语校验和回滚策略。

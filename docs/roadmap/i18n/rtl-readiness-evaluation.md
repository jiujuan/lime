...
- `docs/roadmap/i18n/evidence/rtl-screenshot-smoke-report.md` 与 `docs/roadmap/i18n/evidence/rtl-playwright-smoke-report.json` 已落盘，并附带 `rtl-home-fullpage.png`、`rtl-settings-fullpage.png`、`rtl-user-menu-fullpage.png` 以及自动化版本的 `rtl-home-automated.png`、`rtl-settings-automated.png`、`rtl-user-menu-automated.png`，记录一次强制 `rtl` 下的首页、设置页与用户菜单 smoke。
...
3. 这次已经补了人工截图与自动化 smoke 基线，但仍然只是强制 `dir=rtl` 的页面级验证，尚未把 `ar` / `fa-IR` 纳入主支持列表。
...
- 最后把这次人工 smoke 固化为 Playwright 断言，复核语言切换与主路径交互。
...
3. Playwright smoke 能证明 RTL 下设置页、侧栏、Workspace 和弹窗仍可用。

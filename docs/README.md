...
- Aster / Codex Agent 迁移规则：`../AGENTS.md`、`../internal/aiprompts/governance.md`、`../internal/roadmap/astermigration/`；按 `/Users/coso/Documents/dev/rust/codex` 判定能力去留，Codex 有的能力迁入 Lime current 并接入真实 App Server / 前端 / Evidence 主链，Codex 没有的 Aster-only 能力直接按 `dead / deleted / forbidden-to-restore` 清理。命名优先短、领域化、接近 Codex / Aster 的清晰词，不把 `lime_*`、`aster_*`、`agent_runtime_*` 带进 current API。

...
trait AsterRuntimeHostBridge: Send + Sync {
    ...
    async fn respond_action(&self, request: AgentRuntimeRespondActionRequest)
        -> Result<(), String>;
}
...
impl AsterRuntimeHostBridge for RuntimeCommandContextHost {
    ...
    async fn cancel_turn(
        ...
        let cancelled = self.runtime.state().cancel_session(&session_id).await;
        ...
        let gate_released = finish_active_runtime_turn_if_matches_service(&session_id, &turn_id)?;
...
impl RuntimeEventBridgeSubscriptions {
    ...
    fn register(&self, request: &AsterBackendSubmitRequest) -> Option<String> {
        if let Some(direct_event_scopes) = &self.direct_event_scopes {
            ...
            return Some(app_server_event_bridge_key(&request.session.session_id));
        }

        let app_server_bridge = self.app_server_bridge.as_ref()?.get()?.clone();
        let bridge_key = app_server_event_bridge_key(&request.session.session_id);
...
impl AsterBackendHost for TauriAsterBackendHost {
    async fn submit_turn(
        ...
        self.runtime_host
            .ensure_persisted_session(&request)
            .await
            .map_err(RuntimeCoreError::Backend)?;
        let runtime_request = aster_chat_request_from_backend_request(&request);
        let queued_task =
            build_queued_turn_task(runtime_request).map_err(RuntimeCoreError::Backend)?;
        let bridge_key = self.event_subscriptions.register(&request);
        if let Err(error) = self
            .runtime_host
            .submit_runtime_turn(
                queued_task,
                request.queue_if_busy,
                request.skip_pre_submit_resume,
            )
            .await
        {
    ...
    async fn respond_action(
        ...
        self.runtime_host
            .respond_action(runtime_action_request_from_backend_action_request(request))
            .await
            .map_err(RuntimeCoreError::Backend)?;
...
fn runtime_action_request_from_backend_action_request(
    request: AsterBackendActionRespondRequest,
...
fn append_lime_agent_event_to_app_server_bridge(
    ...
    let turn_id = runtime_event_turn_id(&runtime_event).or_else(|| {
        let fallback_turn_id = fallback_turn_id.trim();
        if fallback_turn_id.is_empty() {
            None
        } else {
            Some(fallback_turn_id.to_string())
        }
    });
    app_server_bridge
        .append_external_runtime_events(&session_id, turn_id.as_deref(), vec![runtime_event])
        .map_err(|error| {
            format!(
...
fn append_lime_agent_payload_to_app_server_bridge(
    ...
    let should_close = should_close_app_server_event_bridge(&lime_agent_event);
    let runtime_event = runtime_event_from_lime_agent_event(&lime_agent_event)?;
    app_server_bridge
        .append_external_runtime_events(session_id, Some(turn_id), vec![runtime_event])
        .map_err(|error| {
            format!(
                "failed to append runtime event to app server: code={}, message={}",
                error.code, error.message
...
async fn ensure_persisted_runtime_session(
    ...
    match crate::agent::AsterAgentWrapper::get_session_sync(
        runtime.db(),
        &request.session.session_id,
    ) {
        Ok(_) => Ok(()),
...
mod tests {
    ...
    use app_server::{
        ...
        ArtifactSummary, MockBackend, RuntimeCore, RuntimeHostContext, RuntimeOptions,
    };
    ...
    impl Default for FakeAsterRuntimeHostBridge {
        fn default() -> Self {
            Self {
                ...
                action_requests: Arc::new(Mutex::new(Vec::new())),
    ...
    impl FakeAsterRuntimeHostBridge {
        ...
        fn ensure_calls(&self) -> Vec<String> {
            self.ensure_calls.lock().expect("ensure calls lock").clone()
        }
        ...
        fn cancel_requests(&self) -> Vec<AsterBackendCancelRequest> {
        ...
        fn action_requests(&self) -> Vec<AgentRuntimeRespondActionRequest> {
            self.action_requests
                .lock()
                .expect("action requests lock")
                .clone()
        }
    }
    ...
    impl AsterRuntimeHostBridge for FakeAsterRuntimeHostBridge {
        ...
        async fn submit_runtime_turn(
            ...
            self.submit_flags
                .lock()
                .expect("submit flags lock")
                .push((queue_if_busy, skip_pre_submit_resume));
            self.submit_result
                .lock()
    ...
    async fn tauri_aster_backend_host_submit_persists_registers_and_delegates_runtime_turn() {
        ...
        assert_eq!(runtime_host.ensure_calls(), vec!["sess_submit".to_string()]);
        assert_eq!(
    ...
    async fn tauri_aster_backend_host_cancel_delegates_to_runtime_host() {
        ...
        let requests = runtime_host.cancel_requests();
        assert_eq!(requests.len(), 1);
        assert_eq!(requests[0].session.session_id, "sess_submit");
        assert_eq!(requests[0].turn.turn_id, "turn_submit");
        assert_eq!(requests[0].event_name, "agentSession/event/sess_submit");
    }

    #[tokio::test]
    async fn tauri_aster_backend_host_respond_action_maps_protocol_params_to_runtime_request() {
        let runtime_host = FakeAsterRuntimeHostBridge::default();
        let registry = FakeRuntimeEventBridgeRegistry::default();
        let host = app_server_host_with_fakes(Arc::new(runtime_host.clone()), registry, None);
        ...
        let requests = runtime_host.action_requests();
        assert_eq!(requests.len(), 1);
        let request = &requests[0];
        assert_eq!(request.session_id, "sess_submit");
        ...
        assert_eq!(request.response.as_deref(), Some("allow"));
        assert_eq!(
            request.user_data.as_ref().expect("user data")["reason"],
            "user-approved"
        );
        assert_eq!(
            request.metadata.as_ref().expect("metadata")["source"],
            "app-server"
        );
        assert_eq!(
        ...
        let scope = request.action_scope.as_ref().expect("action scope");
    ...
    async fn runtime_event_bridge_subscription_replaces_existing_listener_and_closes_on_terminal_event(
        ...
        assert_eq!(
            subscriptions.register(&request).as_deref(),
            Some("sess_bridge")
        );

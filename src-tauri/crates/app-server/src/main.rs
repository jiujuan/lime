...
async fn main() -> anyhow::Result<()> {
    ...
    match config.backend_mode {
        AppServerBackendMode::External => {
            let backend_config = config.external_backend_config()?;
            let server = match config.app_policy_path.as_deref() {
                Some(path) => AppServerRuntimeFactory::external_app_server_with_capability_source(
                    backend_config,
                    Arc::new(load_app_policy_source(path)?),
                ),
                None => AppServerRuntimeFactory::external_app_server(backend_config),
            };
            run_stdio(server).await?;
        }
        AppServerBackendMode::Mock => {
            let server = match config.app_policy_path.as_deref() {
                Some(path) => AppServerRuntimeFactory::mock_app_server_with_capability_source(
                    Arc::new(load_app_policy_source(path)?),
                ),
                None => AppServerRuntimeFactory::mock_app_server(),
            };
            run_stdio(server).await?;
        }
        AppServerBackendMode::Unavailable => {
            let server = match config.app_policy_path.as_deref() {
                Some(path) => {
                    AppServerRuntimeFactory::unavailable_app_server_with_capability_source(
                        Arc::new(load_app_policy_source(path)?),
                    )
                }
                None => AppServerRuntimeFactory::unavailable_app_server(),
            };
            run_stdio(server).await?;
        }
    }
